...
async function main() {
  ...
  const usersResponse = await authedFetch(adminToken, '/iam/users?page=1&pageSize=5');
  if (usersResponse.status !== 200) {
    throw new Error(`Expected 200 from /iam/users, got ${usersResponse.status}`);
  }
  ...
  if (createResponse.status !== 201 && createResponse.status !== 200) {
    throw new Error(`Create user failed: ${createResponse.status} ${await createResponse.text()}`);
  }
  ...
  if (deniedResponse.status !== 403) {
    throw new Error(`Expected 403 for patient write, got ${deniedResponse.status}`);
  }
