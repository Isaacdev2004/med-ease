...
async function waitForFetch(label, url, predicate) {
  ...
  throw new Error(
    `${label} did not become ready within ${STARTUP_TIMEOUT_MS}ms (${url})`,
  );
}
...
async function main() {
  ...
  await probe('API metrics', `${API_BASE}/metrics`, (response, body) => {
    ...
    if (
      !body.includes('medease_http_requests_total') &&
      !body.includes('medease_process_cpu')
    ) {
