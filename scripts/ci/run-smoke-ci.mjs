...
try {
  ...
  runStep(
    'Patients E2E certification',
    join(root, 'scripts/ci/e2e-patients.mjs'),
  );

  runStep(
    'HTTP repository contract tests',
    join(root, 'scripts/ci/run-http-contract.mjs'),
  );

  runStep(
    'OpenAPI regeneration gate',
    join(root, 'scripts/ci/verify-openapi-regen.mjs'),
  );
} catch (error) {
  shutdown();
