...
describe('PermissionEngine', () => {
  ...
  it('allows a granted permission', () => {
    assert.equal(hasPermission(['patients.read', 'patients.write'], 'patients.read'), true);
  ...
  it('applies deny rules with higher precedence than grants', () => {
    ...
    assert.equal(hasPermission(effective.effective, 'patients.read', effective.denies), true);
    assert.equal(hasPermission(effective.effective, 'patients.write', effective.denies), false);
  });

  it('evaluates ALL mode requirements', () => {
    const grants = ['patients.read', 'patients.write'];
    assert.equal(
      evaluatePermissionRequirement(grants, ['patients.read', 'patients.write'], 'all'),
      true,
    );
    assert.equal(
      evaluatePermissionRequirement(grants, ['patients.read', 'appointments.manage'], 'all'),
      false,
    );
  });
  ...
  it('evaluates ANY mode requirements', () => {
    ...
    assert.equal(
      evaluatePermissionRequirement(grants, ['patients.read', 'appointments.manage'], 'any'),
      true,
    );
    assert.equal(
      evaluatePermissionRequirement(grants, ['appointments.manage', 'users.write'], 'any'),
      false,
    );
  });
  ...
  it('resolves physician role permissions from the synced frontend matrix', () => {
    ...
    assert.equal(hasPermission(effective.effective, 'patients.read', effective.denies), true);
    assert.equal(hasPermission(effective.effective, 'medications.prescribe', effective.denies), true);
    assert.equal(hasPermission(effective.effective, 'iam.admin', effective.denies), false);
  });
  ...
  it('grants platform admin access to tenant-scoped admin permissions', () => {
    ...
    assert.equal(hasPermission(effective.effective, 'platform.admin', effective.denies), true);
    assert.equal(hasPermission(effective.effective, 'iam.admin', effective.denies), true);
  });
});
