...
class AppointmentMockRepository {
  ...
  reschedule(
    ...
    if (idx === -1) throw new NotFoundError('Appointment not found');
    const updated = {
  ...
  cancel(appointmentId: string, _input?: CancelAppointmentInput): Appointment {
    const idx = this.appointments.findIndex((a) => a.id === appointmentId);
    if (idx === -1) throw new NotFoundError('Appointment not found');
    const updated = {
      ...this.appointments[idx]!,
  ...
  getQueue(filters?: AppointmentFilters): QueueEntry[] {
    return buildQueueFromAppointments(this.getToday(filters));
  }
}

export const appointmentMockRepository = new AppointmentMockRepository();
