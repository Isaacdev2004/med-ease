...
const PROVIDERS: AppointmentProvider[] = [
  ...
  {
    id: 'prov-008',
    fullName: 'Dr. Nicolas Girard',
    specialty: 'Radiology',
    department: 'Imaging',
  },
  {
    id: 'prov-009',
    fullName: 'Pharm. Claire Lambert',
    specialty: 'Pharmacy Consultation',
    department: 'Pharmacy',
  },
  {
    id: 'prov-010',
    fullName: 'Dr. Camille Moreau',
...
const FACILITIES = [
  {
    ...
    address: "47 Blvd de l'Hôpital, Paris",
  },
  {
    id: 'fac-002',
    name: 'Hôpital Édouard Herriot',
    address: "5 Place d'Arsonval, Lyon",
  },
  {
    id: 'fac-003',
    name: 'Clinique Pasteur',
    address: '45 Avenue de Lombez, Toulouse',
  },
  {
    id: 'fac-004',
    name: 'Centre Médical La Défense',
    address: '15 Parvis de la Défense, Paris',
  },
];
...
export function generateAppointment(index: number): Appointment {
  ...
  const visitType =
    specialty === 'Telemedicine'
      ? 'telemedicine'
      : mapSpecialtyToVisitType(specialty);
  const checkInStatus =
  ...
  return {
    ...
    visitType,
    priority:
      index % 15 === 0 ? 'urgent' : index % 30 === 0 ? 'emergency' : 'routine',
    insurance: pick(
      ['AXA Santé', 'Mutuelle Générale', 'Harmonie Mutuelle', 'CNAM'],
      rand,
    ),
    reason: pick(REASONS, rand),
    clinicalNotes:
      index % 4 === 0
        ? 'Patient reports mild symptoms. Vitals stable.'
        : undefined,
    followUpRequired: index % 6 === 0,
    referralId: index % 10 === 0 ? `ref-${index}` : undefined,
    checkInStatus,
    queuePosition:
      checkInStatus !== 'not_checked_in' ? (index % 8) + 1 : undefined,
    telehealthLink:
      visitType === 'telemedicine'
        ? `https://telehealth.medease.health/session/apt-${index + 1}`
        : undefined,
...
export const MOCK_APPOINTMENTS: Appointment[] = Array.from(
  { length: 500 },
  (_, i) => generateAppointment(i),
);

export const MOCK_WAITLIST: WaitlistEntry[] = Array.from(
  { length: 24 },
  (_, i) => {
...
export function buildQueueFromAppointments(
  ...
  return appointments
    .filter((a) =>
      ['checked_in', 'waiting', 'in_progress', 'delayed'].includes(a.status),
    )
    ...
    .map((a, idx) => ({
      ...
      checkedInAt:
        a.checkInStatus !== 'not_checked_in' ? a.scheduledAt : undefined,
    }));
}
