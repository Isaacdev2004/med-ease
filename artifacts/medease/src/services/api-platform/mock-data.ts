import {
  activeApiKeyCount,
  computeApiAnalytics,
} from '@/services/api-platform/analytics-engine';
...
export const MOCK_API_KEYS: ApiKey[] = Array.from(
  { length: SCALE.keys },
  (_, i) => ({
    ...
    partnerId:
      i % 3 === 0 ? `ptr-${String((i % 10) + 1).padStart(3, '0')}` : undefined,
...
export const MOCK_OAUTH_APPS: OAuthApp[] = Array.from(
  { length: SCALE.oauth },
  (_, i) => ({
    ...
    partnerId:
      i % 2 === 0 ? `ptr-${String((i % 10) + 1).padStart(3, '0')}` : undefined,
    createdAt: daysAgo(i % 120),
    updatedAt: daysAgo(i % 30),
  }),
);

export const MOCK_WEBHOOKS: Webhook[] = Array.from(
  { length: SCALE.webhooks },
  (_, i) => ({
...
export const MOCK_WEBHOOK_DELIVERIES: WebhookDelivery[] = Array.from(
  ...
  (_, i) => ({
    ...
    status: (
      ['delivered', 'delivered', 'failed', 'retrying', 'pending'] as const
    )[i % 5]!,
    attemptCount: 1 + (i % 3),
...
export const MOCK_API_ENDPOINTS: ApiEndpoint[] = Array.from(
  { length: SCALE.endpoints },
  (_, i) => ({
    ...
    rateLimitPolicyId:
      i % 4 === 0 ? `rl-${String((i % 10) + 1).padStart(3, '0')}` : undefined,
  }),
);
...
export const MOCK_API_VERSIONS: ApiVersion[] = [
  ...
  },
  {
    ...
    version: 'v1',
    status: 'deprecated',
    releaseDate: daysAgo(540),
    deprecationDate: daysAgo(30),
    endpointCount: 85,
  },
];
...
export const MOCK_RATE_LIMIT_POLICIES: RateLimitPolicy[] = Array.from(
  { length: SCALE.policies },
  (_, i) => ({
...
export const MOCK_API_PARTNERS: ApiPartner[] = Array.from(
  { length: SCALE.partners },
  (_, i) => ({
...
export const MOCK_SANDBOX_ENVIRONMENTS: SandboxEnvironment[] = Array.from(
  { length: SCALE.sandboxes },
  (_, i) => ({
    sandboxId: `sbx-${String(i + 1).padStart(3, '0')}`,
    name: `Sandbox ${i + 1}`,
    baseUrl: `https://sandbox-${i + 1}.api.medease.io`,
    status: (['active', 'active', 'provisioning', 'suspended'] as const)[
      i % 4
    ]!,
    partnerId:
      i % 2 === 0 ? `ptr-${String((i % 10) + 1).padStart(3, '0')}` : undefined,
...
export function buildApiDashboard(partnerId?: string): ApiDashboard {
  ...
  if (partnerId) {
    ...
    deliveries = deliveries.filter(
      (d) =>
        MOCK_WEBHOOKS.find((w) => w.webhookId === d.webhookId)?.partnerId ===
        partnerId,
    );
  }
  ...
  return {
    ...
    sandboxEnvironments:
      MOCK_SANDBOX_ENVIRONMENTS.filter((s) => s.status === 'active').length * 8,
    partners:
      MOCK_API_PARTNERS.filter((p) => p.status === 'active').length * 12,
...
export function computeApiPlatformAnalytics(partnerId?: string) {
  ...
  return computeApiAnalytics(
    keys,
    MOCK_API_ENDPOINTS,
    deliveries,
    totalSdkDownloads(MOCK_SDK_PACKAGES),
  );
}
