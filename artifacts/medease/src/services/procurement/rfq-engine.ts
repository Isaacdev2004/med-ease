import type { CreateRFQInput, RFQ, RFQResponse } from '@/services/procurement/types';

export function buildRFQ(input: CreateRFQInput, rfqId: string, rfqNumber: string): RFQ {
  return {
    rfqId,
    rfqNumber,
    title: input.title,
    ...
    lineItems: input.lineItems.map((l, i) => ({ ...l, lineId: `${rfqId}-line-${i}` })),
...
export function compareQuotations(responses: RFQResponse[]) {
  ...
  const savings = highest && lowest ? highest.totalQuote - lowest.totalQuote : 0;
...
export function awardRFQ(rfq: RFQ, responseId: string): RFQ {
  ...
  return {
    ...
    responses: rfq.responses.map((r) => ({
      ...r,
      status: r.responseId === responseId ? 'awarded' : r.status === 'awarded' ? r.status : 'declined',
    })),
    updatedAt: new Date().toISOString(),
  };
}
...
export function declineResponse(rfq: RFQ, responseId: string): RFQ {
  return {
    ...rfq,
    responses: rfq.responses.map((r) => (r.responseId === responseId ? { ...r, status: 'declined' } : r)),
    updatedAt: new Date().toISOString(),
  };
}
