...
const DEPARTMENTS = ['pharmacy', 'laboratory', 'radiology', 'facility', 'biomedical', 'general'] as const;
const VENDOR_CATS = ['medical', 'laboratory', 'radiology', 'equipment', 'pharmaceutical', 'international', 'local', 'consumables'] as const;
const REQ_STATUSES = ['draft', 'submitted', 'pending_approval', 'approved', 'rejected', 'cancelled', 'converted'] as const;
const PO_STATUSES = ['draft', 'pending_approval', 'approved', 'ordered', 'partially_received', 'received', 'closed', 'cancelled'] as const;
const RFQ_STATUSES = ['draft', 'open', 'closed', 'awarded', 'cancelled'] as const;
const INV_STATUSES = ['draft', 'pending_match', 'matched', 'approved', 'paid', 'disputed', 'cancelled'] as const;
const CONTRACT_STATUSES = ['draft', 'active', 'expiring', 'expired', 'terminated', 'renewed'] as const;
const DELIVERY_STATUSES = ['scheduled', 'in_transit', 'delivered', 'delayed', 'cancelled'] as const;
const COUNTRIES = ['France', 'Germany', 'USA', 'UK', 'Nigeria', 'India', 'China', 'Netherlands'];
...
export const MOCK_SPEND_CATEGORIES: SpendCategory[] = Array.from({ length: 50 }, (_, i) => ({
  categoryId: `spc-${String(i + 1).padStart(3, '0')}`,
  name: ['Medications', 'Reagents', 'Imaging Supplies', 'Equipment', 'Consumables', 'Maintenance'][i % 6]!,
...
export const MOCK_COST_CENTERS: CostCenter[] = Array.from({ length: 500 }, (_, i) => ({
...
export const MOCK_SUPPLIERS: Supplier[] = Array.from({ length: 3000 }, (_, i) => {
  const isIntl = i % 5 === 0;
  ...
  return {
    ...
    certifications: i % 3 === 0 ? ['ISO 13485', 'GDP'] : i % 5 === 0 ? ['ISO 9001'] : [],
...
function buildRequestLine(reqIdx: number, lineIdx: number) {
  ...
  return {
    ...
    unitPrice,
    total: qty * unitPrice,
    spendCategoryId: MOCK_SPEND_CATEGORIES[(reqIdx + lineIdx) % MOCK_SPEND_CATEGORIES.length]!.categoryId,
  };
}

export const MOCK_PURCHASE_REQUESTS: PurchaseRequest[] = Array.from({ length: 15000 }, (_, i) => {
  const lines = Array.from({ length: 1 + (i % 4) }, (_, j) => buildRequestLine(i, j));
...
export const MOCK_RFQS: RFQ[] = Array.from({ length: 8000 }, (_, i) => {
  ...
  const responses: RFQResponse[] = Array.from({ length: supplierCount }, (_, r) => {
    const sup = MOCK_SUPPLIERS[(i + r) % MOCK_SUPPLIERS.length]!;
    const quotes = lines.map((l) => ({ lineId: l.lineId, unitPrice: 10 + ((i + r) % 50), leadTimeDays: 3 + ((i + r) % 14) }));
    const total = quotes.reduce((s, q, idx) => s + q.unitPrice * lines[idx]!.quantity, 0);
    return {
      responseId: `rfqr-${i}-${r}`,
      ...
      rank: r + 1,
      status: (['submitted', 'shortlisted', 'awarded', 'declined'] as const)[r % 4]!,
  ...
  return {
    ...
    requisitionId: i % 3 === 0 ? MOCK_PURCHASE_REQUESTS[i % MOCK_PURCHASE_REQUESTS.length]!.requestId : undefined,
...
function buildPOItems(poIdx: number): PurchaseOrderItem[] {
  ...
  return Array.from({ length: count }, (_, j) => {
    ...
    return {
      ...
      spendCategoryId: MOCK_SPEND_CATEGORIES[(poIdx + j) % MOCK_SPEND_CATEGORIES.length]!.categoryId,
    };
  });
}

export const MOCK_PURCHASE_ORDERS: PurchaseOrder[] = Array.from({ length: 10000 }, (_, i) => {
  ...
  return {
    ...
    supplierName: sup.name,
    department: dept,
    requisitionId: i % 4 === 0 ? MOCK_PURCHASE_REQUESTS[i % MOCK_PURCHASE_REQUESTS.length]!.requestId : undefined,
    rfqId: i % 6 === 0 ? MOCK_RFQS[i % MOCK_RFQS.length]!.rfqId : undefined,
    contractId: i % 8 === 0 ? `con-${String((i % 5000) + 1).padStart(5, '0')}` : undefined,
    costCenterId: MOCK_COST_CENTERS[i % MOCK_COST_CENTERS.length]!.costCenterId,
...
export const MOCK_INVOICES: ProcurementInvoice[] = Array.from({ length: 12000 }, (_, i) => {
...
export const MOCK_CONTRACTS: Contract[] = Array.from({ length: 5000 }, (_, i) => {
  const sup = MOCK_SUPPLIERS[i % MOCK_SUPPLIERS.length]!;
  ...
  return {
    ...
    supplierName: sup.name,
    department: dept,
    status: endDays <= 30 ? 'expiring' : CONTRACT_STATUSES[i % CONTRACT_STATUSES.length]!,
...
export const MOCK_GOODS_RECEIPTS: GoodsReceipt[] = Array.from({ length: 3000 }, (_, i) => {
  const po = MOCK_PURCHASE_ORDERS[i % MOCK_PURCHASE_ORDERS.length]!;
  return {
...
export const MOCK_SHIPMENTS: Shipment[] = Array.from({ length: 2500 }, (_, i) => {
  const po = MOCK_PURCHASE_ORDERS[i % MOCK_PURCHASE_ORDERS.length]!;
...
export const MOCK_DELIVERIES: Delivery[] = Array.from({ length: 2500 }, (_, i) => {
  const po = MOCK_PURCHASE_ORDERS[i % MOCK_PURCHASE_ORDERS.length]!;
  const status = DELIVERY_STATUSES[i % DELIVERY_STATUSES.length]!;
  return {
...
export const MOCK_APPROVALS: ApprovalWorkflow[] = Array.from({ length: 2000 }, (_, i) => {
  const entityType = (['requisition', 'purchase_order', 'invoice', 'contract'] as const)[i % 4]!;
  const steps = [
    { stepId: 's1', role: 'Department Head', approverName: 'Dept Head', status: 'approved' as ApprovalWorkflow['steps'][0]['status'], signedAt: daysAgo(3) },
    { stepId: 's2', role: 'Finance', approverName: i % 3 === 0 ? undefined : 'Finance Officer', status: (i % 3 === 0 ? 'pending' : 'approved') as ApprovalWorkflow['steps'][0]['status'], threshold: 5000 },
    { stepId: 's3', role: 'CEO', status: (i % 7 === 0 ? 'pending' : 'approved') as ApprovalWorkflow['steps'][0]['status'], threshold: 50000 },
  ];
  return {
    workflowId: `wf-${String(i + 1).padStart(5, '0')}`,
    entityType,
    entityId: entityType === 'requisition' ? MOCK_PURCHASE_REQUESTS[i % 500]!.requestId : MOCK_PURCHASE_ORDERS[i % 500]!.purchaseOrderId,
    steps,
    currentStep: i % 3,
...
export function buildProcurementDashboard(department?: string): ProcurementDashboard {
  ...
  const openPOs = orders.filter((o) => ['approved', 'ordered', 'partially_received'].includes(o.status)).length;
  ...
  const overdueDeliveries = MOCK_DELIVERIES.filter((d) => d.status === 'delayed').length;
  const invoiceMismatches = MOCK_INVOICES.filter((i) => i.variance > 0 || i.status === 'disputed').length;
  const budgetUtil = Math.round(MOCK_BUDGETS.reduce((s, b) => s + (b.spent / b.allocated) * 100, 0) / MOCK_BUDGETS.length);
  ...
  return {
    ...
    pendingApprovalItems: MOCK_APPROVALS.filter((a) => a.status === 'pending').slice(0, 6),
    expiringContracts: MOCK_CONTRACTS.filter((c) => c.status === 'expiring').slice(0, 6),
  };
}
