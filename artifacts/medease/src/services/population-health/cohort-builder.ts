...
export function evaluateCohort(
  ...
  if (
    ...
    !criteria.conditions.includes(member.primaryCondition)
  )
    return false;
  if (criteria.minOpenGaps != null && member.openGaps < criteria.minOpenGaps)
    return false;
  if (
    criteria.riskTiers?.length &&
    !criteria.riskTiers.includes(member.riskTier)
  )
    return false;
  if (criteria.maxDaysSinceVisit != null && member.lastVisit) {
    const days =
      (Date.now() - new Date(member.lastVisit).getTime()) /
      (1000 * 60 * 60 * 24);
...
export function buildCohortFromCriteria(
...
export function parseCriteriaString(criteria: string): CohortCriteria {
  ...
  if (
    criteria.includes('Missed medication') ||
    criteria.includes('No appointment')
  )
    result.maxDaysSinceVisit = 180;
