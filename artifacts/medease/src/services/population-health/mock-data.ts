...
export const MOCK_REGISTRIES: DiseaseRegistry[] = Array.from(
  { length: 100 },
  (_, i) => {
...
export const MOCK_POPULATION: PopulationMember[] = Array.from(
  { length: 8000 },
  (_, i) => {
    ...
    return {
      ...
      gender: i % 2 === 0 ? 'F' : 'M',
      facilityId: FACILITIES[i % 25]!,
      primaryCondition: ['Diabetes', 'Hypertension', 'CHF', 'COPD', 'Asthma'][
        i % 5
      ],
      riskTier: tier,
      openGaps: i % 5,
...
export const MOCK_CARE_GAPS: CareGap[] = Array.from(
  { length: 12000 },
  (_, i) => {
    ...
    return {
      ...
      status:
        overdue > 0
          ? ('overdue' as const)
          : (['open', 'in_progress', 'closed'] as const)[i % 3]!,
...
export const MOCK_COHORTS: PatientCohort[] = Array.from(
  { length: 200 },
  (_, i) => ({
    ...
    criteria:
      'Age > 60 AND Diabetes AND HbA1c > 8 AND No appointment in 6 months',
    createdAt: daysAgo(i % 180),
    dynamic: i % 3 !== 0,
  }),
);

export const MOCK_CHRONIC_PROGRAMS: ChronicProgram[] = Array.from(
  { length: 60 },
  (_, i) => {
    const types = [
      'diabetes',
...
export const MOCK_PREVENTIVE: PreventiveCareItem[] = Array.from(
  { length: 80 },
  (_, i) => {
...
export const MOCK_CAMPAIGNS: OutreachCampaign[] = Array.from(
  { length: 300 },
  (_, i) => {
...
export const MOCK_COMMUNITY: CommunityProgram[] = Array.from(
  { length: 100 },
  (_, i) => ({
    ...
    attendees: 50 + (i % 200),
    sdohFocus: ['Food insecurity', 'Transportation', 'Housing', 'Education'][
      i % 4
    ],
...
export function buildPhmDashboard(facilityId?: string): PhmDashboard {
  ...
  const campaigns = facilityId
    ? MOCK_CAMPAIGNS.filter((c) => c.facilityId === facilityId)
    : MOCK_CAMPAIGNS;
  ...
  return {
    facilityId,
    totalPopulation:
      Math.round(population.length * scale) ||
      Math.round(MOCK_POPULATION.length / 25),
    openCareGaps: gaps.filter((g) => g.status !== 'closed').length * scale,
    highRiskCount:
      population.filter((p) => p.riskTier === 'high').length * scale,
    risingRiskCount:
      population.filter((p) => p.riskTier === 'rising').length * scale,
    registryEnrollment:
      registries.reduce((s, r) => s + r.memberCount, 0) * scale,
    preventiveCompliance: preventive.length
      ? Math.round(
          preventive.reduce((s, p) => s + p.complianceRate, 0) /
            preventive.length,
        )
      : 78,
    ...
    riskDistribution: RISK_TIERS.map((tier) => ({
      tier,
      count: Math.round(
        population.filter((p) => p.riskTier === tier).length * scale,
      ),
    })),
