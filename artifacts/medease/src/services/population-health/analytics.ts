...
export function computePopulationAnalytics(facilityId?: string): PopulationAnalytics {
  ...
  const scores = facilityId ? MOCK_RISK_SCORES.filter((s) => s.facilityId === facilityId) : MOCK_RISK_SCORES;
  ...
  return {
    ...
    diseasePrevalence: registries.length ? Math.round(registries.reduce((s, r) => s + r.memberCount, 0) / Math.max(population.length, 1) * 100) / 10 : 24.5,
    ...
    gapTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({ label, value: Math.round(gaps.length / 2000 + i * 120) })),
    prevalenceByCondition: ['Diabetes', 'Hypertension', 'CHF', 'COPD', 'Asthma'].map((label, i) => ({
      label,
      value: MOCK_CHRONIC_PROGRAMS.filter((p) => p.type.includes(label.toLowerCase().split(' ')[0]!) || i === 0).reduce((s, p) => s + p.enrolledCount, 0) / 10 || 500 + i * 200,
    })),
    riskDistribution: aggregateRiskDistribution(scores.slice(0, 500)),
    geographicTrend: regions.slice(0, 8).map((r) => ({ label: r.zipCode, value: r.prevalenceRate })),
    providerPerformance: ['Dr. Chen', 'Dr. Martin', 'Dr. Dupont', 'Dr. Bernard'].map((label, i) => ({ label, value: 75 + i * 5 })),
  };
}
