...
const HL7_TYPES: Hl7MessageType[] = [
  'ADT',
  'ORM',
  'ORU',
  'SIU',
...
const MAPPING_DIRECTIONS: MappingDirection[] = [
  'internal_to_fhir',
  'internal_to_hl7',
  'internal_to_cda',
  'internal_to_dicom',
...
export const MOCK_ENDPOINTS: IntegrationEndpoint[] = Array.from(
  { length: 200 },
  (_, i) => {
    ...
    return {
      ...
      status: (
        ['active', 'active', 'inactive', 'error', 'maintenance'] as const
      )[i % 5]!,
...
export const MOCK_FHIR_SERVERS: FhirServer[] = Array.from(
  { length: 80 },
  (_, i) => ({
...
export const MOCK_HL7_MESSAGES: Hl7Message[] = Array.from(
  { length: 3000 },
  (_, i) => {
    ...
    return {
      ...
      status: (
        ['received', 'processed', 'failed', 'acknowledged', 'queued'] as const
      )[i % 5]!,
...
export const MOCK_FHIR_BUNDLES: FhirBundle[] = Array.from(
  { length: 800 },
  (_, i) => ({
    bundleId: `bundle-${String(i + 1).padStart(4, '0')}`,
    type: (['transaction', 'batch', 'searchset', 'collection'] as const)[
      i % 4
    ]!,
    entryCount: 1 + (i % 50),
    facilityId: FACILITIES[i % 25],
...
export const MOCK_DICOM_STUDIES: DicomStudy[] = Array.from(
  { length: 600 },
  (_, i) => ({
    ...
    status: (['idle', 'syncing', 'success', 'conflict', 'error'] as const)[
      i % 5
    ]!,
...
export const MOCK_CDA_DOCUMENTS: CdaDocument[] = Array.from(
  { length: 400 },
  (_, i) => ({
...
export const MOCK_JOBS: IntegrationJob[] = Array.from(
  { length: 800 },
  (_, i) => ({
...
export const MOCK_OAUTH_CLIENTS: OAuthClient[] = Array.from(
  ...
  (_, i) => ({
    ...
    name: `API Client ${i + 1}`,
    grantTypes: ['client_credentials', 'authorization_code'].slice(
      0,
      1 + (i % 2),
    ),
...
export const MOCK_SMART_APPS: SmartApp[] = Array.from(
  { length: 50 },
  (_, i) => ({
...
export const MOCK_SUBSCRIPTIONS: Subscription[] = Array.from(
  { length: 100 },
  (_, i) => ({
...
export const MOCK_MAPPINGS: MappingProfile[] = Array.from(
  { length: 200 },
  (_, i) => ({
    ...
    targetFormat: MAPPING_DIRECTIONS[i % MAPPING_DIRECTIONS.length]!.split('_')
      .pop()!
      .toUpperCase(),
    fieldCount: 10 + (i % 40),
    validated: i % 4 !== 0,
...
export const MOCK_VALIDATIONS: ValidationResult[] = Array.from(
  { length: 100 },
  (_, i) => ({
...
export const MOCK_SYNC_STATUS: SyncStatus[] = MOCK_ENDPOINTS.slice(0, 50).map(
  (ep, i) => ({
    syncId: `sync-${String(i + 1).padStart(3, '0')}`,
    endpointId: ep.endpointId,
    state: (['idle', 'syncing', 'success', 'conflict', 'error'] as const)[
      i % 5
    ]!,
...
export const MOCK_QUEUES: IntegrationQueue[] = [
  ...
  {
    queueId: 'q-fhir-transactions',
    name: 'FHIR Transactions',
    pendingCount: 89,
    failedCount: 3,
    throughputPerHour: 800,
  },
  {
    queueId: 'q-dicom-stow',
    name: 'DICOM STOW-RS',
    pendingCount: 34,
    failedCount: 1,
...
export const MOCK_TERMINOLOGY: TerminologyServer[] = [
  {
    serverId: 'term-001',
    name: 'SNOMED CT',
    url: 'https://terminology.medease.local/snomed',
  ...
  },
  {
    serverId: 'term-002',
    name: 'LOINC',
    url: 'https://terminology.medease.local/loinc',
  ...
  },
  {
    serverId: 'term-003',
    name: 'RxNorm',
    url: 'https://terminology.medease.local/rxnorm',
  ...
  },
  {
    serverId: 'term-004',
    name: 'ICD-10',
    url: 'https://terminology.medease.local/icd10',
    systems: ['http://hl7.org/fhir/sid/icd-10'],
    status: 'active',
  },
];
...
export const MOCK_VALUE_SETS: ValueSet[] = Array.from(
  { length: 30 },
  (_, i) => ({
...
export const MOCK_CONCEPT_MAPS: ConceptMap[] = Array.from(
  { length: 20 },
  (_, i) => ({
...
export const MOCK_HIE: HealthInformationExchange[] = Array.from(
  { length: 10 },
  (_, i) => ({
...
export const MOCK_FHIR_TRANSACTIONS: FhirTransaction[] = Array.from(
  { length: 200 },
  (_, i) => ({
...
export function buildInteropDashboard(facilityId?: string): InteropDashboard {
  const endpoints = facilityId
    ? MOCK_ENDPOINTS.filter((e) => e.facilityId === facilityId)
    : MOCK_ENDPOINTS;
  const messages = facilityId
    ? MOCK_HL7_MESSAGES.filter((m) => m.facilityId === facilityId)
    : MOCK_HL7_MESSAGES;
  const jobs = facilityId
    ? MOCK_JOBS.filter((j) => j.facilityId === facilityId)
    : MOCK_JOBS;
  const studies = facilityId
    ? MOCK_DICOM_STUDIES.filter((s) => s.facilityId === facilityId)
    : MOCK_DICOM_STUDIES;
  return {
    facilityId,
