...
class InteroperabilityRepository {
  ...
  getEndpoints(filters?: InteropFilters) {
    ...
    if (filters?.protocol)
      items = items.filter((e) => e.protocol === filters.protocol);
    if (filters?.status)
      items = items.filter((e) => e.status === filters.status);
    if (filters?.q)
      items = items.filter((e) => matchQ(filters.q, e.name, e.baseUrl));
  ...
  getFhirServers(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((s) => matchQ(filters.q, s.name, s.baseUrl));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getHl7Messages(filters?: InteropFilters) {
    ...
    if (filters?.messageType)
      items = items.filter((m) => m.type === filters.messageType);
    if (filters?.status)
      items = items.filter((m) => m.status === filters.status);
    if (filters?.q)
      items = items.filter((m) => matchQ(filters.q, m.controlId, m.patientId));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getDicomStudies(filters?: InteropFilters) {
    ...
    if (filters?.facilityId)
      items = items.filter((s) => s.facilityId === filters.facilityId);
    if (filters?.q)
      items = items.filter((s) => matchQ(filters.q, s.patientName, s.modality));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getCdaDocuments(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((d) => matchQ(filters.q, d.type, d.patientId));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getMappings(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((m) => matchQ(filters.q, m.name, m.sourceEntity));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getSubscriptions(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((s) => matchQ(filters.q, s.criteria, s.endpoint));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getJobs(filters?: InteropFilters) {
    ...
    if (filters?.jobStatus)
      items = items.filter((j) => j.status === filters.jobStatus);
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getWebhooks(filters?: InteropFilters) {
    ...
    if (filters?.facilityId)
      items = items.filter(
        (w) => !w.facilityId || w.facilityId === filters.facilityId,
      );
    if (filters?.q)
  ...
  getSmartApps(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((a) => matchQ(filters.q, a.name, a.launchUrl));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getValidations(filters?: InteropFilters) {
    ...
    if (filters?.q)
      items = items.filter((v) =>
        matchQ(filters.q, v.resourceType, v.resourceId),
      );
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  updateEndpoint(input: UpdateEndpointInput) {
    const idx = this.endpoints.findIndex(
      (e) => e.endpointId === input.endpointId,
    );
  ...
  search(query: string, facilityId?: string) {
    ...
    return {
      endpoints: this.endpoints
        .filter(
          (e) =>
            (!facilityId || e.facilityId === facilityId) && matches(e.name),
        )
        .slice(0, 12),
      messages: MOCK_HL7_MESSAGES.filter(
        (m) =>
          (!facilityId || m.facilityId === facilityId) &&
          matches(m.controlId, m.type),
      ).slice(0, 12),
  ...
  exportData(format: 'csv' | 'pdf' | 'xlsx') {
    return {
      ...
      recordCount: this.endpoints.length + MOCK_HL7_MESSAGES.length,
    };
  }

  favorite(
    userId: string,
    entityType: InteropFavorite['entityType'],
    entityId: string,
  ) {
...
export const interoperabilityMockRepository = new InteroperabilityRepository();
