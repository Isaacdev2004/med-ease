...
const HOSPITALS = Array.from({ length: 25 }, (_, i) => ({
  ...
  name: ['Pitié-Salpêtrière', 'Hôpital Européen', 'Centre Hospitalier Lyon', 'CHU Bordeaux', 'Hôpital Saint-Louis'][i % 5] ?? `Hospital ${i + 1}`,
}));
...
const EQUIP_CATEGORIES = ['Imaging', 'Ventilator', 'Monitor', 'Infusion Pump', 'Defibrillator', 'Analyzer', 'Sterilizer', 'Ultrasound'];
const MANUFACTURERS = ['GE Healthcare', 'Siemens Healthineers', 'Philips', 'Medtronic', 'Dräger', 'BD', 'Stryker'];
const UTILITY_TYPES = ['electrical', 'hvac', 'water', 'medical_gas', 'generator', 'ups', 'fire_safety', 'security'] as const;
...
export const MOCK_BUILDINGS: Building[] = Array.from({ length: 350 }, (_, i) => {
...
export const MOCK_ROOMS: Room[] = Array.from({ length: 8000 }, (_, i) => {
  ...
  const types = ['patient', 'operating', 'icu', 'isolation', 'consultation', 'utility', 'storage', 'office'] as const;
  return {
    ...
    capacity: 1 + (i % 4),
    status: (['available', 'occupied', 'maintenance', 'cleaning'] as const)[i % 4]!,
    departmentId: `dept-${String((i % 300) + 1).padStart(4, '0')}`,
  };
});

export const MOCK_BEDS: Bed[] = Array.from({ length: 12000 }, (_, i) => {
  const room = MOCK_ROOMS.filter((r) => r.type === 'patient' || r.type === 'icu')[i % 4000] ?? MOCK_ROOMS[i % MOCK_ROOMS.length]!;
  const wards = ['ICU', 'General', 'Maternity', 'Pediatric', 'Surgical', 'Emergency'];
...
export const MOCK_EQUIPMENT: MedicalEquipment[] = Array.from({ length: 10000 }, (_, i) => {
  const fac = HOSPITALS[i % 25]!;
  const bld = MOCK_BUILDINGS[i % MOCK_BUILDINGS.length]!;
  const room = MOCK_ROOMS[i % MOCK_ROOMS.length]!;
  const statuses = ['operational', 'maintenance', 'calibration_due', 'out_of_service', 'decommissioned'] as const;
...
export const MOCK_BIOMEDICAL_DEVICES: BiomedicalDevice[] = MOCK_EQUIPMENT.slice(0, 6000).map((eq, i) => ({
  ...eq,
...
export const MOCK_WORK_ORDERS: WorkOrder[] = Array.from({ length: 15000 }, (_, i) => {
  const fac = HOSPITALS[i % 25]!;
  const eq = MOCK_EQUIPMENT[i % MOCK_EQUIPMENT.length]!;
  const statuses = ['draft', 'open', 'assigned', 'in_progress', 'on_hold', 'completed', 'cancelled'] as const;
  ...
  return {
    ...
    requestId: i % 3 === 0 ? `mr-${String(i + 1).padStart(5, '0')}` : undefined,
    ...
    status,
    assignedTechnicianId: status !== 'open' && status !== 'draft' ? `emp-${String((i % 500) + 1).padStart(5, '0')}` : undefined,
    assignedTechnicianName: status !== 'open' && status !== 'draft' ? `Technician ${(i % 50) + 1}` : undefined,
...
export const MOCK_PREVENTIVE: PreventiveMaintenance[] = Array.from({ length: 6000 }, (_, i) => {
  const eq = MOCK_EQUIPMENT[i % MOCK_EQUIPMENT.length]!;
  ...
  return {
    ...
    status: dueDays < 0 ? 'overdue' as const : dueDays <= 7 ? 'due' as const : 'compliant' as const,
    assignedTeamId: `team-${String((i % 700) + 1).padStart(4, '0')}`,
  };
});

export const MOCK_CALIBRATION: CalibrationRecord[] = Array.from({ length: 4000 }, (_, i) => {
  ...
  return {
    ...
    nextDue: daysFromNow(dueDays),
    status: dueDays < 0 ? 'overdue' as const : dueDays <= 14 ? 'due' as const : statuses[i % statuses.length]!,
    technicianId: `emp-${String((i % 200) + 1).padStart(5, '0')}`,
    certificateNumber: `CAL-CERT-${String(10000 + i)}`,
  };
});

export const MOCK_INSPECTIONS: Inspection[] = Array.from({ length: 2000 }, (_, i) => {
  const fac = HOSPITALS[i % 25]!;
  const bld = MOCK_BUILDINGS[i % MOCK_BUILDINGS.length]!;
  const types = ['safety', 'fire', 'electrical', 'hygiene', 'regulatory'] as const;
...
export const MOCK_VENDORS: Vendor[] = Array.from({ length: 800 }, (_, i) => ({
  ...
  category: ['Biomedical', 'HVAC', 'Electrical', 'General Maintenance', 'Calibration'][i % 5]!,
  rating: 3 + (i % 20) / 10,
  contractCount: 1 + (i % 10),
  status: i % 50 === 0 ? 'inactive' as const : 'active' as const,
}));

export const MOCK_CONTRACTS: ServiceContract[] = Array.from({ length: 500 }, (_, i) => {
...
export const MOCK_UTILITIES: UtilitySystem[] = Array.from({ length: 200 }, (_, i) => {
  const fac = HOSPITALS[i % 25]!;
...
export const MOCK_IOT_SENSORS: IoTSensor[] = Array.from({ length: 1500 }, (_, i) => {
  const room = MOCK_ROOMS[i % MOCK_ROOMS.length]!;
  return {
    sensorId: `sns-${String(i + 1).padStart(5, '0')}`,
    name: `Sensor ${i + 1}`,
    type: ['temperature', 'humidity', 'pressure', 'motion', 'air_quality'][i % 5]!,
    facilityId: room.facilityId,
...
export const MOCK_ENVIRONMENTAL: EnvironmentalReading[] = Array.from({ length: 5000 }, (_, i) => {
  const sns = MOCK_IOT_SENSORS[i % MOCK_IOT_SENSORS.length]!;
  const metrics = ['temperature', 'humidity', 'pressure', 'co2', 'air_quality'] as const;
  const metric = metrics[i % metrics.length]!;
  const value = metric === 'temperature' ? 18 + (i % 10) : metric === 'humidity' ? 40 + (i % 30) : 50 + (i % 50);
  return {
    ...
    metric,
    value,
    unit: metric === 'temperature' ? '°C' : metric === 'humidity' ? '%' : metric === 'pressure' ? 'Pa' : 'ppm',
    timestamp: daysAgo(i % 48),
    status: value > 90 ? 'critical' as const : value > 75 ? 'warning' as const : 'normal' as const,
  };
});

export const MOCK_INCIDENTS: Incident[] = Array.from({ length: 3000 }, (_, i) => {
  const fac = HOSPITALS[i % 25]!;
...
export function buildFacilitiesDashboard(facilityId?: string): FacilitiesDashboard {
  const buildings = facilityId ? MOCK_BUILDINGS.filter((b) => b.facilityId === facilityId) : MOCK_BUILDINGS;
  const rooms = facilityId ? MOCK_ROOMS.filter((r) => r.facilityId === facilityId) : MOCK_ROOMS;
  const beds = facilityId ? MOCK_BEDS.filter((b) => b.facilityId === facilityId) : MOCK_BEDS;
  ...
  const utilities = facilityId ? MOCK_UTILITIES.filter((u) => u.facilityId === facilityId) : MOCK_UTILITIES;
  ...
  return {
    ...
    calibrationDue: cal.filter((c) => c.status === 'due' || c.status === 'overdue').length,
    utilityAlerts: utilities.filter((u) => u.status !== 'normal').length,
    recentWorkOrders: workOrders.filter((w) => w.status !== 'completed').slice(0, 8),
