...
export function prioritizeWorkOrder(wo: WorkOrder): number {
  ...
  return (
    (priorityScore[wo.priority] ?? 0) +
    (statusScore[wo.status] ?? 0) +
    (wo.slaBreached ? 50 : 0)
  );
}
...
export function checkSlaBreached(
  createdAt: string,
  slaHours: number,
...
export function schedulePreventiveMaintenance(
  ...
  return {
    ...
    status:
      daysUntilDue <= 0 ? 'overdue' : daysUntilDue <= 7 ? 'due' : 'compliant',
  };
}

export function escalateWorkOrder(wo: WorkOrder): WorkOrder {
  const order: WorkOrder['priority'][] = [
    'low',
    'medium',
    'high',
    'critical',
  ...
  return {
    ...wo,
    priority: order[Math.min(idx + 1, order.length - 1)]!,
...
export function generateWeeklyMaintenanceSchedule(
  equipmentIds: string[],
  startDate: Date,
): { equipmentId: string; day: number }[] {
  const offset = startDate.getDay();
  return equipmentIds.flatMap((equipmentId, idx) =>
    Array.from({ length: 7 }, (_, day) => ({
      equipmentId,
      day: (day + offset) % 7,
    })).filter((_, day) => (idx + day) % 3 === 0),
  );
}
