...
import { prioritizeWorkOrder, schedulePreventiveMaintenance } from '@/services/facilities/maintenance-engine';
import {
...
class FacilitiesRepository {
  ...
  listFacilities(filters?: FacilitiesFilters) {
    ...
    if (filters?.q) items = items.filter((f) => matchQ(filters.q, f.name, f.code, f.city));
  ...
  getBuildings(filters?: FacilitiesFilters) {
    ...
    if (filters?.q) items = items.filter((b) => matchQ(filters.q, b.name, b.code));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getRooms(filters?: FacilitiesFilters) {
    ...
    if (filters?.buildingId) items = items.filter((r) => r.buildingId === filters.buildingId);
    if (filters?.q) items = items.filter((r) => matchQ(filters.q, r.name, r.number));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getBeds(filters?: FacilitiesFilters) {
    let items = MOCK_BEDS;
    if (filters?.facilityId) items = items.filter((b) => b.facilityId === filters.facilityId);
    if (filters?.roomId) items = items.filter((b) => b.roomId === filters.roomId);
  ...
  getEquipment(filters?: FacilitiesFilters) {
    ...
    if (filters?.q) items = items.filter((e) => matchQ(filters.q, e.name, e.assetTag, e.serialNumber));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getBiomedicalDevices(filters?: FacilitiesFilters) {
    ...
    if (filters?.q) items = items.filter((d) => matchQ(filters.q, d.name, d.assetTag));
    return paginate(items, filters?.page, filters?.pageSize);
  }

  getWorkOrders(filters?: FacilitiesFilters) {
    let items = this.workOrders;
    if (filters?.facilityId) items = items.filter((w) => w.facilityId === filters.facilityId);
    if (filters?.status) items = items.filter((w) => w.status === filters.status);
    if (filters?.q) items = items.filter((w) => matchQ(filters.q, w.title, w.equipmentName));
    return paginate([...items].sort((a, b) => prioritizeWorkOrder(b) - prioritizeWorkOrder(a)), filters?.page, filters?.pageSize);
  }
  ...
  getUtilities(filters?: FacilitiesFilters) {
    ...
    if (filters?.facilityId) items = items.filter((u) => u.facilityId === filters.facilityId);
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  getEnvironmental(filters?: FacilitiesFilters) {
    ...
    if (filters?.facilityId) items = items.filter((e) => e.facilityId === filters.facilityId);
    return paginate([...items].sort((a, b) => b.timestamp.localeCompare(a.timestamp)), filters?.page, filters?.pageSize);
  }

  getVendors(filters?: FacilitiesFilters) {
    let items = MOCK_VENDORS;
    if (filters?.q) items = items.filter((v) => matchQ(filters.q, v.name, v.category));
    return paginate(items, filters?.page, filters?.pageSize);
  }
  ...
  createMaintenanceRequest(input: CreateMaintenanceRequestInput) {
    const eq = input.equipmentId ? this.equipment.find((e) => e.equipmentId === input.equipmentId) : undefined;
    const fac = MOCK_FACILITY_SITES.find((f) => f.facilityId === input.facilityId);
    const wo = {
      ...
      slaHours: input.priority === 'emergency' ? 4 : input.priority === 'critical' ? 8 : 24,
      slaBreached: false,
  ...
  assignWorkOrder(input: AssignWorkOrderInput) {
    const idx = this.workOrders.findIndex((w) => w.workOrderId === input.workOrderId);
  ...
  completeWorkOrder(workOrderId: string) {
    ...
    wo.downtimeHours = Math.round((Date.now() - new Date(wo.createdAt).getTime()) / (1000 * 60 * 60));
    wo.updatedAt = new Date().toISOString();
    this.workOrders[idx] = wo;
    return wo;
  }
  ...
  schedulePreventive(scheduleId: string, performedDate: string) {
    ...
    if (idx < 0) return null;
    this.preventive[idx] = schedulePreventiveMaintenance(this.preventive[idx]!, performedDate);
  ...
  recordCalibration(input: RecordCalibrationInput) {
    ...
    const record = {
      ...
      nextDue: new Date(new Date(input.performedDate).getTime() + 365 * 24 * 60 * 60 * 1000).toISOString(),
  ...
  recordInspection(inspectionId: string, passed: boolean) {
    const idx = this.inspections.findIndex((i) => i.inspectionId === inspectionId);
    if (idx < 0) return null;
  ...
  updateEquipment(equipmentId: string, status: string) {
    ...
    if (idx < 0) return null;
    this.equipment[idx]!.status = status as typeof this.equipment[0]['status'];
  ...
  search(query: string, facilityId?: string) {
    ...
    const filter = <T extends { facilityId?: string }>(items: T[], ...fields: (string | undefined)[]) =>
      items.filter((item) => (!facilityId || item.facilityId === facilityId) && fields.some((f) => f?.toLowerCase().includes(q))).slice(0, 20);

    return {
      facilities: filter(MOCK_FACILITY_SITES, ...MOCK_FACILITY_SITES.map((f) => f.name)),
      equipment: filter(this.equipment, ...this.equipment.slice(0, 100).flatMap((e) => [e.name, e.assetTag])),
      workOrders: filter(this.workOrders, ...this.workOrders.slice(0, 100).map((w) => w.title)),
    };
  }

  exportData(format: 'csv' | 'pdf' | 'xlsx') {
    return { format, exportedAt: new Date().toISOString(), recordCount: this.workOrders.length };
  }

  favorite(userId: string, entityType: FacilitiesFavorite['entityType'], entityId: string) {
