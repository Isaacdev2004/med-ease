...
export function computeFacilitiesAnalytics(
  facilityId?: string,
): FacilitiesAnalytics {
  ...
  const cal = facilityId
    ...
    : MOCK_CALIBRATION;
  const env = facilityId
    ? MOCK_ENVIRONMENTAL.filter((e) => e.facilityId === facilityId)
    : MOCK_ENVIRONMENTAL;
  ...
  const avgUtil = equipment.length
    ? equipment.reduce((s, e) => s + e.utilizationPercent, 0) / equipment.length
    : 0;
  ...
  return {
    ...
    preventiveCompliance: pm.length
      ? Math.round((pmCompliant / pm.length) * 100)
      : 100,
    slaCompliance: workOrders.length
      ? Math.round((slaOk / workOrders.length) * 100)
      : 100,
    ...
    calibrationCompliance: cal.length
      ? Math.round((calOk / cal.length) * 100)
      : 100,
    energyUsageKwh:
      45000 +
      (facilityId ? parseInt(facilityId.replace(/\D/g, ''), 10) * 1000 : 0),
    utilityConsumption: 12000 + equipment.length * 2,
    environmentalCompliance: env.length
      ? Math.round((envOk / env.length) * 100)
      : 100,
    openIncidents: incidents.filter(
      (i) => i.status === 'open' || i.status === 'investigating',
    ).length,
    vendorPerformance: MOCK_VENDORS.length
      ? Math.round(
          (MOCK_VENDORS.reduce((s, v) => s + v.rating, 0) /
            MOCK_VENDORS.length) *
            20,
        )
      : 85,
    workOrderTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(
      (label, i) => ({
        label,
    ...
    maintenanceByType: [
      {
        label: 'Preventive',
        value: workOrders.filter((w) => w.type === 'preventive').length,
      },
      {
        label: 'Corrective',
        value: workOrders.filter((w) => w.type === 'corrective').length,
      },
      {
        label: 'Emergency',
        value: workOrders.filter((w) => w.type === 'emergency').length,
      },
    ],
  };
}
