...
export function computeMonitoringAnalytics(): MonitoringAnalytics {
  ...
  const rpmActive = MOCK_RPM_PROGRAMS.filter((r) => r.status === 'active').length;
  ...
  return {
    ...
    dailyVitalTrends: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
      value: 120 + i * 8 + (MOCK_VITALS.length % 20),
    })),
    populationBloodPressure: ['Normal', 'Elevated', 'Stage 1', 'Stage 2', 'Crisis'].map((label, i) => ({
      label,
      value: [45, 22, 18, 10, 5][i]!,
    })),
    heartRateDistribution: ['<55', '55-70', '70-90', '90-110', '>110'].map((label, i) => ({
      label,
      value: [8, 25, 40, 20, 7][i]!,
    })),
    glucoseTrends: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({
      label,
      value: 105 + i * 3,
    })),
    alertTrends: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
      value: 12 + (i % 5) * 3,
    })),
    deviceUtilizationChart: ['Wearable', 'Bedside', 'Home', 'Mobile', 'Gateway'].map((label, i) => ({
      label,
      value: 60 + i * 8,
    })),
    ...
    rpmEnrollmentGrowth: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({
      label,
      value: 20 + i * 15,
    })),
  };
