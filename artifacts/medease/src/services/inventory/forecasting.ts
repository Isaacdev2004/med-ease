...
export function forecastDemand(inventoryId?: string): DemandForecast[] {
  const items = inventoryId
    ? MOCK_INVENTORY.filter((i) => i.inventoryId === inventoryId)
    : MOCK_INVENTORY.slice(0, 50);
  return items.map((item) => {
    const avgDaily =
      1 + (parseInt(item.inventoryId.replace('inv-', ''), 10) % 20);
    ...
    return {
      ...
      recommendedOrder: Math.max(
        0,
        item.reorderQuantity - item.availableQuantity,
      ),
...
export function abcAnalysis() {
  const sorted = [...MOCK_INVENTORY].sort(
    (a, b) =>
      b.quantityOnHand * b.purchasePrice - a.quantityOnHand * a.purchasePrice,
  );
