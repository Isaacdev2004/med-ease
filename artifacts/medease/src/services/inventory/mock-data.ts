...
export const MOCK_WAREHOUSES: Warehouse[] = Array.from(
  { length: 250 },
  (_, i) => ({
      ...
      qty === 0
        ...
        : qty <= reorder
          ...
          : i % 200 === 0
            ...
            : STATUSES[i % 3]!;
    const expiry =
      cat === 'medication' || cat === 'vaccine' || cat === 'reagent'
        ? daysFromNow(30 + (i % 365))
        : undefined;
    return {
...
export const MOCK_ASSETS: MedicalAsset[] = Array.from(
  { length: 1200 },
  (_, i) => {
    const types = [
      'imaging',
    ...
    return {
      ...
      status:
        i % 20 === 0 ? 'maintenance' : i % 50 === 0 ? 'offline' : 'operational',
...
export const MOCK_MOVEMENTS: StockMovement[] = Array.from(
  { length: 2500 },
  (_, i) => {
...
export const MOCK_TRANSFERS: StockTransfer[] = Array.from(
  { length: 400 },
  (_, i) => {
...
export const MOCK_CYCLE_COUNTS: CycleCount[] = Array.from(
  { length: 300 },
  (_, i) => {
    const wh = MOCK_WAREHOUSES[i % MOCK_WAREHOUSES.length]!;
    const statuses = [
...
export const MOCK_EXPIRY_ALERTS: ExpiryAlert[] = MOCK_INVENTORY.filter(
  (i) => i.expiryDate,
)
  .slice(0, 500)
  .map((item, i) => {
...
export function buildDashboard(department?: string, warehouseId?: string) {
  ...
  if (warehouseId)
    items = items.filter((i) => i.warehouseLocation === warehouseId);
  ...
  const pendingOrders = MOCK_PURCHASE_ORDERS.filter((p) =>
    ['pending_approval', 'approved', 'ordered'].includes(p.status),
  ).length;
  const activeTransfers = MOCK_TRANSFERS.filter(
    (t) => t.status === 'pending' || t.status === 'in_transit',
  ).length;
  const assetUtil = Math.round(
  ...
  return {
    ...
    expiryAlerts: MOCK_EXPIRY_ALERTS.filter(
