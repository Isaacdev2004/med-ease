import { activeCampaignCount, campaignOpenRate } from '@/services/messaging/campaign-engine';
import { channelHealthScore } from '@/services/messaging/channel-engine';
import { avgDeliveryTimeMs, bounceRate, deliverySuccessRate, pendingDeliveryCount } from '@/services/messaging/delivery-engine';
...
const SCALE = { messages: 300, inbox: 150, announcements: 40, threads: 50, templates: 60, campaigns: 35, deliveries: 400, broadcasts: 30 };
const ENTERPRISE = { messages: 12_000_000, inbox: 850_000, campaigns: 2_400, templates: 1_200, deliveries: 15_000_000, broadcasts: 48_000, channels: 7 };

const CHANNELS = ['sms', 'email', 'push', 'whatsapp', 'teams', 'slack', 'in_app'] as const;
const SUBJECTS = [
  'Appointment Reminder', 'Lab Results Available', 'Prescription Ready', 'Shift Change Notice',
  'Policy Update', 'Emergency Alert', 'Patient Discharge Summary', 'Meeting Invitation',
  'Credential Renewal', 'Quality Review Required',
];
...
export const MOCK_MESSAGES: Message[] = Array.from({ length: SCALE.messages }, (_, i) => ({
  ...
  status: (['sent', 'delivered', 'read', 'queued', 'failed', 'bounced'] as const)[i % 6]!,
  ...
  templateId: i % 4 === 0 ? `tpl-${String((i % 20) + 1).padStart(3, '0')}` : undefined,
  campaignId: i % 5 === 0 ? `cmp-${String((i % 15) + 1).padStart(3, '0')}` : undefined,
...
export const MOCK_INBOX: InboxItem[] = Array.from({ length: SCALE.inbox }, (_, i) => ({
...
export const MOCK_ANNOUNCEMENTS: Announcement[] = Array.from({ length: SCALE.announcements }, (_, i) => ({
  ...
  facilityId: i % 2 === 0 ? `fac-${String((i % 8) + 1).padStart(3, '0')}` : undefined,
  audience: (['all', 'staff', 'patients', 'department'] as const)[i % 4]!,
  department: i % 4 === 3 ? ['Nursing', 'Pharmacy', 'Radiology', 'Admin'][i % 4] : undefined,
...
export const MOCK_THREADS: ChatThread[] = Array.from({ length: SCALE.threads }, (_, i) => ({
  ...
  unreadCount: i % 4,
  isSecure: i % 3 === 0,
  facilityId: i % 2 === 0 ? `fac-${String((i % 5) + 1).padStart(3, '0')}` : undefined,
}));

export const MOCK_CHAT_MESSAGES: ChatMessage[] = Array.from({ length: 200 }, (_, i) => ({
...
export const MOCK_SECURE_MESSAGES: SecureMessage[] = Array.from({ length: 25 }, (_, i) => ({
...
export const MOCK_BROADCASTS: Broadcast[] = Array.from({ length: SCALE.broadcasts }, (_, i) => ({
  ...
  audience: ['All Staff', 'Nursing Unit', 'Emergency Dept', 'Outpatient'][i % 4]!,
...
export const MOCK_TEMPLATES: MessageTemplate[] = Array.from({ length: SCALE.templates }, (_, i) => ({
...
export const MOCK_CAMPAIGNS: Campaign[] = Array.from({ length: SCALE.campaigns }, (_, i) => ({
  campaignId: `cmp-${String(i + 1).padStart(3, '0')}`,
  name: `Campaign ${(i % 15) + 1}`,
  description: `Multi-channel outreach campaign ${i + 1}`,
  channels: [CHANNELS[i % CHANNELS.length]!, CHANNELS[(i + 2) % CHANNELS.length]!],
  templateId: MOCK_TEMPLATES[i % MOCK_TEMPLATES.length]!.templateId,
  status: (['running', 'scheduled', 'completed', 'draft', 'cancelled'] as const)[i % 5]!,
...
export const MOCK_DELIVERIES: DeliveryRecord[] = Array.from({ length: SCALE.deliveries }, (_, i) => ({
  deliveryId: `dlv-${String(i + 1).padStart(5, '0')}`,
  messageId: MOCK_MESSAGES[i % MOCK_MESSAGES.length]!.messageId,
  channel: CHANNELS[i % CHANNELS.length]!,
  recipientId: `user-${String((i % 30) + 1).padStart(5, '0')}`,
  status: (['queued', 'sent', 'delivered', 'read', 'failed', 'bounced'] as const)[i % 6]!,
  attempts: 1 + (i % 3),
...
export const MOCK_CHANNELS: ChannelConfig[] = CHANNELS.map((channel, i) => ({
  ...
  provider: ['Twilio', 'SendGrid', 'Firebase', 'Meta', 'Microsoft', 'Slack', 'Med-Ease'][i]!,
  health: (['healthy', 'healthy', 'degraded', 'healthy', 'healthy', 'offline', 'healthy'] as const)[i]!,
  enabled: i !== 5,
...
export const MOCK_INTEGRATIONS: Integration[] = [
  { integrationId: 'int-001', name: 'Twilio SMS', provider: 'Twilio', channel: 'sms', status: 'connected', lastSyncAt: daysAgo(0) },
  { integrationId: 'int-002', name: 'SendGrid Email', provider: 'SendGrid', channel: 'email', status: 'connected', lastSyncAt: daysAgo(0) },
  { integrationId: 'int-003', name: 'Firebase Push', provider: 'Firebase', channel: 'push', status: 'connected', lastSyncAt: daysAgo(1) },
  { integrationId: 'int-004', name: 'WhatsApp Business', provider: 'Meta', channel: 'whatsapp', status: 'connected', lastSyncAt: daysAgo(0) },
  { integrationId: 'int-005', name: 'Microsoft Teams', provider: 'Microsoft', channel: 'teams', status: 'connected', lastSyncAt: daysAgo(2) },
  { integrationId: 'int-006', name: 'Slack Workspace', provider: 'Slack', channel: 'slack', status: 'error', lastSyncAt: daysAgo(5) },
];
...
export function buildMessagingDashboard(facilityId?: string): MessagingDashboard {
  ...
  return {
    ...
    messageTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
      value: 1200 + i * 180,
    })),
...
export function computeMessagingAnalytics(facilityId?: string): MessagingAnalytics {
  ...
  return {
    ...
    campaignPerformance: ['Q1 Outreach', 'Patient Reminders', 'Staff Alerts', 'Compliance Notices'].map((label, i) => ({
      label,
      value: 70 + i * 8,
    })),
  };
