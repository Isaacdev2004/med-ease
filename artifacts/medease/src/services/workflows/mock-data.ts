...
import {
  slaBreaches,
  slaComplianceRate,
} from '@/services/workflows/sla-engine';
import {
  automationRate,
  pendingTaskCount,
} from '@/services/workflows/task-engine';
...
export const MOCK_WORKFLOW_DEFINITIONS: WorkflowDefinition[] = Array.from(
...
export const MOCK_WORKFLOW_INSTANCES: WorkflowInstance[] = Array.from(
  { length: SCALE.instances },
  (_, i) => {
    const def =
      MOCK_WORKFLOW_DEFINITIONS[i % MOCK_WORKFLOW_DEFINITIONS.length]!;
    return {
      ...
      facilityId:
        i % 3 === 0
          ...
          : undefined,
      slaStatus: (['on_track', 'on_track', 'at_risk', 'breached'] as const)[
        i % 4
      ]!,
    };
  },
...
export const MOCK_WORKFLOW_TASKS: WorkflowTask[] = Array.from(
  { length: SCALE.tasks },
  (_, i) => ({
    taskId: `task-${String(i + 1).padStart(5, '0')}`,
    instanceId:
      MOCK_WORKFLOW_INSTANCES[i % MOCK_WORKFLOW_INSTANCES.length]!.instanceId,
    workflowName:
      MOCK_WORKFLOW_INSTANCES[i % MOCK_WORKFLOW_INSTANCES.length]!.workflowName,
    title: `Task ${(i % 50) + 1}`,
    type: i % 4 === 0 ? ('automated' as const) : ('human' as const),
    status: (
      ['pending', 'in_progress', 'completed', 'escalated', 'cancelled'] as const
    )[i % 5]!,
    assigneeId:
      i % 4 !== 0 ? `user-${String((i % 25) + 1).padStart(5, '0')}` : undefined,
    module: MODULES[i % MODULES.length]!,
...
export const MOCK_APPROVALS: Approval[] = Array.from(
  { length: SCALE.approvals },
  (_, i) => ({
...
export const MOCK_BUSINESS_RULES: BusinessRule[] = Array.from(
  { length: SCALE.rules },
  (_, i) => ({
...
export const MOCK_SLAS: SLA[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  workflowId:
    MOCK_WORKFLOW_DEFINITIONS[i % MOCK_WORKFLOW_DEFINITIONS.length]!.workflowId,
  name: `SLA ${i + 1}`,
...
export const MOCK_ESCALATIONS: Escalation[] = Array.from(
  { length: 40 },
  (_, i) => ({
    escalationId: `esc-${String(i + 1).padStart(4, '0')}`,
    taskId: MOCK_WORKFLOW_TASKS[i % MOCK_WORKFLOW_TASKS.length]!.taskId,
    reason: ['SLA breach', 'Approver unavailable', 'Priority escalation'][
      i % 3
    ]!,
...
export const MOCK_TIMERS: WorkflowTimer[] = Array.from(
  { length: 50 },
  (_, i) => ({
...
export const MOCK_WORKFLOW_EVENTS: WorkflowEvent[] = Array.from(
  { length: SCALE.events },
  (_, i) => ({
...
export const MOCK_BACKGROUND_JOBS: BackgroundJob[] = Array.from(
  { length: SCALE.jobs },
  (_, i) => ({
...
export const MOCK_SCHEDULES: WorkflowSchedule[] = Array.from(
  { length: 25 },
  (_, i) => ({
...
export const MOCK_PROCESS_TEMPLATES: ProcessTemplate[] = WORKFLOW_NAMES.map(
  (name, i) => ({
...
export const MOCK_WORKFLOW_TRIGGERS: WorkflowTrigger[] = Array.from(
  { length: 40 },
  (_, i) => ({
...
export const MOCK_WORKFLOW_AUDITS: WorkflowAudit[] = Array.from(
  { length: SCALE.audits },
  (_, i) => ({
...
export function buildWorkflowDashboard(facilityId?: string): WorkflowDashboard {
  ...
  if (facilityId)
    instances = instances.filter((i) => i.facilityId === facilityId);

  return {
    totalDefinitions: ENTERPRISE.definitions,
    activeInstances:
      instances.filter((i) => i.status === 'running').length * 120,
    pendingTasks: pendingTaskCount(MOCK_WORKFLOW_TASKS) * 200,
    pendingApprovals:
      MOCK_APPROVALS.filter((a) => a.status === 'pending').length * 80,
    ...
    instanceTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(
      (label, i) => ({
        label,
        value: 80 + i * 15,
      }),
...
export function computeWorkflowAnalytics(
  ...
  return {
    ...
    slaTrend: ['W1', 'W2', 'W3', 'W4'].map((label, i) => ({
      label,
      value: 92 - i * 2,
    })),
  };
