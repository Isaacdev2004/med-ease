...
export function computeAiAnalytics(facilityId?: string): AiAnalytics {
  ...
  const forecasts = facilityId
    ? MOCK_FORECASTS.filter((f) => f.facilityId === facilityId)
    : MOCK_FORECASTS;
  ...
  return {
    ...
    modelDriftScore:
      Math.round(
        (MOCK_AI_ALERTS.filter((a) => a.type === 'model_drift').length /
          Math.max(MOCK_AI_ALERTS.length, 1)) *
          100,
      ) / 100,
    biasAlerts: bias.length * 15,
    forecastAccuracy: forecastAccuracy(forecasts),
    predictionTrend: dashboard.predictionTrend,
    modelPerformance: ['Accuracy', 'Precision', 'Recall', 'F1', 'AUC'].map(
      (label, i) => ({
        label,
        value: Math.round(
          (evaluations.length / Math.max(MOCK_MODEL_EVALUATIONS.length, 1)) *
            100 +
            i * 2,
        ),
      }),
    ...
    recommendationTrend: ['W1', 'W2', 'W3', 'W4'].map((label, i) => ({
      label,
      value: Math.round(
        recommendations.filter((r) => r.status === 'accepted').length / 4 +
          i * 20,
      ),
    })),
    usageByModule: [
      ...
      { label: 'Operations', value: forecasts.length * 30 },
      {
        ...
        value: Math.round(predictions.length * 0.4) * 100,
      },
      { label: 'Research', value: Math.round(predictions.length * 0.15) * 100 },
      {
        label: 'Public Health',
        value: Math.round(predictions.length * 0.2) * 100,
      },
    ],
  };
}
