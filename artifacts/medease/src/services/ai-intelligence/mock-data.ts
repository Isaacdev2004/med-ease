import { PREDICTION_LABELS, PREDICTION_TYPES, computePredictionScore, scoreToRiskLevel } from '@/services/ai-intelligence/predictive-models';
import { RECOMMENDATION_CATEGORIES, recommendationPriority } from '@/services/ai-intelligence/recommendation-engine';
import { FORECAST_LABELS, FORECAST_TYPES, buildForecastTrend } from '@/services/ai-intelligence/forecasting';
...
export const MOCK_PREDICTIONS: AiPrediction[] = Array.from({ length: 400 }, (_, i) => {
  ...
  return {
    ...
    encounterId: i % 2 === 0 ? `enc-${String((i % 3000) + 1).padStart(5, '0')}` : undefined,
  };
});

export const MOCK_RISK_ASSESSMENTS: RiskAssessment[] = Array.from({ length: 300 }, (_, i) => {
  const category = PREDICTION_LABELS[PREDICTION_TYPES[i % PREDICTION_TYPES.length]!]!;
  const score = computePredictionScore(PREDICTION_TYPES[i % PREDICTION_TYPES.length]!, i + 7);
  return {
    ...
    factors: ['Age', 'Comorbidities', 'Recent vitals trend'].slice(0, 2 + (i % 2)),
    modelVersion: `v${1 + (i % 4)}.0`,
  };
});

export const MOCK_RECOMMENDATIONS: ClinicalRecommendation[] = Array.from({ length: 500 }, (_, i) => {
  const category = RECOMMENDATION_CATEGORIES[i % RECOMMENDATION_CATEGORIES.length]!;
  const status = (['pending', 'accepted', 'rejected', 'overridden'] as const)[i % 4]!;
  return {
...
export const MOCK_COPILOT_SESSIONS: CopilotSession[] = Array.from({ length: 300 }, (_, i) => {
  const prompt = COPILOT_PROMPTS[i % COPILOT_PROMPTS.length]!;
  return {
    sessionId: `cop-${String(i + 1).padStart(5, '0')}`,
    patientId: i % 3 === 0 ? `phr-${String((i % 2000) + 1).padStart(4, '0')}` : undefined,
    facilityId: FACILITIES[i % 25]!,
    ...
    messages: [
      { messageId: `msg-${i}-1`, role: 'user', content: prompt, timestamp: daysAgo(i % 14) },
      { messageId: `msg-${i}-2`, role: 'assistant', content: 'Based on available clinical data, I recommend reviewing recent vitals and care plan goals.', timestamp: daysAgo(i % 13) },
    ],
  };
});

export const MOCK_CLINICAL_SUMMARIES: ClinicalSummary[] = Array.from({ length: 400 }, (_, i) => {
  ...
  return {
    ...
    keyFindings: ['Vitals stable', 'Medications reconciled', 'Follow-up scheduled'],
    generatedAt: daysAgo(i % 25),
    modelVersion: `v2.${i % 5}`,
  };
});

export const MOCK_FORECASTS: OperationalForecast[] = Array.from({ length: 150 }, (_, i) => {
  ...
  return {
    ...
    horizonDays: 7 + (i % 14),
    predictedValue,
    confidenceInterval: { lower: predictedValue - 8, upper: predictedValue + 8 },
...
export const MOCK_AI_ALERTS: AiAlert[] = Array.from({ length: 200 }, (_, i) => {
  ...
  return {
    ...
    facilityId: FACILITIES[i % 25]!,
    type: ['model_drift', 'bias_threshold', 'prediction_spike', 'copilot_usage'][i % 4]!,
    severity,
    ...
    acknowledged: i % 4 === 0,
    modelId: i % 2 === 0 ? `model-${String((i % 50) + 1).padStart(3, '0')}` : undefined,
  };
});

export const MOCK_MODEL_VERSIONS: ModelVersion[] = Array.from({ length: 50 }, (_, i) => {
  const status = (['draft', 'staging', 'production', 'archived'] as const)[i % 4]!;
  const type = i % 3 === 0 ? PREDICTION_TYPES[i % PREDICTION_TYPES.length]! : i % 3 === 1 ? FORECAST_TYPES[i % FORECAST_TYPES.length]! : (['nlp', 'copilot', 'recommendation'] as const)[i % 3]!;
  return {
...
export const MOCK_MODEL_EVALUATIONS: ModelEvaluation[] = Array.from({ length: 150 }, (_, i) => {
...
export const MOCK_EXPLAINABILITY: ExplainabilityReport[] = Array.from({ length: 200 }, (_, i) => {
...
export const MOCK_BIAS_METRICS: BiasMetric[] = Array.from({ length: 80 }, (_, i) => {
  ...
  return {
    ...
    status: disparityIndex > 0.12 ? 'action_required' : disparityIndex > 0.08 ? 'monitoring' : 'within_threshold',
    measuredAt: daysAgo(i % 30),
  };
});

export const MOCK_AI_AUDIT: import('@/services/ai-intelligence/types').AiAuditLog[] = Array.from({ length: 100 }, (_, i) => ({
  auditId: `aiaudit-${String(i + 1).padStart(4, '0')}`,
  action: ['generate_prediction', 'start_copilot', 'approve_model', 'rate_recommendation', 'export'][i % 5]!,
  actorId: `user-${String((i % 50) + 1).padStart(3, '0')}`,
  resourceType: ['prediction', 'copilot', 'model', 'recommendation', 'summary'][i % 5]!,
...
export function buildAiDashboard(facilityId?: string): AiIntelligenceDashboard {
  ...
  const alerts = facilityId ? MOCK_AI_ALERTS.filter((a) => a.facilityId === facilityId) : MOCK_AI_ALERTS;
  ...
  return {
    ...
    highRiskPatients: predictions.filter((p) => p.riskLevel === 'high' || p.riskLevel === 'critical').length * SCALE.riskAssessments,
    pendingRecommendations: recommendations.filter((r) => r.status === 'pending').length * SCALE.recommendations,
    activeCopilotSessions: sessions.filter((s) => new Date(s.lastMessageAt).getTime() > Date.now() - 86400000).length * 50,
    modelAccuracy: Math.round((models.reduce((s, m) => s + m.accuracy, 0) / Math.max(models.length, 1)) * 100),
    alertsOpen: alerts.filter((a) => !a.acknowledged).length * SCALE.alerts,
    predictionTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
      value: Math.round(predictions.length / 7 + i * (predictions.length / 20)) * SCALE.predictions,
    })),
    riskDistribution: ['low', 'moderate', 'high', 'critical'].map((label) => ({
      label,
      value: predictions.filter((p) => p.riskLevel === label).length * SCALE.predictions,
    })),
