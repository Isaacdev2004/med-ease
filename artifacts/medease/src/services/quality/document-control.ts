...
export function policyComplianceRate(policies: PolicyDocument[]): number {
  ...
  const compliant = policies.filter((p) => p.status === 'published' || p.status === 'approved').length;
  return Math.round((compliant / policies.length) * 100);
}

export function policiesDueForReview(policies: PolicyDocument[], withinDays = 30): PolicyDocument[] {
...
export function expiredPolicies(policies: PolicyDocument[]): PolicyDocument[] {
  return policies.filter((p) => p.status === 'expired' || (p.expiryDate && new Date(p.expiryDate) < new Date()));
}
