...
export function computeQualityAnalytics(facilityId?: string): QualityAnalytics {
  ...
  const indicators = facilityId ? MOCK_QUALITY_INDICATORS.filter((i) => i.facilityId === facilityId) : MOCK_QUALITY_INDICATORS;
  ...
  return {
    ...
    incidentTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({ label, value: Math.round(incidents.length / 600 + i * 8) })),
    riskByCategory: ['clinical', 'operational', 'financial', 'it', 'cybersecurity', 'regulatory'].map((label) => ({
      label,
      value: risks.filter((r) => r.category === label).length,
    })),
    capaTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({ label, value: Math.round(capa.filter((c) => c.status === 'closed').length / 6 + i * 5) })),
    departmentRisk: risks.slice(0, 8).map((r) => ({ label: r.departmentId ?? 'General', value: r.riskScore })),
