...
export const MOCK_INCIDENTS: IncidentReport[] = Array.from(
  { length: 5000 },
  (_, i) => {
    ...
    return {
      ...
      carePlanId:
        i % 5 === 0
          ? `cp-${String((i % 2000) + 1).padStart(5, '0')}`
          : undefined,
      appointmentId:
        i % 6 === 0
          ? `apt-${String((i % 8000) + 1).padStart(5, '0')}`
          : undefined,
    };
  },
);
...
export const MOCK_RISKS: Risk[] = Array.from({ length: 2500 }, (_, i) => {
  ...
  return {
    ...
    mitigationPlan:
      i % 2 === 0 ? 'Implement controls and monitoring' : undefined,
...
export const MOCK_CAPA: CapaRecord[] = Array.from({ length: 2000 }, (_, i) => {
  ...
  return {
    ...
    incidentId:
      i % 3 === 0
        ? `inc-${String((i % 5000) + 1).padStart(5, '0')}`
        : undefined,
    riskId:
      i % 4 === 0
        ? `risk-${String((i % 2500) + 1).padStart(5, '0')}`
        : undefined,
    facilityId: fac.id,
    assignedTo: `emp-${String((i % 500) + 1).padStart(5, '0')}`,
    dueDate: daysFromNow(14 + (i % 45)),
    rootCause:
      i % 2 === 0 ? 'Process deviation identified during review' : undefined,
    actionPlan: 'Update SOP, train staff, verify effectiveness',
    verificationNotes:
      statuses[i % statuses.length] === 'closed'
        ? 'Verified effective'
        : undefined,
    effectivenessScore:
      statuses[i % statuses.length] === 'closed' ? 85 + (i % 15) : undefined,
  };
});

export const MOCK_AUDITS: AuditRecord[] = Array.from(
  { length: 1500 },
  (_, i) => {
    const fac = FACILITIES[i % 25]!;
...
export const MOCK_AUDIT_FINDINGS: AuditFinding[] = Array.from(
  { length: 800 },
  (_, i) => ({
...
export const MOCK_POLICIES: PolicyDocument[] = Array.from(
  { length: 800 },
  (_, i) => {
    const fac = FACILITIES[i % 25]!;
    ...
    return {
      ...
      approvedBy:
        i % 3 !== 0
          ? `emp-${String((i % 50) + 1).padStart(5, '0')}`
          : undefined,
      contentSummary:
        'Enterprise policy for quality, safety, and regulatory compliance.',
    };
  },
);

export const MOCK_ACCREDITATION: AccreditationStandard[] = Array.from(
  { length: 200 },
  (_, i) => {
    const frameworks = [
      'jci',
...
export const MOCK_COMPLIANCE: ComplianceRecord[] = Array.from(
  { length: 150 },
  (_, i) => {
    const domains = [
      'hipaa',
...
export const MOCK_INFECTIONS: InfectionRecord[] = Array.from(
  { length: 500 },
  (_, i) => {
    const fac = FACILITIES[i % 25]!;
    ...
    return {
      ...
      outbreakId:
        i % 15 === 0
          ? `obk-${String((i % 20) + 1).padStart(3, '0')}`
          : undefined,
...
export const MOCK_RCA: RootCauseAnalysis[] = Array.from(
  { length: 300 },
  (_, i) => ({
    ...
    categories:
      i % 3 === 0
        ...
        : undefined,
    whys:
      i % 3 === 1 ? ['Why 1', 'Why 2', 'Why 3', 'Why 4', 'Why 5'] : undefined,
...
export function buildQualityDashboard(facilityId?: string): QualityDashboard {
  ...
  const infections = facilityId
    ...
    : MOCK_INFECTIONS;
  const compliance = facilityId
    ? MOCK_COMPLIANCE.filter((c) => c.facilityId === facilityId)
    : MOCK_COMPLIANCE;
  ...
  const avgAudit =
    ...
    Math.max(audits.filter((a) => a.score != null).length, 1);
  const openFindings =
    MOCK_AUDIT_FINDINGS.filter((f) => f.status !== 'closed').length /
    (facilityId ? 25 : 1);
  const infectionRate =
    Math.round((infections.length / Math.max(incidents.length, 1)) * 1000) / 10;
  const accreditationReadiness = Math.round(
    MOCK_ACCREDITATION.reduce((s, a) => s + a.complianceScore, 0) /
      MOCK_ACCREDITATION.length,
  );
  const compliancePercent = Math.round(
    compliance.reduce((s, c) => s + c.score, 0) /
      Math.max(compliance.length, 1),
  );
  const publishedPolicies = policies.filter(
    (p) => p.status === 'published',
  ).length;
  const policyCompliance = policies.length
    ? Math.round((publishedPolicies / policies.length) * 100)
    : 92;
  ...
  return {
    ...
    escalatedIncidents:
      incidents.filter((i) => i.escalated).length / (facilityId ? 1 : 25),
    openRisks:
      risks.filter((r) => r.status !== 'closed').length / (facilityId ? 1 : 25),
    highRisks:
      risks.filter((r) => r.level === 'high' || r.level === 'extreme').length /
      (facilityId ? 1 : 25),
    openCapa:
      capa.filter((c) => c.status !== 'closed').length / (facilityId ? 1 : 25),
    capaCompletionRate: capa.length
      ? Math.round((closedCapa / capa.length) * 100)
      : 88,
    ...
    riskHeatMap: [
      {
        ...
        likelihood: 4,
        impact: 4,
        count: risks.filter((r) => r.category === 'clinical').length,
      },
      {
        label: 'Operational',
        likelihood: 3,
        impact: 3,
        count: risks.filter((r) => r.category === 'operational').length,
      },
      {
        label: 'IT/Cyber',
        likelihood: 4,
        impact: 5,
        count: risks.filter(
          (r) => r.category === 'it' || r.category === 'cybersecurity',
        ).length,
      },
      {
        label: 'Financial',
        likelihood: 2,
        impact: 4,
        count: risks.filter((r) => r.category === 'financial').length,
      },
      {
        label: 'Regulatory',
        likelihood: 3,
        impact: 5,
        count: risks.filter((r) => r.category === 'regulatory').length,
      },
    ],
  };
}
