...
export function accreditationReadiness(
  ...
  return Math.round(
    standards.reduce((s, st) => s + st.complianceScore, 0) / standards.length,
  );
}
...
export function frameworkScore(
  standards: AccreditationStandard[],
  framework: AccreditationStandard['framework'],
): number {
  const items = standards.filter((s) => s.framework === framework);
  if (!items.length) return 0;
  return Math.round(
    items.reduce((s, i) => s + i.complianceScore, 0) / items.length,
  );
}
