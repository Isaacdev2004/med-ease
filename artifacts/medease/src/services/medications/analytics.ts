...
export function computeMedicationAnalytics(
  ...
  const adherenceAverage = active.length
    ? Math.round(
        active.reduce((s, m) => s + m.adherencePercent, 0) / active.length,
      )
    : 0;
  ...
  for (const rx of prescriptions) {
    drugCounts.set(
      rx.medication.name,
      (drugCounts.get(rx.medication.name) ?? 0) + 1,
    );
  }
  ...
  return {
    ...
    mostPrescribed,
    adherenceByMonth: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map(
      (label, i) => ({
        label,
    ...
    prescriptionStats: [
      ...
      {
        label: 'Expired',
        value: prescriptions.filter((r) => r.status === 'expired').length,
      },
      {
        label: 'Cancelled',
        value: prescriptions.filter((r) => r.status === 'cancelled').length,
      },
    ],
    safetyReports: [
      ...
      { label: 'High severity', value: 28 },
      {
        ...
        value: prescriptions.filter((r) => r.medication.controlledSubstance)
          .length,
      },
    ],
    medicationClasses: [...new Set(active.map((m) => m.medicationClass))]
      .slice(0, 6)
      .map((label, i) => ({
        label,
