...
const BASE_DRUGS = [
  { name: 'Atorvastatine', generic: 'Atorvastatin', class: 'Statin', type: 'Tablet', strength: '20 mg' },
  { name: 'Metformine', generic: 'Metformin', class: 'Antidiabetic', type: 'Tablet', strength: '850 mg' },
  { name: 'Lisinopril', generic: 'Lisinopril', class: 'ACE Inhibitor', type: 'Tablet', strength: '10 mg' },
  { name: 'Oméprazole', generic: 'Omeprazole', class: 'PPI', type: 'Capsule', strength: '20 mg' },
  { name: 'Salbutamol', generic: 'Albuterol', class: 'Bronchodilator', type: 'Inhaler', strength: '100 mcg' },
  { name: 'Lévothyroxine', generic: 'Levothyroxine', class: 'Thyroid', type: 'Tablet', strength: '75 mcg' },
  { name: 'Amoxicilline', generic: 'Amoxicillin', class: 'Antibiotic', type: 'Capsule', strength: '500 mg' },
  { name: 'Paracétamol', generic: 'Acetaminophen', class: 'Analgesic', type: 'Tablet', strength: '500 mg' },
  { name: 'Warfarine', generic: 'Warfarin', class: 'Anticoagulant', type: 'Tablet', strength: '5 mg', controlled: true },
  { name: 'Sertraline', generic: 'Sertraline', class: 'SSRI', type: 'Tablet', strength: '50 mg' },
  { name: 'Amlodipine', generic: 'Amlodipine', class: 'Calcium Channel Blocker', type: 'Tablet', strength: '5 mg' },
  { name: 'Insuline glargine', generic: 'Insulin glargine', class: 'Antidiabetic', type: 'Injection', strength: '100 U/mL' },
  { name: 'Prednisone', generic: 'Prednisone', class: 'Corticosteroid', type: 'Tablet', strength: '20 mg' },
  { name: 'Morphine', generic: 'Morphine', class: 'Opioid', type: 'Tablet', strength: '10 mg', controlled: true },
  { name: 'Vaccin grippe', generic: 'Influenza vaccine', class: 'Vaccine', type: 'Injection', strength: '0.5 mL' },
];

const FORMULATIONS = ['Biogaran', 'Sandoz', 'Mylan', 'Teva', 'Arrow', 'Zentiva'];

export const MEDICATION_CATALOG = Array.from({ length: 300 }, (_, i) => {
  const base = BASE_DRUGS[i % BASE_DRUGS.length]!;
  const suffix = i >= BASE_DRUGS.length ? ` ${FORMULATIONS[i % FORMULATIONS.length]}` : '';
  return {
...
const PHYSICIANS = ['Dr. Emily Chen', 'Dr. Jean-Luc Martin', 'Dr. Sophie Bernard', 'Dr. Antoine Moreau', 'Dr. Marie Dupont'];
const PHARMACIES = Array.from({ length: 25 }, (_, i) => ({
  id: `pharm-${String(i + 1).padStart(3, '0')}`,
  name: ['Pharmacie Centrale', 'Pharmacie du Marché', 'Pharmacie Pasteur', 'Pharmacie de la Gare', 'Pharmacie République'][i % 5]! + (i >= 5 ? ` ${i + 1}` : ''),
}));

export const MOCK_PHARMACISTS = Array.from({ length: 50 }, (_, i) => ({
  id: `pharmacist-${String(i + 1).padStart(3, '0')}`,
  name: ['Marie Lefèvre', 'Pierre Martin', 'Sophie Bernard', 'Lucas Dubois', 'Camille Rousseau'][i % 5]!,
...
const CONDITIONS = ['Hypertension', 'Type 2 Diabetes', 'Hyperlipidemia', 'Asthma', 'Hypothyroidism', 'Anxiety'];
...
export function generatePrescription(index: number, patientIdx: number): Prescription {
  ...
  const statusPool = index % 20 === 0 ? 'expired' : index % 15 === 0 ? 'cancelled' : 'active';
  ...
  return {
    ...
    route: drug.type === 'Inhaler' ? 'inhalation' : drug.type === 'Injection' ? 'injection' : 'oral',
    durationDays,
    startDate,
...
export function prescriptionToPatientMedication(rx: Prescription): PatientMedication {
  const end = rx.endDate ? new Date(rx.endDate) : null;
  const remainingDays = end ? Math.max(0, Math.ceil((end.getTime() - Date.now()) / 86400000)) : undefined;
  const status: PatientMedication['status'] =
    rx.status === 'cancelled' ? 'cancelled'
      : rx.status === 'expired' ? 'completed'
        : remainingDays !== undefined && remainingDays <= 0 ? 'completed'
          : rx.status === 'pending' ? 'future'
            : 'active';
  ...
  return {
    ...
    sideEffects: ['Nausea', 'Headache', 'Dry mouth'].slice(0, (parseInt(rx.id.replace(/\D/g, ''), 10) % 3) + 1),
...
export const MOCK_PRESCRIPTIONS: Prescription[] = Array.from({ length: 2500 }, (_, i) =>
  generatePrescription(i, i % 40),
);

export const MOCK_MEDICATIONS: PatientMedication[] = MOCK_PRESCRIPTIONS
  .filter((rx) => ['active', 'renewed', 'pending'].includes(rx.status))
  .map(prescriptionToPatientMedication);

function buildTodaySchedule(): ScheduledDose[] {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  const slots: ScheduledDose['slot'][] = ['morning', 'afternoon', 'evening', 'night'];
  const hours = [8, 13, 18, 22];

  return MOCK_MEDICATIONS.slice(0, 400).flatMap((med, idx) =>
    slots.slice(0, med.frequency.includes('Twice') ? 2 : 1).map((slot, slotIdx) => {
      ...
      return {
        ...
        slot,
        dose: med.dose,
        status: isPast ? (idx % 5 === 0 ? 'missed' : idx % 3 === 0 ? 'late' : 'taken') : 'pending',
...
export const MOCK_DOSE_LOGS: DoseLog[] = Array.from({ length: 1500 }, (_, i) => {
...
export const MOCK_REMINDERS: MedicationReminder[] = Array.from({ length: 500 }, (_, i) => {
  const med = MOCK_MEDICATIONS[i % MOCK_MEDICATIONS.length]!;
  return {
...
export const MOCK_REFILL_REQUESTS: RefillRequest[] = Array.from({ length: 600 }, (_, i) => {
  const med = MOCK_MEDICATIONS[i % MOCK_MEDICATIONS.length]!;
  ...
  return {
    ...
    status: (['pending', 'approved', 'rejected', 'dispensed', 'partial'] as const)[i % 5]!,
...
export const MOCK_INTERACTIONS: DrugInteraction[] = Array.from({ length: 250 }, (_, i) => {
  const med = MOCK_MEDICATIONS[i % MOCK_MEDICATIONS.length]!;
  const types = ['medication', 'allergy', 'food', 'pregnancy', 'alcohol', 'supplement'] as const;
  ...
  return {
    ...
    type,
    source: med.name,
    target: type === 'allergy' ? 'Penicillin' : DRUGS[(i + 3) % DRUGS.length]!.name,
    severity: (['critical', 'high', 'moderate', 'low'] as const)[i % 4]!,
    recommendation: type === 'allergy' ? 'Do not dispense — documented allergy.' : 'Monitor closely and adjust therapy if needed.',
    active: i % 7 !== 0,
  };
});

export const MOCK_ADMINISTRATIONS: MedicationAdministration[] = Array.from({ length: 1500 }, (_, i) => {
  const med = MOCK_MEDICATIONS[i % MOCK_MEDICATIONS.length]!;
  return {
    ...
    status: (['completed', 'refused', 'held', 'partial'] as const)[i % 10 === 0 ? 1 : 0]!,
    notes: i % 5 === 0 ? 'Patient tolerated well' : undefined,
  };
});

export const MOCK_DISPENSES: MedicationDispense[] = Array.from({ length: 800 }, (_, i) => {
...
export const MOCK_COURSES: MedicationCourse[] = Array.from({ length: 100 }, (_, i) => {
  const med = MOCK_MEDICATIONS[i % MOCK_MEDICATIONS.length]!;
  const total = 14 + (i % 30);
  ...
  return {
    ...
    status: completed >= total ? 'completed' : i % 9 === 0 ? 'paused' : 'active',
  };
});

export function buildMedicationEducation(medicationId: string): MedicationEducation | null {
  ...
  return {
    ...
    instructions: [med.instructions, `Take ${med.frequency}`, `Route: ${med.route}`],
    sideEffects: med.sideEffects,
    storage: med.storage ?? 'Store at room temperature',
    whenToCall: ['Severe allergic reaction', 'Unusual bleeding', 'Persistent vomiting'],
  };
}

export const MOCK_PHARMACY_QUEUE: PharmacyPrescriptionQueue[] = MOCK_PRESCRIPTIONS.slice(0, 120).map((rx, i) => ({
  id: `pq-${i + 1}`,
  prescriptionId: rx.id,
  patientName: rx.patientName,
  medicationName: rx.medication.name,
  status: (['pending', 'approved', 'dispensed', 'delivered'] as const)[i % 4]!,
...
export function buildTimeline(patientId: string): MedicationTimelineEntry[] {
  ...
  for (const rx of MOCK_PRESCRIPTIONS.filter((r) => r.patientId === patientId).slice(0, 8)) {
    entries.push({
      id: `tl-rx-${rx.id}`,
      patientId,
      date: rx.createdAt,
  ...
  for (const log of MOCK_DOSE_LOGS.filter((l) => l.patientId === patientId).slice(0, 6)) {
    entries.push({
      id: `tl-log-${log.id}`,
      patientId,
      date: log.loggedAt,
