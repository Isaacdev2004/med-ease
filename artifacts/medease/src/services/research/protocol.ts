import type { ProtocolDeviation, DeviationStatus } from '@/services/research/types';

export function computeComplianceRate(totalVisits: number, deviations: number): number {
...
export function deviationSeverityWeight(severity: ProtocolDeviation['severity']): number {
  return { minor: 1, major: 3, critical: 5 }[severity];
}

export function aggregateDeviationStatus(items: ProtocolDeviation[]): DeviationStatus {
