import { THERAPEUTIC_AREAS, TRIAL_PHASES, TRIAL_STATUSES } from '@/services/research/clinical-trials';
...
const SCALE = { trials: 5, participants: 50, visits: 20, ae: 5, specimens: 5, audit: 5 };
...
export const MOCK_TRIALS: ClinicalTrial[] = Array.from({ length: 120 }, (_, i) => {
  const phase = TRIAL_PHASES[i % TRIAL_PHASES.length]!;
  const status = TRIAL_STATUSES[i % TRIAL_STATUSES.length]!;
  const target = 50 + (i % 20) * 25;
  const current = Math.min(target, Math.round(target * (0.3 + (i % 7) * 0.1)));
  return {
    ...
    sponsor: ['NIH', 'PharmaCorp', 'BioMed Inc', 'University Consortium', 'WHO'][i % 5]!,
    ...
    carePlanId: i % 4 === 0 ? `cp-${String((i % 200) + 1).padStart(4, '0')}` : undefined,
    fhirResearchStudyId: `rs-${String(i + 1).padStart(4, '0')}`,
  };
});

export const MOCK_INVESTIGATORS: Investigator[] = Array.from({ length: 80 }, (_, i) => ({
...
export const MOCK_COORDINATORS: Coordinator[] = Array.from({ length: 30 }, (_, i) => ({
...
export const MOCK_SITES: StudySite[] = Array.from({ length: 40 }, (_, i) => ({
  ...
  facilityId: FACILITIES[i % 25]!,
  trialIds: [MOCK_TRIALS[i % 120]!.trialId, MOCK_TRIALS[(i + 5) % 120]!.trialId],
...
export const MOCK_PARTICIPANTS: ResearchParticipant[] = Array.from({ length: 500 }, (_, i) => {
  const trial = MOCK_TRIALS[i % 120]!;
  const statuses = ['screening', 'enrolled', 'active', 'completed', 'withdrawn', 'screen_failed'] as const;
  ...
  return {
    ...
    status,
    enrolledAt: status !== 'screening' && status !== 'screen_failed' ? daysAgo(i % 90) : undefined,
    randomizationArm: status === 'enrolled' || status === 'active' ? ['control', 'intervention_a', 'intervention_b'][i % 3] : undefined,
    consentStatus: (['pending', 'signed', 'signed', 'withdrawn', 'expired'] as const)[i % 5]!,
    providerId: `prov-${String((i % 500) + 1).padStart(4, '0')}`,
    appointmentId: i % 3 === 0 ? `appt-${String((i % 5000) + 1).padStart(5, '0')}` : undefined,
  };
});

export const MOCK_VISITS: StudyVisit[] = Array.from({ length: 2000 }, (_, i) => {
  const participant = MOCK_PARTICIPANTS[i % 500]!;
  const visitNum = (i % 12) + 1;
  const statuses = ['scheduled', 'completed', 'missed', 'cancelled', 'in_progress'] as const;
...
export const MOCK_CONSENTS: ConsentRecord[] = MOCK_PARTICIPANTS.filter((p) => p.consentStatus !== 'pending').map((p, i) => ({
...
export const MOCK_DEVIATIONS: ProtocolDeviation[] = Array.from({ length: 200 }, (_, i) => ({
  deviationId: `dev-${String(i + 1).padStart(4, '0')}`,
  trialId: MOCK_TRIALS[i % 120]!.trialId,
  participantId: i % 3 === 0 ? MOCK_PARTICIPANTS[i % 500]!.participantId : undefined,
  facilityId: FACILITIES[i % 25]!,
...
export const MOCK_ADVERSE_EVENTS: AdverseEvent[] = Array.from({ length: 300 }, (_, i) => {
  const participant = MOCK_PARTICIPANTS[i % 500]!;
  const severities = ['mild', 'moderate', 'severe', 'life_threatening', 'fatal'] as const;
  const severity = severities[i % 5]!;
  const serious = severity === 'life_threatening' || severity === 'fatal' || i % 10 === 0;
  return {
    ...
    severity,
    serious,
    status: (['reported', 'under_review', 'submitted', 'closed'] as const)[i % 4]!,
    reportedAt: daysAgo(i % 60),
    medicationId: i % 4 === 0 ? `med-${String((i % 800) + 1).padStart(4, '0')}` : undefined,
    labResultId: i % 5 === 0 ? `lab-${String((i % 2000) + 1).padStart(5, '0')}` : undefined,
  };
});

export const MOCK_BIOSPECIMENS: Biospecimen[] = Array.from({ length: 600 }, (_, i) => {
  const participant = MOCK_PARTICIPANTS[i % 500]!;
  const statuses = ['collected', 'processed', 'stored', 'shipped', 'analyzed', 'disposed'] as const;
  return {
...
export const MOCK_PUBLICATIONS: Publication[] = Array.from({ length: 100 }, (_, i) => ({
  ...
  status: (['draft', 'submitted', 'under_review', 'accepted', 'published'] as const)[i % 5]!,
  submittedAt: i % 5 !== 0 ? daysAgo(i % 180) : undefined,
  publishedAt: i % 5 === 4 ? daysAgo(i % 365) : undefined,
  doi: i % 5 === 4 ? `10.1000/pub.${i + 1}` : undefined,
}));

export const MOCK_INNOVATION: InnovationProject[] = Array.from({ length: 60 }, (_, i) => ({
  ...
  status: (['ideation', 'pilot', 'scaling', 'deployed', 'archived'] as const)[i % 5]!,
  leadInvestigatorId: MOCK_INVESTIGATORS[i % 80]!.investigatorId,
  budget: 50000 + (i % 20) * 25000,
  startDate: daysAgo(365 - (i % 300)),
}));

export const MOCK_GRANTS: GrantApplication[] = Array.from({ length: 80 }, (_, i) => ({
  ...
  status: (['draft', 'submitted', 'under_review', 'awarded', 'rejected'] as const)[i % 5]!,
  submittedAt: i % 5 !== 0 ? daysAgo(i % 200) : undefined,
}));

export const MOCK_REGULATORY: RegulatorySubmission[] = Array.from({ length: 150 }, (_, i) => ({
  submissionId: `reg-${String(i + 1).padStart(4, '0')}`,
  trialId: MOCK_TRIALS[i % 120]!.trialId,
  facilityId: FACILITIES[i % 25]!,
  type: (['IRB', 'FDA', 'EMA', 'DSMB', 'IND', 'IDE'] as const)[i % 6]!,
  status: (['draft', 'submitted', 'approved', 'rejected', 'amended'] as const)[i % 5]!,
  submittedAt: daysAgo(i % 180),
  approvedAt: i % 5 === 2 ? daysAgo(i % 90) : undefined,
}));

export const MOCK_RESEARCH_AUDIT: ResearchAuditLog[] = Array.from({ length: 200 }, (_, i) => ({
  auditId: `raudit-${String(i + 1).padStart(4, '0')}`,
  action: ['enroll_participant', 'record_consent', 'report_ae', 'close_deviation', 'submit_publication', 'register_specimen'][i % 6]!,
  actorId: `user-${String((i % 100) + 1).padStart(3, '0')}`,
  resourceType: ['trial', 'participant', 'consent', 'adverse_event', 'deviation', 'publication'][i % 6]!,
...
export function buildResearchDashboard(facilityId?: string): ResearchDashboard {
  const trials = facilityId ? MOCK_TRIALS.filter((t) => t.facilityId === facilityId) : MOCK_TRIALS;
  const participants = facilityId ? MOCK_PARTICIPANTS.filter((p) => p.facilityId === facilityId) : MOCK_PARTICIPANTS;
  const deviations = facilityId ? MOCK_DEVIATIONS.filter((d) => d.facilityId === facilityId) : MOCK_DEVIATIONS;
  const consents = facilityId ? MOCK_CONSENTS.filter((c) => c.facilityId === facilityId) : MOCK_CONSENTS;
  const aes = facilityId ? MOCK_ADVERSE_EVENTS.filter((e) => e.facilityId === facilityId) : MOCK_ADVERSE_EVENTS;
  const specimens = facilityId ? MOCK_BIOSPECIMENS.filter((s) => s.facilityId === facilityId) : MOCK_BIOSPECIMENS;
  const pubs = MOCK_PUBLICATIONS;

  const activeTrials = trials.filter((t) => t.status === 'active' || t.status === 'recruiting');
  const enrolledThisMonth = participants.filter((p) => p.enrolledAt && new Date(p.enrolledAt) > new Date(daysAgo(30))).length;
  ...
  return {
    ...
    pendingConsents: consents.filter((c) => c.status === 'pending').length + participants.filter((p) => p.consentStatus === 'pending').length,
    seriousAdverseEvents: aes.filter((e) => e.serious).length * SCALE.ae,
    biospecimensStored: specimens.filter((s) => s.status === 'stored' || s.status === 'processed').length * SCALE.specimens,
    publicationsThisYear: pubs.filter((p) => p.status === 'published').length * 8,
    topTrials: [...trials].sort((a, b) => b.currentEnrollment - a.currentEnrollment).slice(0, 5),
    enrollmentTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({
      label,
      value: Math.round(participants.length / 6 + i * (participants.length / 20)),
    })),
  };
}
