...
export function isSeriousAdverseEvent(event: AdverseEvent): boolean {
  return (
    event.serious || ['life_threatening', 'fatal'].includes(event.severity)
  );
}
...
export function severityRank(severity: AdverseEventSeverity): number {
  return { mild: 1, moderate: 2, severe: 3, life_threatening: 4, fatal: 5 }[
    severity
  ];
}
...
export function toFhirAdverseEvent(event: AdverseEvent) {
  return {
    ...
    category: [
      {
        coding: [
          { code: event.serious ? 'product-problem' : 'medication-mistake' },
        ],
      },
    ],
