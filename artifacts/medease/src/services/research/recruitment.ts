...
export function computeRecruitmentScore(trial: ClinicalTrial, candidates: number): number {
...
export function isEligibleForScreening(participant: ResearchParticipant, criteriaMet: boolean): boolean {
  return participant.status === 'screening' && criteriaMet;
}

export const DEFAULT_RANDOMIZATION_ARMS = ['control', 'intervention_a', 'intervention_b'];
