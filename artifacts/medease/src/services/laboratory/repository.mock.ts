...
class LaboratoryMockRepository {
  ...
  async getCriticalAlerts(patientId?: string) {
  ...
  async getPendingResults(patientId?: string) {
    return (await this.getAllResults(patientId ? { patientId } : undefined)).filter(
  ...
  async uploadResult(input: UploadResultInput) {
    ...
    const obs = input.observations.map((o, i) => {
      ...
      return {
        ...
        flag: test ? computeResultFlag(test, o.numericValue ?? 0) : 'normal' as const,
...
export const laboratoryMockRepository = new LaboratoryMockRepository();
