...
const RESULT_STATUSES: LabResultStatus[] = [
  'pending',
  'processing',
  'verified',
  'released',
...
export function generateLabOrder(index: number): LabOrder {
  ...
  const testIds = Array.from(
    { length: testCount },
    (_, i) => LAB_TEST_CATALOG[(index + i) % LAB_TEST_CATALOG.length]!.id,
  );
  const testNames = testIds.map((id) => getTestById(id)!.name);
  const status = pick(ORDER_STATUSES, index);
  const priority =
    index % 20 === 0 ? 'stat' : index % 7 === 0 ? 'urgent' : 'routine';
  ...
  return {
    ...
    priority,
    status,
    collectionMethod: pick(
      ['in_clinic', 'home_collection', 'external_lab', 'referral'] as const,
      index,
    ),
    ...
    diagnosis:
      index % 3 === 0
        ? pick(
            ['E11.9 Type 2 diabetes', 'I10 Hypertension', 'J44.9 COPD'],
            index,
          )
        : undefined,
    carePlanId:
    ...
    collectedAt: ['collected', 'in_progress', 'completed'].includes(status)
      ? daysAgo(daysBack - 1)
      : undefined,
  ...
  return {
    ...
    interpretation:
      flag === 'normal'
        ...
        : `${test.name} ${flag.replace('_', ' ')}.`,
    patientFriendlyText:
      flag === 'normal'
        ? `Your ${test.name} result is within the normal range.`
        : `Your ${test.name} result needs review by your care team.`,
...
function generateReport(order: LabOrder, index: number): LabDiagnosticReport {
  ...
  return {
    ...
    summary:
      status === 'released'
        ? 'Results within expected clinical context.'
        : undefined,
    observationIds: order.testIds.map((tid) => `obs-${order.id}-${tid}`),
    verifiedBy: ['verified', 'released', 'corrected'].includes(status)
      ? pick(PHYSICIANS, index).name
      : undefined,
...
function generateSpecimen(order: LabOrder, index: number): SpecimenRecord {
  ...
  return {
    ...
    collectedAt,
    receivedAt: ['received', 'processing', 'stored'].includes(status)
      ? daysAgo(index % 15)
      : undefined,
    temperature: '2–8°C',
    storageLocation:
      status === 'stored' ? `Rack ${index % 10}-Shelf ${index % 5}` : undefined,
    rejectionReason:
      status === 'rejected'
        ? 'Hemolyzed sample — recollection required.'
        : undefined,
    chainOfCustody: [
      {
        id: 'c1',
      ...
      {
        id: 'c2',
        timestamp: daysAgo(index % 18),
        status: 'in_transit',
        actor: 'Transport Unit',
      },
      {
        id: 'c3',
        timestamp: daysAgo(index % 16),
        status: 'received',
        actor: 'Lab Reception',
      },
    ],
...
export const MOCK_LAB_ORDERS: LabOrder[] = Array.from(
  { length: 2000 },
  (_, i) => generateLabOrder(i),
);

const completedOrders = MOCK_LAB_ORDERS.filter((o) => o.status === 'completed');

export const MOCK_LAB_OBSERVATIONS: LabObservation[] = completedOrders.flatMap(
  (order, i) =>
    order.testIds.map((testId, ti) =>
      generateObservation(order, testId, i * 10 + ti),
    ),
);

export const MOCK_LAB_REPORTS: LabDiagnosticReport[] = completedOrders.map(
  (order, i) => generateReport(order, i),
);

export const MOCK_SPECIMENS: SpecimenRecord[] = MOCK_LAB_ORDERS.filter(
  (_, i) => i % 4 === 0,
)
  .slice(0, 500)
  .map((order, i) => generateSpecimen(order, i));

export const MOCK_LAB_ALERTS: LabAlert[] = MOCK_LAB_OBSERVATIONS.filter(
  (o) => o.flag !== 'normal',
)
  .slice(0, 200)
  .map((obs, i) => generateAlert(obs, i));

const MICRO_REPORTS = MOCK_LAB_REPORTS.filter(
  (r) => r.category === 'microbiology' || r.category === 'virology',
);
...
const COMPONENTS: BloodBankRecord['component'][] = [
  'RBC',
  'Platelets',
  'Plasma',
  'Whole Blood',
...
export const MOCK_MICROBIOLOGY: MicrobiologyResult[] = MICRO_REPORTS.slice(
  0,
  120,
).map((r, i) => ({
  id: `micro-${r.id}`,
  ...
  status: r.status,
  gramStain:
    i % 3 === 0 ? 'Gram-negative rods' : 'Gram-positive cocci in clusters',
  cultures: [
    {
...
export const MOCK_PATHOLOGY: PathologyResult[] = PATH_REPORTS.slice(0, 80).map(
  (r, i) => ({
    id: `path-${r.id}`,
    reportId: r.id,
    patientId: r.patientId,
    ...
    microscopic:
      'Sections show tissue architecture with focal inflammatory changes.',
    histology: [
      {
        id: `hist-${r.id}`,
        reportId: r.id,
        ...
        diagnosis:
          i % 9 === 0 ? 'Suspicious — recommend follow-up' : 'Benign histology',
...
export const MOCK_BLOOD_BANK: BloodBankRecord[] = Array.from(
  { length: 100 },
  (_, i) => ({
    ...
    crossMatchResult:
      i % 11 === 0 ? 'incompatible' : i % 5 === 0 ? 'pending' : 'compatible',
...
export const MOCK_INSTRUMENTS: LaboratoryInstrument[] = [
  ...
  },
  {
    ...
    lastCalibration: daysAgo(1),
  },
];

export const MOCK_QUALITY_CONTROL: QualityControl[] = MOCK_INSTRUMENTS.flatMap(
  (inst, i) =>
    Array.from({ length: 8 }, (_, j) => ({
      id: `qc-${inst.id}-${j}`,
      instrumentId: inst.id,
      testName: ['Hemoglobin', 'Glucose', 'WBC', 'Creatinine', 'PT/INR'][
        j % 5
      ]!,
...
export function buildQualityDashboard(): QualityDashboard {
  const pending = MOCK_LAB_REPORTS.filter(
  ...
  return {
    ...
    kpis: [
      ...
      { label: 'Pending verify', value: pending },
      {
        ...
        value: MOCK_SPECIMENS.filter((s) => s.status === 'rejected').length,
      },
...
export function buildLabTimeline(patientId: string): LabTimelineEntry[] {
  ...
  for (const order of MOCK_LAB_ORDERS.filter(
  ...
  for (const spec of MOCK_SPECIMENS.filter(
  ...
  for (const rep of MOCK_LAB_REPORTS.filter(
...
export function buildDashboard(patientId?: string): LaboratoryDashboard {
  ...
  const collectedSamples = orders.filter(
  ...
  const resultsReady = MOCK_LAB_REPORTS.filter((r) =>
    patientId
      ? r.patientId === patientId && r.status === 'released'
      : r.status === 'released',
  ).length;
  const criticalResults = MOCK_LAB_ALERTS.filter((a) =>
    patientId
      ? a.patientId === patientId && a.severity === 'critical'
      : a.severity === 'critical',
  ).length;
  ...
  return {
    ...
    resultsReady,
    criticalResults,
    rejectedSamples: MOCK_SPECIMENS.filter((s) => s.status === 'rejected')
      .length,
...
export function buildTrends(patientId: string): LabTrendSeries[] {
  ...
  return trendTests.map((testId, ti) => {
    const test = getTestById(testId)!;
    const obs = MOCK_LAB_OBSERVATIONS.filter(
      (o) => o.patientId === patientId && o.testId === testId,
    )
      .slice(0, 8)
      .sort(
        (a, b) =>
    ...
    const points = obs.length
      ? obs.map((o) => ({
          date: o.resultedAt ?? o.collectedAt,
          value: o.numericValue ?? 0,
        }))
...
export function buildAnalytics(): LaboratoryAnalytics {
  const completed = MOCK_LAB_ORDERS.filter(
  ...
  return {
    ...
    criticalResultCount: MOCK_LAB_ALERTS.filter(
      (a) => a.severity === 'critical',
    ).length,
    rejectionRate: 2.4,
    qualityScore: 94,
    verificationRate: 96.2,
    pendingVerification: MOCK_LAB_REPORTS.filter(
      (r) => r.status === 'processing' || r.status === 'pending',
    ).length,
    instrumentUtilization: 72,
