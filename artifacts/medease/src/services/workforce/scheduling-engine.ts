...
export function validateShiftSwap(
  ...
  if (shiftA.departmentId !== shiftB.departmentId)
    return { valid: false, reason: 'Different departments' };
  if (shiftA.status === 'completed' || shiftB.status === 'completed')
    return { valid: false, reason: 'Shift already completed' };
  return { valid: true };
}
...
export function generateWeeklyRoster(
  employeeIds: string[],
  startDate: Date,
): { employeeId: string; day: number; shiftType: Shift['shiftType'] }[] {
  const weekOffset = startDate.getDay();
  return employeeIds.flatMap((employeeId, idx) =>
    Array.from({ length: 7 }, (_, day) => ({
      employeeId,
      day,
      shiftType: (['day', 'evening', 'night'] as const)[
        (idx + day + weekOffset) % 3
      ]!,
    })),
  );
}

export function balanceNightShifts(
  shifts: Shift[],
): { employeeId: string; nightCount: number }[] {
  const counts = new Map<string, number>();
  for (const s of shifts.filter((x) => x.shiftType === 'night')) {
    counts.set(s.employeeId, (counts.get(s.employeeId) ?? 0) + 1);
  }
  return [...counts.entries()]
    .map(([employeeId, nightCount]) => ({ employeeId, nightCount }))
    .sort((a, b) => a.nightCount - b.nightCount);
}
