...
export function getExpiringCredentials(employee: Employee, withinDays = 30) {
  ...
  const licenses = employee.licenses.filter(
    (l) => new Date(l.expiryDate).getTime() <= cutoff,
  );
  const certs = employee.certifications.filter(
    (c) => new Date(c.expiryDate).getTime() <= cutoff,
  );
  return { licenses, certifications: certs };
}

export function verifyCredential(item: ProfessionalLicense | Certification): {
  verified: boolean;
  status: string;
} {
...
export function mandatoryTrainingCompliance(training: Training[]): {
  complete: number;
  overdue: number;
  rate: number;
} {
  ...
  return {
    complete,
    overdue,
    rate: mandatory.length
      ? Math.round((complete / mandatory.length) * 100)
      : 100,
  };
}
