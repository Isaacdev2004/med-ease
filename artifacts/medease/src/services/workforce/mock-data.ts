...
const CLINICS = Array.from({ length: 50 }, (_, i) => ({
  id: `cln-${String(i + 1).padStart(3, '0')}`,
  name: `Clinic ${i + 1}`,
  parent: FACILITIES[i % 20]!.id,
}));
...
export const MOCK_EMPLOYEES: Employee[] = Array.from(
  ...
  (_, i) => {
    ...
    const empType = (
      ['full_time', 'part_time', 'contract', 'agency', 'volunteer'] as const
    )[i % 5]!;
    const first = [
      'James',
      'Maria',
    ...
    return {
      ...
      providerId:
        i % 3 === 0
          ? `prov-${String((i % 500) + 1).padStart(4, '0')}`
          : undefined,
      ...
      status:
        i % 50 === 0 ? 'on_leave' : i % 200 === 0 ? 'probation' : 'active',
      hireDate: daysAgo(365 + (i % 2000)),
      managerId:
        i > 0 ? `emp-${String((i % 100) + 1).padStart(5, '0')}` : undefined,
      licenses:
        i % 4 === 0
          ? [
              {
...
export const MOCK_SHIFTS: Shift[] = Array.from({ length: 20000 }, (_, i) => {
  ...
  return {
    ...
    shiftType: (['day', 'night', 'evening', 'on_call', 'weekend'] as const)[
      i % 5
    ]!,
    status:
      dayOffset < 0
...
export const MOCK_ATTENDANCE: Attendance[] = Array.from(
  { length: 80000 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
...
export const MOCK_LEAVE_REQUESTS: LeaveRequest[] = Array.from(
  { length: 12000 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
    const days = 1 + (i % 14);
    return {
      ...
      employeeName: emp.fullName,
      leaveType: (
        ['annual', 'sick', 'maternity', 'study', 'compassionate'] as const
      )[i % 5]!,
...
export const MOCK_TRAINING: Training[] = Array.from(
  { length: 15000 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
    const st = (
      ['not_started', 'in_progress', 'completed', 'overdue'] as const
    )[i % 4]!;
    return {
...
export const MOCK_PERFORMANCE: PerformanceReview[] = Array.from(
  { length: 4000 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
    const score = 60 + (i % 40);
...
export const MOCK_ON_CALL: OnCallSchedule[] = Array.from(
  { length: 500 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
    return {
...
export const MOCK_PAYROLL: PayrollSummary[] = Array.from(
  { length: 300 },
  (_, i) => {
    const emp = MOCK_EMPLOYEES[i % MOCK_EMPLOYEES.length]!;
...
export function buildWorkforceDashboard(
  facilityId?: string,
  departmentId?: string,
): WorkforceDashboard {
  let employees = MOCK_EMPLOYEES.filter(
  ...
  const pendingLeave = MOCK_LEAVE_REQUESTS.filter(
    (l) => l.status === 'pending',
  );
  const expiringCerts = MOCK_EMPLOYEES.flatMap((e) => e.certifications).filter(
    (c) => c.status === 'expiring',
  );
