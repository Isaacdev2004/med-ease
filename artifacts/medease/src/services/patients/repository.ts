...
class PatientsRepository implements PatientsRepositoryContract {
  ...
  async listPatients(filters?: PatientFilters) {
    const dto = await this.transport.get(getPatientsControllerListPatientsUrl(), {
  ...
  async searchPatients(filters: PatientSearchFilters) {
    const dto = await this.transport.get(getPatientsControllerSearchPatientsUrl(), {
      query: filtersToQuery(filters),
    });
    return mapPaginatedDto(dto, mapPatientDto);
  }
  ...
  async getPatient(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetPatientUrl(patientId));
    return mapPatientDto(dto);
  }

  async createPatient(input: CreatePatientInput) {
    const dto = await this.transport.post(getPatientsControllerRegisterPatientUrl(), { body: input });
    return mapPatientDto(dto);
  }

  async updatePatient(patientId: string, input: UpdatePatientInput) {
    const dto = await this.transport.patch(getPatientsControllerUpdatePatientUrl(patientId), { body: input });
    return mapPatientDto(dto);
  }

  async archivePatient(patientId: string, _updatedBy: string) {
    const dto = await this.transport.delete(getPatientsControllerArchivePatientUrl(patientId));
    return mapPatientDto(dto);
  }

  async restorePatient(patientId: string, _updatedBy: string) {
    const dto = await this.transport.post(getPatientsControllerRestorePatientUrl(patientId));
    return mapPatientDto(dto);
  }

  async getIdentifiers(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetIdentifiersUrl(patientId));
    return mapArrayDto(dto, mapPatientIdentifierDto);
  }

  async getContacts(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetContactsUrl(patientId));
    return mapArrayDto(dto, mapPatientContactDto);
  }

  async getAddresses(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetAddressesUrl(patientId));
    return mapArrayDto(dto, mapPatientAddressDto);
  }

  async getEmergencyContacts(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetEmergencyContactsUrl(patientId));
    return mapArrayDto(dto, mapPatientEmergencyContactDto);
  }

  async getAllergies(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetAllergiesUrl(patientId));
    return mapArrayDto(dto, mapPatientAllergyDto);
  }

  async getPreferences(patientId: string) {
    const dto = await this.transport.get(getPatientsControllerGetPreferencesUrl(patientId));
    if (dto == null) {
      return null;
    }
  ...
  async exportPatients(input: ExportPatientsInput) {
    const dto = await this.transport.post(getPatientsControllerExportPatientsUrl(), { body: input });
