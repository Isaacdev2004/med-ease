...
export function evaluateAccess(
  ...
  const denyPolicy = policies.find((p) => p.effect === 'deny' && p.enabled && evaluateAbac(p, context));
  if (denyPolicy) return 'deny';
  if (evaluateRbac(userRoles, requiredPermission, roleNames)) return 'allow';
  const allowPolicy = policies.find((p) => p.effect === 'allow' && p.enabled && evaluateAbac(p, context));
