...
const SCALE = { users: 30, sessions: 250, logins: 800, audit: 5000, consents: 250 };
...
export const MOCK_ORGANIZATIONS: IamOrganization[] = Array.from({ length: 80 }, (_, i) => ({
  organizationId: `org-${String(i + 1).padStart(4, '0')}`,
  tenantId: MOCK_TENANTS[i % 15]!.tenantId,
  name: ['Central Hospital', 'North Clinic', 'South Medical', 'East Health', 'West Care'][i % 5]!,
  type: (['health_system', 'hospital', 'clinic', 'department'] as const)[i % 4]!,
  parentId: i % 4 === 3 ? `org-${String(Math.floor(i / 4) + 1).padStart(4, '0')}` : undefined,
  facilityCount: 1 + (i % 8),
}));

export const MOCK_IAM_USERS: IamUser[] = Array.from({ length: 400 }, (_, i) => ({
  ...
  facilityId: i % 3 === 0 ? `fac-${String((i % 25) + 1).padStart(3, '0')}` : undefined,
  status: (['active', 'active', 'active', 'inactive', 'locked', 'pending'] as const)[i % 6]!,
  roles: [['platform_admin', 'physician', 'facility_admin', 'pharmacist', 'patient'][i % 5]!],
  mfaEnabled: i % 3 !== 0,
...
export const MOCK_IAM_ROLES: IamRole[] = [
  'platform_admin', 'facility_admin', 'physician', 'pharmacist', 'nurse', 'patient',
  'transport_dispatcher', 'lab_tech', 'radiologist', 'billing_clerk', 'research_coordinator',
].map((name, i) => ({
...
export const MOCK_IAM_PERMISSIONS: IamPermission[] = Array.from({ length: 120 }, (_, i) => ({
  permissionId: `perm-${String(i + 1).padStart(4, '0')}`,
  name: ['iam.read', 'iam.write', 'patients.read', 'medications.prescribe', 'billing.read', 'executive.analytics'][i % 6]! + (i > 5 ? `.${i}` : ''),
  module: ['iam', 'clinical', 'operations', 'intelligence'][i % 4]!,
  description: `Permission for module access ${i + 1}`,
}));

export const MOCK_IAM_POLICIES: IamPolicy[] = Array.from({ length: 80 }, (_, i) => ({
  ...
  conditions: i % 3 === 0 ? ['business_hours'] : i % 5 === 0 ? ['tenant_match'] : undefined,
  tenantId: i % 4 === 0 ? MOCK_TENANTS[i % 15]!.tenantId : undefined,
  enabled: i % 10 !== 0,
}));

export const MOCK_IAM_SESSIONS: IamSession[] = Array.from({ length: 300 }, (_, i) => {
  const user = MOCK_IAM_USERS[i % 400]!;
  const status = (['active', 'active', 'expired', 'revoked'] as const)[i % 4]!;
  return {
    sessionId: `sess-${String(i + 1).padStart(5, '0')}`,
    userId: user.userId,
    deviceId: i % 2 === 0 ? `dev-${String((i % 100) + 1).padStart(4, '0')}` : undefined,
...
export const MOCK_LOGIN_HISTORY: LoginAttempt[] = Array.from({ length: 500 }, (_, i) => ({
...
export const MOCK_MFA_DEVICES: MfaDevice[] = Array.from({ length: 200 }, (_, i) => ({
  ...
  label: ['Authenticator App', 'SMS Phone', 'Email', 'YubiKey', 'Push'][i % 5]!,
  verified: i % 5 !== 0,
  registeredAt: daysAgo(60 - (i % 50)),
  lastUsedAt: i % 3 === 0 ? daysAgo(i % 7) : undefined,
}));

export const MOCK_TRUSTED_DEVICES: TrustedDevice[] = Array.from({ length: 250 }, (_, i) => ({
  deviceId: `tdev-${String(i + 1).padStart(5, '0')}`,
  userId: MOCK_IAM_USERS[i % 400]!.userId,
  name: ['MacBook Pro', 'Windows Laptop', 'iPhone', 'iPad', 'Android'][i % 5]!,
...
export const MOCK_OAUTH_CLIENTS: OAuthClient[] = Array.from({ length: 60 }, (_, i) => ({
  ...
  name: ['Mobile App', 'Partner Portal', 'Analytics Integration', 'Research API'][i % 4]!,
  tenantId: MOCK_TENANTS[i % 15]!.tenantId,
...
export const MOCK_API_KEYS: ApiKey[] = Array.from({ length: 150 }, (_, i) => ({
  ...
  status: i % 15 === 0 ? 'revoked' as const : i % 20 === 0 ? 'expired' as const : 'active' as const,
  createdAt: daysAgo(90 - (i % 60)),
  expiresAt: i % 5 === 0 ? daysAgo(-30) : undefined,
  lastUsedAt: daysAgo(i % 10),
}));

export const MOCK_CONSENTS: ConsentRecord[] = Array.from({ length: 300 }, (_, i) => {
  const status = (['active', 'active', 'revoked', 'expired'] as const)[i % 4]!;
  return {
    consentId: `consent-${String(i + 1).padStart(5, '0')}`,
    patientId: `phr-${String((i % 3000) + 1).padStart(4, '0')}`,
    granteeId: MOCK_IAM_USERS[i % 400]!.userId,
    purpose: ['Treatment', 'Research', 'Billing', 'Care Coordination'][i % 4]!,
    status,
...
export const MOCK_DELEGATIONS: DelegationRecord[] = Array.from({ length: 80 }, (_, i) => ({
...
export const MOCK_PROXY_ACCESS: ProxyAccess[] = Array.from({ length: 100 }, (_, i) => ({
...
export const MOCK_BREAK_GLASS: BreakGlassEvent[] = Array.from({ length: 50 }, (_, i) => {
  const status = (['active', 'ended', 'reviewed'] as const)[i % 3]!;
  return {
    eventId: `bg-${String(i + 1).padStart(4, '0')}`,
    userId: MOCK_IAM_USERS[i % 400]!.userId,
    patientId: i % 2 === 0 ? `phr-${String((i % 500) + 1).padStart(4, '0')}` : undefined,
...
export const MOCK_SECURITY_INCIDENTS: SecurityIncident[] = Array.from({ length: 40 }, (_, i) => {
  const status = (['open', 'investigating', 'resolved'] as const)[i % 3]!;
  return {
    incidentId: `inc-${String(i + 1).padStart(4, '0')}`,
    title: ['Failed login spike', 'Suspicious API access', 'MFA bypass attempt', 'Policy violation'][i % 4]!,
    severity: (['low', 'medium', 'high', 'critical'] as const)[i % 4]!,
    category: ['authentication', 'authorization', 'api', 'compliance'][i % 4]!,
    status,
...
export const MOCK_IAM_AUDIT: IamAuditEvent[] = Array.from({ length: 500 }, (_, i) => ({
  auditId: `iamaudit-${String(i + 1).padStart(5, '0')}`,
  action: ['login', 'logout', 'role_assign', 'policy_update', 'session_revoke', 'break_glass'][i % 6]!,
...
export const MOCK_RISK_SCORES: RiskScore[] = Array.from({ length: 150 }, (_, i) => {
  const score = computeRiskScore(['new_device', 'unusual_location'].slice(0, i % 3), 0.2 + (i % 5) * 0.1);
  return {
    scoreId: `risk-${String(i + 1).padStart(4, '0')}`,
    userId: MOCK_IAM_USERS[i % 400]!.userId,
    score,
    factors: ['new_device', 'failed_logins', 'geo_anomaly'].slice(0, 1 + (i % 3)),
    level: riskLevel(score),
    assessedAt: daysAgo(i % 7),
  };
});

export const MOCK_SAML_PROVIDERS: SamlProvider[] = Array.from({ length: 10 }, (_, i) => ({
...
export const MOCK_OIDC_PROVIDERS: OidcProvider[] = Array.from({ length: 10 }, (_, i) => ({
...
export function buildIamDashboard(tenantId?: string): IamDashboard {
  ...
  const sessions = tenantId ? MOCK_IAM_SESSIONS.filter((s) => MOCK_IAM_USERS.find((u) => u.userId === s.userId)?.tenantId === tenantId) : MOCK_IAM_SESSIONS;
  const logins = MOCK_LOGIN_HISTORY;
  const incidents = MOCK_SECURITY_INCIDENTS.filter((i) => i.status !== 'resolved');
  ...
  return {
    ...
    mfaAdoptionRate: Math.round((users.filter((u) => u.mfaEnabled).length / Math.max(users.length, 1)) * 100),
    ...
    sessionTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
