...
import { AUTH_USER_PATIENT_MAP, BILLING_PATIENT_IDS } from '@/services/billing/types';
...
const PAYERS = ['AXA Santé', 'Allianz Care', 'CNAM', 'Mutuelle Générale', 'BlueCross', 'Self-Pay'];
const PATIENT_NAMES = ['Sarah Jenkins', 'Jean Dupont', 'Marie Laurent', 'Pierre Martin', 'Sophie Bernard'];
const CATEGORIES: InvoiceLineItem['category'][] = [
  'consultation', 'laboratory', 'radiology', 'medication', 'monitoring', 'telemedicine', 'procedure',
];
const INVOICE_STATUSES: PatientInvoice['status'][] = ['draft', 'issued', 'partial', 'paid', 'overdue', 'cancelled'];
const CLAIM_STATUSES: InsuranceClaim['status'][] = ['draft', 'submitted', 'pending', 'approved', 'partially_approved', 'denied', 'appealed', 'resubmitted', 'paid'];
const PAYMENT_METHODS: Payment['method'][] = ['cash', 'card', 'bank_transfer', 'insurance', 'stripe', 'paystack', 'flutterwave', 'mobile_money'];
const PAYMENT_STATUSES: Payment['status'][] = ['pending', 'processing', 'completed', 'failed', 'refunded'];
...
export const MOCK_INVOICES: PatientInvoice[] = Array.from({ length: 5000 }, (_, i) => {
  ...
  return {
    ...
    currency: 'EUR',
    status,
    paymentMethod: paidAmount > 0 ? PAYMENT_METHODS[i % PAYMENT_METHODS.length] : undefined,
    lineItems: items,
...
export const MOCK_CLAIMS: InsuranceClaim[] = Array.from({ length: 3500 }, (_, i) => {
  const patientId = BILLING_PATIENT_IDS[i % 40]!;
  ...
  const approvedAmount = status === 'approved' || status === 'paid' ? totalClaim : status === 'partially_approved' ? Math.round(totalClaim * 0.7) : status === 'denied' ? 0 : Math.round(totalClaim * 0.3);
  ...
  return {
    ...
    adjudicationDate: ['approved', 'partially_approved', 'denied', 'paid'].includes(status) ? daysAgo(i % 60) : undefined,
    denialReason: status === 'denied' ? 'Service not covered under plan' : undefined,
...
export const MOCK_PAYMENTS: Payment[] = Array.from({ length: 6000 }, (_, i) => {
  ...
  const amount = status === 'completed' ? Math.min(invoice.total, 50 + ((i * 23) % invoice.total)) : 0;
  return {
...
export const MOCK_REFUNDS: Refund[] = Array.from({ length: 500 }, (_, i) => {
  ...
  return {
    ...
    reason: i % 3 === 0 ? 'Duplicate payment' : i % 3 === 1 ? 'Service cancelled' : 'Billing error',
...
export const MOCK_POLICIES: InsurancePolicy[] = Array.from({ length: 400 }, (_, i) => {
  const patientId = BILLING_PATIENT_IDS[i % 40]!;
  return {
    policyId: `pol-${i + 1}`,
    patientId,
...
export const MOCK_RECEIPTS: Receipt[] = MOCK_PAYMENTS.filter((p) => p.status === 'completed').slice(0, 3000).map((p, i) => ({
...
export function buildDashboard(patientId?: string, providerId?: string, facilityId?: string) {
  ...
  const dailyPayments = payments.filter((p) => p.status === 'completed' && p.paidAt.slice(0, 10) === today);
  const monthlyPayments = payments.filter((p) => p.status === 'completed' && p.paidAt >= monthStart);
  ...
  const pendingClaims = claims.filter((c) => ['submitted', 'pending', 'resubmitted'].includes(c.status)).length;
  const deniedClaims = claims.filter((c) => c.status === 'denied').length;
  const refunds = MOCK_REFUNDS.filter((r) => r.status === 'completed' && (!patientId || invoices.some((i) => i.patientId === patientId))).length;
  const grossRevenue = invoices.reduce((s, i) => s + i.total, 0);
  const collected = invoices.reduce((s, i) => s + i.paidAmount, 0);
  const collectionRate = grossRevenue > 0 ? Math.round((collected / grossRevenue) * 100) : 0;
  const netRevenue = collected - MOCK_REFUNDS.filter((r) => r.status === 'completed').reduce((s, r) => s + r.amount, 0) / 50;
...
export function buildPaymentTimeline(invoiceId: string) {
  ...
  return [
    { id: `${invoiceId}-inv`, invoiceId, date: invoice.issueDate, type: 'invoice' as const, title: 'Invoice issued', amount: invoice.total, status: invoice.status },
    ...payments.map((p) => ({ id: p.paymentId, invoiceId, date: p.paidAt, type: 'payment' as const, title: `Payment via ${p.method}`, amount: p.amount, status: p.status })),
    ...claims.map((c) => ({ id: c.claimId, invoiceId, date: c.submissionDate ?? c.createdAt, type: 'claim' as const, title: `Claim to ${c.payer}`, amount: c.totalClaim, status: c.status })),
    ...refunds.map((r) => ({ id: r.refundId, invoiceId, date: r.processedAt ?? r.createdAt, type: 'refund' as const, title: 'Refund processed', amount: -r.amount, status: r.status })),
