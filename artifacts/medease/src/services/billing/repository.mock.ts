...
class BillingMockRepository {
  ...
  async submitClaim(input: SubmitClaimInput, patientName: string) {
  ...
  async recordPayment(input: RecordPaymentInput) {
    const invoice =
      this.invoices.find((i) => i.invoiceId === input.invoiceId) ?? null;
  ...
  async refundPayment(input: RefundPaymentInput) {
    ...
    const invoice =
      this.invoices.find((i) => i.invoiceId === payment.invoiceId) ?? null;
    if (invoice) {
  ...
  async getRefunds(filters?: BillingFilters) {
    ...
    if (filters?.patientId) {
      items = items.filter((r) => {
        const inv = this.invoices.find((i) => i.invoiceId === r.invoiceId);
  ...
  async downloadInvoice(invoiceId: string) {
    const inv = this.invoices.find((i) => i.invoiceId === invoiceId) ?? null;
    if (!inv) return null;
    return {
...
export const billingMockRepository = new BillingMockRepository();
