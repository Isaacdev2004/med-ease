...
export function computeFinanceAnalytics(facilityId?: string): FinanceAnalytics {
  ...
  const budgets = facilityId ? MOCK_BUDGETS.filter((b) => b.facilityId === facilityId) : MOCK_BUDGETS;
  const cash = facilityId ? MOCK_CASH_ACCOUNTS.filter((c) => c.facilityId === facilityId) : MOCK_CASH_ACCOUNTS;
  const banks = facilityId ? MOCK_BANK_ACCOUNTS.filter((b) => b.facilityId === facilityId) : MOCK_BANK_ACCOUNTS;
  ...
  return {
    ...
    revenueTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({ label, value: Math.round(revenue / 6 + i * 12000) })),
    expenseTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({ label, value: Math.round(expenses / 6 + i * 8000) })),
    departmentProfitability: budgets.slice(0, 8).map((b) => ({ label: b.name.slice(0, 20), value: b.variance })),
    costCenterExpenses: budgets.slice(0, 6).map((b) => ({ label: b.costCenterId ?? 'General', value: b.spent })),
  };
}
