...
export function calculateARAging(
  ...
  return buckets.map(({ bucket, min, max }) => {
    const items = receivables.filter(
      (r) => r.agingDays >= min && r.agingDays <= max && r.outstanding > 0,
    );
    return {
      bucket,
      amount: items.reduce((s, r) => s + r.outstanding, 0),
      count: items.length,
    };
  });
}
...
export function badDebtProvision(
  receivables: CustomerReceivable[],
  thresholdDays = 90,
): number {
  return receivables
    .filter((r) => r.agingDays > thresholdDays)
    .reduce((s, r) => s + r.outstanding * 0.5, 0);
}
