...
export function threeWayMatch(bill: VendorBill): { matched: boolean; issues: string[] } {
  const issues: string[] = [];
...
export function calculateAPAging(bills: VendorBill[]): { bucket: string; amount: number; count: number }[] {
  const buckets = [
  ...
  return buckets.map(({ bucket, min, max }) => {
    const items = bills.filter((b) => b.agingDays >= min && b.agingDays <= max && b.status !== 'paid');
    return { bucket, amount: items.reduce((s, b) => s + b.totalAmount, 0), count: items.length };
  });
}
...
export function paymentRunTotal(bills: VendorBill[], maxDate?: string): number {
  ...
  return bills.filter((b) => b.status === 'open' && b.dueDate <= cutoff).reduce((s, b) => s + b.totalAmount, 0);
}
