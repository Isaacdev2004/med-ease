export function estimateStorageGb(documentCount: number, avgSizeBytes: number): number {
  return Math.round((documentCount * avgSizeBytes) / (1024 ** 3) * 100) / 100;
}

export function storagePath(tenantId: string, documentId: string, version: number): string {
