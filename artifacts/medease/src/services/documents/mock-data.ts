...
const MODULES = ['clinical', 'radiology', 'laboratory', 'billing', 'finance', 'workforce', 'quality', 'research', 'procurement', 'facilities'];
...
export const MOCK_TAGS: DocumentTag[] = ['urgent', 'signed', 'confidential', 'archived', 'review', 'patient', 'contract', 'invoice'].map((name, i) => ({
  tagId: `tag-${String(i + 1).padStart(3, '0')}`,
  name,
  color: ['#ef4444', '#22c55e', '#3b82f6', '#a855f7', '#f59e0b', '#06b6d4', '#64748b', '#ec4899'][i]!,
}));

export const MOCK_FOLDERS: DocumentFolder[] = Array.from({ length: SCALE.folders }, (_, i) => ({
  folderId: `folder-${String(i + 1).padStart(4, '0')}`,
  name: ['Clinical Records', 'Lab Reports', 'Invoices', 'Contracts', 'SOPs', 'HR Files', 'Imaging', 'Research'][i % 8]!,
  parentId: i % 5 === 0 ? undefined : `folder-${String(Math.floor(i / 5) + 1).padStart(4, '0')}`,
  tenantId: 'tenant-001',
...
export const MOCK_DOCUMENTS: Document[] = Array.from({ length: SCALE.documents }, (_, i) => {
  ...
  return {
    ...
    type,
    status: (['active', 'active', 'active', 'draft', 'archived', 'legal_hold'] as const)[i % 6]!,
    folderId: MOCK_FOLDERS[i % MOCK_FOLDERS.length]!.folderId,
    categoryId: MOCK_CATEGORIES[i % MOCK_CATEGORIES.length]!.categoryId,
    tenantId: 'tenant-001',
    facilityId: i % 4 === 0 ? `fac-${String((i % 10) + 1).padStart(3, '0')}` : undefined,
    patientId: module === 'clinical' && i % 3 === 0 ? `pat-${String((i % 50) + 1).padStart(4, '0')}` : undefined,
    module,
    tags: [MOCK_TAGS[i % MOCK_TAGS.length]!.name, MOCK_TAGS[(i + 2) % MOCK_TAGS.length]!.name],
    ...
    retentionPolicyId: i % 5 === 0 ? `ret-${String((i % 8) + 1).padStart(3, '0')}` : undefined,
    legalHoldId: i % 40 === 0 ? `hold-${String((i % 5) + 1).padStart(3, '0')}` : undefined,
  };
});

export const MOCK_VERSIONS: DocumentVersion[] = Array.from({ length: SCALE.versions }, (_, i) => ({
...
export const MOCK_METADATA: DocumentMetadata[] = Array.from({ length: 200 }, (_, i) => ({
...
export const MOCK_TEMPLATES: DocumentTemplate[] = Array.from({ length: SCALE.templates }, (_, i) => ({
  templateId: `tpl-${String(i + 1).padStart(4, '0')}`,
  name: ['Consent Form', 'Referral Letter', 'Discharge Summary', 'Invoice', 'SOP Template', 'CAPA Form'][i % 6]!,
  categoryId: MOCK_CATEGORIES[i % MOCK_CATEGORIES.length]!.categoryId,
...
export const MOCK_SCAN_JOBS: ScanJob[] = Array.from({ length: 80 }, (_, i) => ({
  ...
  documentId: i % 3 === 0 ? MOCK_DOCUMENTS[i % MOCK_DOCUMENTS.length]!.documentId : undefined,
...
export const MOCK_OCR_RESULTS: OCRResult[] = Array.from({ length: SCALE.ocr }, (_, i) => ({
...
export const MOCK_SIGNATURE_REQUESTS: SignatureRequest[] = Array.from({ length: SCALE.signatureRequests }, (_, i) => ({
...
export const MOCK_SIGNATURES: DocumentSignature[] = Array.from({ length: SCALE.signatures }, (_, i) => ({
  signatureId: `sig-${String(i + 1).padStart(4, '0')}`,
  documentId: MOCK_DOCUMENTS[i % MOCK_DOCUMENTS.length]!.documentId,
  requestId: MOCK_SIGNATURE_REQUESTS[i % MOCK_SIGNATURE_REQUESTS.length]!.requestId,
  signerId: `user-${String((i % 20) + 11).padStart(5, '0')}`,
  signedAt: daysAgo(i % 30),
  method: (['digital', 'wet', 'eid'] as const)[i % 3]!,
}));

export const MOCK_RETENTION_POLICIES: RetentionPolicy[] = Array.from({ length: 12 }, (_, i) => ({
...
export const MOCK_LEGAL_HOLDS: LegalHold[] = Array.from({ length: 8 }, (_, i) => ({
  holdId: `hold-${String(i + 1).padStart(3, '0')}`,
  name: `Legal hold ${i + 1}`,
  reason: ['Litigation', 'Audit', 'Investigation', 'Regulatory inquiry'][i % 4]!,
...
export const MOCK_RECORDS: RecordItem[] = Array.from({ length: SCALE.records }, (_, i) => ({
  ...
  retentionPolicyId: MOCK_RETENTION_POLICIES[i % MOCK_RETENTION_POLICIES.length]!.policyId,
  archivedAt: i % 4 === 0 ? daysAgo(i % 60) : undefined,
  status: (['active', 'active', 'archived', 'destroyed'] as const)[i % 4]!,
}));

export const MOCK_ARCHIVE_JOBS: ArchiveJob[] = Array.from({ length: SCALE.archiveJobs }, (_, i) => ({
...
export const MOCK_SHARED_LINKS: SharedLink[] = Array.from({ length: SCALE.sharedLinks }, (_, i) => ({
...
export const MOCK_ACCESS_LOGS: AccessLog[] = Array.from({ length: 300 }, (_, i) => ({
...
export const MOCK_CONTENT_INDEX: ContentIndex[] = Array.from({ length: 200 }, (_, i) => ({
...
export const MOCK_COMMENTS: DocumentComment[] = Array.from({ length: 60 }, (_, i) => ({
...
export function buildDocumentDashboard(tenantId?: string, facilityId?: string): DocumentDashboard {
  ...
  const avgSize = docs.reduce((s, d) => s + d.sizeBytes, 0) / Math.max(docs.length, 1);

  return {
    totalDocuments: ENTERPRISE.documents,
    totalVersions: ENTERPRISE.versions,
    totalFolders: ENTERPRISE.folders,
    pendingSignatures: MOCK_SIGNATURE_REQUESTS.filter((r) => r.status === 'pending').length * 40,
    activeLegalHolds: MOCK_LEGAL_HOLDS.filter((h) => h.status === 'active').length,
    ocrJobsToday: MOCK_SCAN_JOBS.filter((j) => j.status === 'completed').length * 15,
    ...
    uploadTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((label, i) => ({
      label,
...
export function computeDocumentAnalytics(tenantId?: string, facilityId?: string): DocumentAnalytics {
  const dashboard = buildDocumentDashboard(tenantId, facilityId);
  const docs = MOCK_DOCUMENTS;
  const avgSize = docs.reduce((s, d) => s + d.sizeBytes, 0) / Math.max(docs.length, 1);

  return {
    uploadRate: Math.round(dashboard.uploadTrend.reduce((s, t) => s + t.value, 0) / 7),
    ...
    retentionComplianceRate: retentionComplianceRate(docs, MOCK_RETENTION_POLICIES),
    searchQueriesDaily: 1840,
    archiveJobsCompleted: MOCK_ARCHIVE_JOBS.filter((j) => j.status === 'completed').length * 50,
