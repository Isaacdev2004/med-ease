...
import {
  buildForecastTrend,
  FORECAST_HORIZONS,
} from '@/services/executive/forecasting';
import {
  KPI_CATEGORIES,
  KPI_NAMES,
  computeTrend,
  kpiChangePercent,
} from '@/services/executive/kpi-engine';
import type {
  BenchmarkReport,
  CapacitySnapshot,
  DepartmentScorecard,
...
export const MOCK_ENTERPRISE_KPIS: EnterpriseKpi[] = Array.from(
  { length: 250 },
  (_, i) => {
    ...
    return {
      ...
      departmentId:
        i % 3 === 0
          ? `dept-${String((i % 12) + 1).padStart(2, '0')}`
          : undefined,
...
export const MOCK_EXECUTIVE_DASHBOARDS: ExecutiveDashboardConfig[] = Array.from(
  { length: 30 },
  (_, i) => ({
...
export const MOCK_OPERATIONAL_METRICS: OperationalMetric[] = Array.from(
  { length: 400 },
  (_, i) => {
    const value = 40 + (i % 55);
...
export const MOCK_DEPARTMENT_SCORECARDS: DepartmentScorecard[] = Array.from(
  { length: 80 },
  (_, i) => ({
...
export const MOCK_BENCHMARK_REPORTS: BenchmarkReport[] = Array.from(
  { length: 100 },
  (_, i) => {
...
export const MOCK_EXECUTIVE_FORECASTS: ExecutiveForecast[] = Array.from(
  { length: 150 },
  (_, i) => {
...
export const MOCK_STRATEGIC_INITIATIVES: StrategicInitiative[] = Array.from(
  { length: 50 },
  (_, i) => {
    const status = (
      ['planning', 'active', 'on_track', 'at_risk', 'completed'] as const
    )[i % 5]!;
    ...
    return {
      ...
      description:
        'Enterprise strategic initiative aligned with organizational goals.',
    };
  },
);

export const MOCK_ENTERPRISE_ALERTS: EnterpriseAlert[] = Array.from(
  { length: 200 },
  (_, i) => {
    ...
    return {
      ...
      facilityId: FACILITIES[i % 25]!,
      category: ['capacity', 'quality', 'financial', 'workforce', 'clinical'][
        i % 5
      ]!,
      severity,
      ...
      message:
        'Aggregated from enterprise modules requiring executive attention.',
      sourceModule: ['quality', 'finance', 'workforce', 'ai', 'publicHealth'][
        i % 5
      ]!,
...
export const MOCK_CAPACITY_SNAPSHOTS: CapacitySnapshot[] = Array.from(
  { length: 150 },
  (_, i) => {
    const total = 20 + (i % 40);
...
export const MOCK_PERFORMANCE_REPORTS: PerformanceReport[] = Array.from(
  { length: 100 },
  (_, i) => ({
...
export const MOCK_EXEC_AUDIT: import('@/services/executive/types').ExecutiveAuditLog[] =
...
export function buildExecutiveCommandCenter(
  ...
  const kpis = facilityId
    ? MOCK_ENTERPRISE_KPIS.filter((k) => k.facilityId === facilityId)
    : MOCK_ENTERPRISE_KPIS;
  const alerts = facilityId
    ? MOCK_ENTERPRISE_ALERTS.filter((a) => a.facilityId === facilityId)
    : MOCK_ENTERPRISE_ALERTS;
  const capacity = facilityId
    ? MOCK_CAPACITY_SNAPSHOTS.filter((c) => c.facilityId === facilityId)
    : MOCK_CAPACITY_SNAPSHOTS;
  const initiatives = facilityId
    ? MOCK_STRATEGIC_INITIATIVES.filter(
        (i) => !i.facilityId || i.facilityId === facilityId,
      )
  ...
  return {
    ...
    initiativesOnTrack:
      initiatives.filter(
        (i) => i.status === 'on_track' || i.status === 'active',
      ).length * 10,
    ...
    alertDistribution: [
      ...
      label,
      value:
        alerts.filter((a) => a.category === label || a.sourceModule === label)
          .length * SCALE.alerts,
    })),
