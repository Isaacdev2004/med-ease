import type { HospitalOperations, OperationalMetric } from '@/services/executive/types';

export function computeThroughputIndex(metrics: OperationalMetric[]): number {
  if (metrics.length === 0) return 0;
  const avg = metrics.reduce((s, m) => s + (m.value / Math.max(m.benchmark, 1)), 0) / metrics.length;
  return Math.round(avg * 100);
}

export function buildHospitalOperations(facilityId: string, metrics: OperationalMetric[]): HospitalOperations {
...
export function operationalEfficiency(metrics: OperationalMetric[]): number {
  if (metrics.length === 0) return 0;
  const scores = metrics.map((m) => Math.min(100, Math.round((m.benchmark / Math.max(m.value, 1)) * 100)));
