...
export function matchesStudyFilters(
  ...
  if (filters.facilityId && study.facilityId !== filters.facilityId)
    return false;
  if (filters.radiologistId && study.radiologistId !== filters.radiologistId)
    return false;
  if (
    filters.isCritical !== undefined &&
    study.isCritical !== filters.isCritical
  )
    return false;
  if (filters.q) {
    const q = filters.q.toLowerCase();
    if (
      !`${study.accessionNumber} ${study.patientName} ${study.reason} ${study.modality}`
        .toLowerCase()
        .includes(q)
    )
      return false;
