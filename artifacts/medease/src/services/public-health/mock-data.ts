...
export const MOCK_COMMUNITY_MEMBERS: CommunityMember[] = Array.from(
  { length: 500 },
  (_, i) => ({
    ...
    enrolledPrograms:
      i % 3 === 0 ? [`prog-${String((i % 50) + 1).padStart(3, '0')}`] : [],
  }),
);

export const MOCK_OUTBREAKS: OutbreakInvestigation[] = Array.from(
  { length: 40 },
  (_, i) => {
    const status = (
      ['monitoring', 'investigation', 'containment', 'resolved'] as const
    )[i % 4]!;
    return {
...
export const MOCK_DISEASE_CASES: DiseaseCase[] = Array.from(
  { length: 300 },
  (_, i) => {
    const disease = REPORTABLE_DISEASES[i % REPORTABLE_DISEASES.length]!;
    const status = (
      ['suspected', 'confirmed', 'probable', 'ruled_out', 'closed'] as const
    )[i % 5]!;
    return {
...
export const MOCK_CONTACT_TRACING: ContactTracingRecord[] = Array.from(
  { length: 200 },
  (_, i) => {
...
export const MOCK_IMMUNIZATIONS: ImmunizationRecord[] = Array.from(
  { length: 400 },
  (_, i) => {
    const vaccine = VACCINES[i % VACCINES.length]!;
    const status = (
      ['scheduled', 'administered', 'overdue', 'declined'] as const
    )[i % 4]!;
    return {
      ...
      medicationId:
        i % 2 === 0
          ? `med-${String((i % 500) + 1).padStart(4, '0')}`
          : undefined,
      appointmentId:
        i % 3 === 0
          ? `appt-${String((i % 3000) + 1).padStart(5, '0')}`
          : undefined,
    };
  },
);

export const MOCK_REGISTRIES: ImmunizationRegistry[] = Array.from(
  { length: 10 },
  (_, i) => ({
...
export const MOCK_COMMUNITY_PROGRAMS: CommunityProgram[] = Array.from(
  { length: 50 },
  (_, i) => ({
    ...
    carePlanId:
      i % 4 === 0 ? `cp-${String((i % 100) + 1).padStart(4, '0')}` : undefined,
  }),
);

export const MOCK_MATERNAL: MaternalRecord[] = Array.from(
  { length: 80 },
  (_, i) => ({
...
export const MOCK_CHILD_HEALTH: ChildHealthRecord[] = Array.from(
  { length: 150 },
  (_, i) => ({
    ...
    facilityId: FACILITIES[i % 25]!,
    ageMonths: 1 + (i % 216),
    wellnessStatus: (['on_track', 'at_risk', 'referral_needed'] as const)[
      i % 3
    ]!,
...
export const MOCK_SCHOOL_SCREENINGS: SchoolHealthScreening[] = Array.from(
  { length: 100 },
  (_, i) => ({
...
export const MOCK_OCCUPATIONAL: OccupationalAssessment[] = Array.from(
  { length: 50 },
  (_, i) => ({
...
export const MOCK_ENVIRONMENTAL: EnvironmentalInspection[] = Array.from(
  { length: 60 },
  (_, i) => ({
    ...
    status: (['scheduled', 'passed', 'failed', 'follow_up_required'] as const)[
      i % 4
    ]!,
...
export const MOCK_SDOH: SdohAssessment[] = Array.from(
  { length: 200 },
  (_, i) => {
    const score = 10 + (i % 90);
    return {
...
export function buildPublicHealthDashboard(
  facilityId?: string,
): PublicHealthDashboard {
  const cases = facilityId
    ? MOCK_DISEASE_CASES.filter((c) => c.facilityId === facilityId)
    : MOCK_DISEASE_CASES;
  const outbreaks = facilityId
    ? MOCK_OUTBREAKS.filter((o) => o.facilityId === facilityId)
    : MOCK_OUTBREAKS;
  ...
  const diseaseCounts = REPORTABLE_DISEASES.map((d) => ({
    label: d,
    value:
      cases.filter((c) => c.disease === d && c.status !== 'closed').length *
      SCALE.cases,
  }))
  ...
  return {
    ...
    activeOutbreaks:
      outbreaks.filter((o) => o.status !== 'resolved').length * 20,
    immunizationsDue:
      immunizations.filter(
        (i) => i.status === 'overdue' || i.status === 'scheduled',
      ).length * SCALE.immunizations,
    contactsMonitoring:
      contacts.filter(
        (c) => c.status === 'monitoring' || c.status === 'quarantined',
      ).length * SCALE.contacts,
    communityProgramsActive:
      programs.filter((p) => p.status === 'active').length * 10,
