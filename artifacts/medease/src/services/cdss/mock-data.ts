...
const PROVIDERS = Array.from(
  { length: 200 },
  (_, i) => `prov-${String(i + 1).padStart(4, '0')}`,
);
...
const CALC_TYPES: CalculatorType[] = [
  'chads_vasc',
  'has_bled',
  'wells',
  'curb65',
...
export const MOCK_CLINICAL_RULES: ClinicalRule[] = Array.from(
  { length: 1500 },
  (_, i) => ({
...
export const MOCK_GUIDELINES: Guideline[] = Array.from(
  { length: 800 },
  (_, i) => {
    ...
    return {
      ...
      source,
      condition,
      status: (['published', 'published', 'draft', 'archived'] as const)[
        i % 4
      ]!,
      ...
      versions: [
        {
          versionId: `gv-${i}`,
          version: `v${1 + (i % 5)}.0`,
          publishedAt: daysAgo(i % 365),
          summary: `Evidence-based ${condition} management`,
        },
      ],
...
export const MOCK_ALERTS: ClinicalAlert[] = Array.from(
  { length: 2000 },
  (_, i) => ({
    ...
    relatedEntityId:
      i % 2 === 0
        ? `lab-${String((i % 1000) + 1).padStart(4, '0')}`
...
export const MOCK_ORDER_SETS: OrderSet[] = Array.from(
  { length: 500 },
  (_, i) => {
    ...
    return {
      ...
      items: template.map((label, j) => ({
        itemId: `osi-${i}-${j}`,
        type: (['medication', 'lab', 'imaging', 'nursing', 'consult'] as const)[
          j % 5
        ]!,
        label,
      })),
...
export const MOCK_DIAGNOSTIC_SUGGESTIONS: DiagnosticSuggestion[] = Array.from(
  { length: 300 },
  (_, i) => ({
...
export const MOCK_PATHWAYS: ClinicalPathway[] = Array.from(
  { length: 200 },
  (_, i) => ({
...
export const MOCK_EVIDENCE: EvidenceArticle[] = Array.from(
  { length: 150 },
  (_, i) => ({
...
export const MOCK_PREVENTIVE: PreventiveReminder[] = Array.from(
  { length: 250 },
  (_, i) => ({
...
export const MOCK_DRUG_INTERACTIONS: DrugInteractionAlert[] = Array.from(
  { length: 400 },
  (_, i) => ({
...
export const MOCK_ALLERGY_ALERTS: AllergyAlert[] = Array.from(
  { length: 200 },
  (_, i) => ({
...
export const MOCK_CONTRAINDICATIONS: ContraindicationAlert[] = Array.from(
  { length: 150 },
  (_, i) => ({
...
export const MOCK_DUPLICATE_THERAPY: DuplicateTherapyAlert[] = Array.from(
  { length: 100 },
  (_, i) => ({
...
export const MOCK_PROTOCOLS: ClinicalProtocol[] = Array.from(
  { length: 120 },
  (_, i) => ({
...
export const MOCK_DECISION_TREES: DecisionTree[] = Array.from(
  { length: 20 },
  (_, i) => ({
    ...
    nodes: [
      {
        nodeId: 'n1',
        question: 'Is patient hemodynamically stable?',
        yesBranch: 'n2',
        noBranch: 'n3',
      },
      {
        nodeId: 'n2',
        question: 'Are diagnostics complete?',
        yesBranch: 'n4',
        noBranch: 'n5',
      },
      {
        nodeId: 'n3',
        recommendation: 'Activate emergency protocol and ICU consult',
      },
...
export const MOCK_AUDIT: AlertAudit[] = Array.from({ length: 500 }, (_, i) => ({
  ...
  action: (['triggered', 'acknowledged', 'overridden', 'resolved'] as const)[
    i % 4
  ]!,
  providerId: PROVIDERS[i % 200],
...
export function buildCdssDashboard(facilityId?: string): CdssDashboard {
  const alerts = facilityId
    ? MOCK_ALERTS.filter((a) => a.facilityId === facilityId)
    : MOCK_ALERTS;
  const recs = facilityId
    ? MOCK_RECOMMENDATIONS.filter((r) => r.facilityId === facilityId)
    : MOCK_RECOMMENDATIONS;
  const orderSets = facilityId
    ? MOCK_ORDER_SETS.filter(
        (o) => !o.facilityId || o.facilityId === facilityId,
      )
    : MOCK_ORDER_SETS;
  const preventive = facilityId
  ...
  return {
    ...
    guidelineCompliance: Math.round(
      MOCK_GUIDELINES.reduce((s, g) => s + g.complianceRate, 0) /
        MOCK_GUIDELINES.length,
    ),
