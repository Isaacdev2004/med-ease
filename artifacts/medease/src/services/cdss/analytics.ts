...
export function computeCdssAnalytics(facilityId?: string): CdssAnalytics {
  ...
  const recs = facilityId
    ...
    : MOCK_RECOMMENDATIONS;
  const guidelines = facilityId
    ? MOCK_GUIDELINES.filter(
        (g) => !g.facilityId || g.facilityId === facilityId,
      )
    : MOCK_GUIDELINES;
  ...
  const byType = [
    ...
    value:
      alerts.filter((a) => a.type === label).length ||
      Math.round(alerts.length / 10),
  }));
  ...
  return {
    ...
    acceptanceRate: Math.round(
      (recs.filter((r) => r.status === 'accepted').length /
        Math.max(recs.length, 1)) *
        100,
    ),
    overrideRate: Math.round(
      (alerts.filter((a) => a.status === 'overridden').length /
        Math.max(alerts.length, 1)) *
        100,
    ),
    guidelineCompliance: dashboard.guidelineCompliance,
    orderSetUsage: dashboard.orderSetsApplied,
    alertTrend: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'].map((label, i) => ({
      label,
      value: Math.round(alerts.length / 8 + i * 40),
    })),
    alertsByType: byType,
    ...
    topGuidelines: guidelines
      .slice(0, 6)
      .map((g) => ({ label: g.title.slice(0, 30), value: g.complianceRate })),
  };
}
