...
import { AUTH_USER_PATIENT_MAP, TELEMEDICINE_PATIENT_IDS } from '@/services/telemedicine/types';
...
const PLATFORMS: VideoPlatform[] = ['webrtc', 'twilio', 'agora', 'daily', 'zoom', 'teams'];
const SESSION_TYPES: SessionType[] = ['consultation', 'follow_up', 'urgent', 'specialist', 'group'];
const STATUSES: SessionStatus[] = ['scheduled', 'waiting', 'in_progress', 'completed', 'cancelled', 'no_show'];
...
export const MOCK_SESSIONS: TelemedicineSession[] = Array.from({ length: 500 }, (_, i) => {
  ...
  return {
    ...
    scheduledEnd: end,
    actualStart: status === 'completed' || status === 'in_progress' ? start : undefined,
    ...
    status,
    recordingStatus: i % 4 === 0 ? 'available' : i % 7 === 0 ? 'recording' : 'none',
...
export const MOCK_MESSAGES: ChatMessage[] = Array.from({ length: 3000 }, (_, i) => {
  ...
  return {
    ...
    reactions: i % 20 === 0 ? [{ emoji: '👍', userId: session.clinicianId }] : undefined,
    pinned: i % 100 === 0,
  };
});

export const MOCK_ATTACHMENTS: SessionAttachment[] = Array.from({ length: 600 }, (_, i) => {
  const session = MOCK_SESSIONS[i % MOCK_SESSIONS.length]!;
  const types = ['image', 'pdf', 'lab', 'radiology', 'prescription', 'consent', 'document'] as const;
...
export const MOCK_RECORDINGS: SessionRecording[] = Array.from({ length: 200 }, (_, i) => {
  const session = MOCK_SESSIONS.filter((s) => s.recordingStatus !== 'none')[i % 100] ?? MOCK_SESSIONS[i]!;
  return {
    id: `rec-${String(i + 1).padStart(4, '0')}`,
    sessionId: session.sessionId,
    consentGiven: true,
    status: session.recordingStatus === 'recording' ? 'recording' : 'available',
    durationSeconds: 900 + (i % 1800),
    storageUrl: `/mock/recordings/${session.sessionId}.mp4`,
    retentionDays: 90,
    transcription: i % 3 === 0 ? 'Patient discussed symptoms and treatment plan...' : undefined,
    startedAt: session.actualStart,
    stoppedAt: session.actualEnd,
  };
});

export const MOCK_WAITING_ROOM: WaitingRoomEntry[] = Array.from({ length: 150 }, (_, i) => {
  const session = MOCK_SESSIONS.filter((s) => s.status === 'waiting' || s.status === 'scheduled')[i % 50] ?? MOCK_SESSIONS[i]!;
  return {
    id: `wr-${String(i + 1).padStart(4, '0')}`,
    sessionId: session.sessionId,
    patientId: session.patientId,
    patientName: session.patientName,
    status: (['waiting', 'admitted', 'rejected', 'left'] as const)[i % 8 === 0 ? 2 : i % 6 === 0 ? 1 : 0]!,
...
export const MOCK_WHITEBOARDS: WhiteboardSession[] = Array.from({ length: 100 }, (_, i) => ({
...
export const MOCK_PARTICIPANTS: VideoParticipant[] = MOCK_SESSIONS.slice(0, 300).flatMap((s, i) => [
  {
    id: `part-${i}-p`,
...
export const MOCK_CLINICAL_NOTES: ClinicalNote[] = MOCK_SESSIONS.filter((s) => s.status === 'completed').slice(0, 200).map((s, i) => ({
...
export const MOCK_PROVIDER_AVAILABILITY: ProviderAvailability[] = CLINICIANS.map((c, i) => ({
  providerId: c.id,
  providerName: c.name,
  specialty: c.specialty,
  availableSlots: Array.from({ length: 5 }, (_, j) => ({
    start: daysFromNow(j + 1 + i),
    end: new Date(new Date(daysFromNow(j + 1 + i)).getTime() + 30 * 60 * 1000).toISOString(),
  })),
...
export function buildDashboard(patientId?: string, clinicianId?: string) {
  ...
  if (clinicianId) sessions = sessions.filter((s) => s.clinicianId === clinicianId);
  const now = new Date();
  const upcoming = sessions.filter((s) => s.status === 'scheduled' && new Date(s.scheduledStart) >= now);
  const active = sessions.filter((s) => s.status === 'in_progress' || s.status === 'waiting');
  const completedToday = sessions.filter((s) => s.status === 'completed' && s.actualEnd?.slice(0, 10) === now.toISOString().slice(0, 10));
  const waiting = MOCK_WAITING_ROOM.filter((w) => w.status === 'waiting' && sessions.some((s) => s.sessionId === w.sessionId));
  ...
  return {
    ...
    averageQuality: Math.round(sessions.reduce((sum, s) => sum + (s.qualityScore ?? 80), 0) / Math.max(sessions.length, 1)),
...
export function buildTimeline(sessionId: string): SessionTimelineEntry[] {
  ...
  if (!session) return [];
  const messages = MOCK_MESSAGES.filter((m) => m.sessionId === sessionId).slice(0, 10);
  const notes = MOCK_CLINICAL_NOTES.filter((n) => n.sessionId === sessionId);
  const files = MOCK_ATTACHMENTS.filter((a) => a.sessionId === sessionId).slice(0, 5);
  const entries: SessionTimelineEntry[] = [
    { id: `${sessionId}-sched`, sessionId, date: session.scheduledStart, type: 'join', title: 'Session scheduled', description: session.specialty },
    ...(session.actualStart ? [{ id: `${sessionId}-start`, sessionId, date: session.actualStart, type: 'join' as const, title: 'Session started', description: 'Video consultation began', actor: session.clinicianName }] : []),
    ...messages.map((m) => ({ id: m.id, sessionId, date: m.sentAt, type: 'message' as const, title: 'Chat message', description: m.content.slice(0, 80), actor: m.senderName })),
    ...files.map((f) => ({ id: f.id, sessionId, date: f.uploadedAt, type: 'file' as const, title: 'File uploaded', description: f.name })),
    ...notes.map((n) => ({ id: n.id, sessionId, date: n.updatedAt, type: 'note' as const, title: 'Clinical note saved', description: n.assessment.slice(0, 80) })),
  ];
