...
} from '@/services/reporting/schedule-engine';
...
export const MOCK_REPORT_DEFINITIONS: ReportDefinition[] = Array.from(
...
export const MOCK_REPORT_INSTANCES: ReportInstance[] = Array.from(
...
export const MOCK_REPORT_TEMPLATES: ReportTemplate[] = REPORT_NAMES.map(
  (name, i) => ({
...
export const MOCK_REPORT_SCHEDULES: ReportSchedule[] = Array.from(
  { length: SCALE.schedules },
  (_, i) => ({
    scheduleId: `rps-${String(i + 1).padStart(3, '0')}`,
    reportId:
      MOCK_REPORT_DEFINITIONS[i % MOCK_REPORT_DEFINITIONS.length]!.reportId,
    name: `Schedule ${i + 1}`,
...
export const MOCK_REPORT_EXPORTS: ReportExport[] = Array.from(
  { length: SCALE.exports },
  (_, i) => {
    const def = MOCK_REPORT_DEFINITIONS[i % MOCK_REPORT_DEFINITIONS.length]!;
    const format = (['pdf', 'xlsx', 'csv'] as const)[i % 3]!;
    const recordCount = 50 + (i % 200) * 5;
    const status = (
      ['completed', 'completed', 'processing', 'queued', 'failed'] as const
    )[i % 5]!;
    return {
      ...
      fileSizeKb:
        status === 'completed'
          ? estimateFileSizeKb(recordCount, format)
          : undefined,
    };
  },
);

export const MOCK_REPORT_DESIGNERS: ReportDesigner[] =
  MOCK_REPORT_DEFINITIONS.slice(0, 30).map((d, i) => ({
...
export const MOCK_REPORT_FIELDS: ReportField[] = Array.from(
  { length: 60 },
  (_, i) => ({
...
export const MOCK_REPORT_CHARTS: ReportChart[] = Array.from(
  { length: 40 },
  (_, i) => ({
...
export const MOCK_REPORT_DATA_SOURCES: ReportDataSource[] = Array.from(
  { length: 35 },
  (_, i) => ({
...
export const MOCK_COMPLIANCE_REPORTS: ComplianceReport[] = Array.from(
  { length: 24 },
  (_, i) => ({
...
export const MOCK_REPORT_AUDITS: ReportAudit[] = Array.from(
  { length: SCALE.audits },
  (_, i) => ({
...
export function buildReportDashboard(facilityId?: string): ReportDashboard {
  ...
  return {
    ...
    generationTrend: ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(
      (label, i) => ({
        label,
        value: 120 + i * 20,
      }),
