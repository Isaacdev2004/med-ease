...
import {
  computeHealthScore,
  systemUptimePercent,
} from '@/services/platform-admin/health-engine';
import {
  jobSuccessRate,
  queueDepth,
  runningJobCount,
} from '@/services/platform-admin/job-engine';
import {
  activeTenantCount,
  tenantGrowthRate,
} from '@/services/platform-admin/tenant-engine';
import type {
  BackupJob,
  BackgroundWorker,
  BrandingConfig,
...
export const MOCK_TENANTS: Tenant[] = Array.from(
  { length: SCALE.tenants },
  (_, i) => ({
    tenantId: `ten-${String(i + 1).padStart(4, '0')}`,
    name: `Health System ${i + 1}`,
    slug: `health-system-${i + 1}`,
    status: (['active', 'active', 'trial', 'suspended', 'archived'] as const)[
      i % 5
    ]!,
...
export const MOCK_HOSPITALS: Hospital[] = Array.from(
  { length: SCALE.hospitals },
  (_, i) => ({
...
export const MOCK_FACILITIES: FacilityConfig[] = Array.from(
  { length: SCALE.facilities },
  (_, i) => ({
...
export const MOCK_DEPARTMENTS: Department[] = Array.from(
  { length: SCALE.departments },
  (_, i) => ({
    departmentId: `dept-${String(i + 1).padStart(4, '0')}`,
    facilityId: MOCK_FACILITIES[i % MOCK_FACILITIES.length]!.facilityId,
    name: ['Emergency', 'Cardiology', 'Radiology', 'Pharmacy', 'Laboratory'][
      i % 5
    ]!,
    code: `D-${String(i + 1).padStart(3, '0')}`,
    specialty: ['Emergency Medicine', 'Cardiology', 'Diagnostic Imaging'][
      i % 3
    ],
...
export const MOCK_LOCALIZATION: Localization[] = MOCK_TENANTS.slice(0, 15).map(
  (t, i) => ({
...
export const MOCK_EMAIL_SERVERS: EmailServerConfig[] = MOCK_TENANTS.slice(
  0,
  10,
).map((t, i) => ({
...
export const MOCK_SMS_SERVERS: SmsServerConfig[] = MOCK_TENANTS.slice(
  0,
  10,
).map((t, i) => ({
...
export const MOCK_BRANDING: BrandingConfig[] = MOCK_TENANTS.slice(0, 15).map(
  (t, i) => ({
...
export const MOCK_STORAGE: StorageConfig[] = MOCK_TENANTS.slice(0, 20).map(
  (t, i) => ({
...
export const MOCK_FEATURE_FLAGS: FeatureFlagConfig[] = [
  ...
  },
  {
    ...
    key: 'telemedicine',
    label: 'Telemedicine',
    description: 'Virtual visit capabilities',
    enabled: true,
    scope: 'tenant',
    updatedAt: daysAgo(2),
  },
  ...
  },
  {
    ...
    key: 'whiteLabel',
    label: 'White Label',
    description: 'Custom branding per tenant',
    enabled: true,
    scope: 'tenant',
    updatedAt: daysAgo(7),
  },
  ...
  },
  {
    ...
    description: 'Clinical research tools',
    enabled: true,
    scope: 'facility',
...
export const MOCK_SYSTEM_JOBS: SystemJob[] = Array.from(
  { length: SCALE.jobs },
  (_, i) => ({
...
export const MOCK_WORKERS: BackgroundWorker[] = Array.from(
  { length: 12 },
  (_, i) => ({
...
export const MOCK_BACKUPS: BackupJob[] = Array.from({ length: 30 }, (_, i) => ({
  ...
  status: (['completed', 'completed', 'running', 'failed', 'pending'] as const)[
    i % 5
  ]!,
  sizeGb: 50 + i * 10,
...
export const MOCK_MAINTENANCE: MaintenanceMode[] = Array.from(
  { length: 8 },
  (_, i) => ({
...
export const MOCK_PLATFORM_AUDITS: PlatformAudit[] = Array.from(
  { length: SCALE.audits },
  (_, i) => ({
    auditId: `pa-${String(i + 1).padStart(5, '0')}`,
    tenantId:
      i % 3 === 0 ? MOCK_TENANTS[i % MOCK_TENANTS.length]!.tenantId : undefined,
    action: [
      'tenant.create',
      'tenant.suspend',
      'branding.update',
    ...
    resource: ['tenant', 'branding', 'feature-flag', 'backup', 'maintenance'][
      i % 5
    ]!,
...
export function buildPlatformDashboard(tenantId?: string): PlatformDashboard {
  ...
  return {
    ...
    failedJobs:
      MOCK_SYSTEM_JOBS.filter((j) => j.status === 'failed').length * 3,
...
export function computePlatformAnalytics(tenantId?: string): PlatformAnalytics {
  ...
  return {
    ...
    avgFacilitiesPerTenant:
      Math.round((MOCK_FACILITIES.length / tenants.length) * 10) / 10,
    licenseUtilizationRate:
      totalSeats > 0 ? Math.round((usedSeats / totalSeats) * 100) : 0,
    storageUtilizationRate:
      capacityGb > 0 ? Math.round((usedGb / capacityGb) * 100) : 0,
