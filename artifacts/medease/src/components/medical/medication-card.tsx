  ...
  return (
    <Card className={cn("overflow-hidden hover-elevate transition-all duration-300", className)}>
      <div className={cn(
        ...
        status === "paused" ? "bg-warning" : "bg-muted"
      )} />
          ...
          <div className="flex items-start space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-2 mb-1">
                ...
                <Badge variant={status === "active" ? "default" : "secondary"} className="text-xs h-5 px-1.5 uppercase">
                  {status}
                </Badge>
              </div>
              <p className="text-sm font-medium text-foreground mb-1">{dosage}</p>
          ...
          <div className="mt-5 pt-4 border-t flex items-center justify-between text-sm">
            ...
            {refillsRemaining !== undefined && (
              <div className={cn(
                ...
                refillsRemaining === 0 ? "text-destructive" : "text-muted-foreground"
              )}>
                {refillsRemaining === 0 && <AlertCircle className="mr-1 h-3.5 w-3.5" />}
