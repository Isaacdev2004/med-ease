  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(equipment.data?.items ?? []).slice(0, 12).map((e) => (
          <EquipmentCard key={e.equipmentId} equipment={e} />
        ))}
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(biomed.data?.items ?? []).slice(0, 6).map((d) => (
    ...
    <div className="space-y-6">
              ...
              wo.status === 'open'
                ? () =>
                    assignWorkOrder.mutate({
                      ...
                      technicianId: 'emp-00100',
                    })
                : undefined
            }
            onComplete={
              wo.status === 'in_progress' || wo.status === 'assigned'
                ? () => completeWorkOrder.mutate(wo.workOrderId)
                : undefined
            }
          />
        ))}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(preventive.data?.items ?? []).slice(0, 6).map((p) => (
          <PreventiveMaintenanceCard key={p.scheduleId} schedule={p} />
        ))}
      </div>
    </div>
  );
...
export function BuildingsSection({ filters }: { filters?: FacilitiesFilters }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(rooms.data?.items ?? []).slice(0, 9).map((r) => (
          <RoomCard key={r.roomId} room={r} />
        ))}
      </div>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {(beds.data?.items ?? []).slice(0, 8).map((b) => (
...
export function EnvironmentSection({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(env.data?.items ?? []).slice(0, 12).map((r) => (
...
export function HousekeepingSection({
  ...
  const cleaning = (rooms.data?.items ?? []).filter(
    (r) => r.status === 'cleaning' || r.status === 'maintenance',
  );
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {cleaning.slice(0, 12).map((r) => (
        <RoomCard key={r.roomId} room={r} />
      ))}
    </div>
  );
...
export function FleetSection({ filters }: { filters?: FacilitiesFilters }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(vehicles.data?.items ?? []).slice(0, 12).map((v) => (
...
export function CalibrationSection({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(calibration.data?.items ?? []).slice(0, 12).map((c) => (
...
export function VendorsSection() {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {(vendors.data?.items ?? []).slice(0, 12).map((v) => (
...
export function ContractsSection({ filters }: { filters?: FacilitiesFilters }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(contracts.data?.items ?? []).slice(0, 12).map((c) => (
...
export function SystemHealthSection({
  ...
  if (utilities.isLoading) return <LoadingView />;
  const alerts = [
    ...(utilities.data?.items ?? []).filter((u) => u.status !== 'normal'),
    ...(env.data?.items ?? []).filter((e) => e.status !== 'normal').slice(0, 6),
  ];
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {alerts
          ...
          .map((a) =>
            'utilityId' in a ? (
              <UtilityCard key={a.utilityId} utility={a} />
            ) : (
              <EnvironmentalCard key={a.readingId} reading={a} />
            ),
          )}
      </div>
    </div>
...
export function FacilitiesSectionContent({
  section,
  filters,
}: {
  ...
  switch (section) {
    ...
    case 'maintenance':
      return <MaintenanceSection filters={filters} />;
    case 'buildings':
      return <BuildingsSection filters={filters} />;
    case 'assets':
      return <AssetsSection filters={filters} />;
    case 'utilities':
      return <UtilitiesSection filters={filters} />;
    case 'environment':
      return <EnvironmentSection filters={filters} />;
    case 'housekeeping':
      return <HousekeepingSection filters={filters} />;
    case 'fleet':
      return <FleetSection filters={filters} />;
    case 'workOrders':
      return <WorkOrdersSection filters={filters} />;
    case 'calibration':
      return <CalibrationSection filters={filters} />;
    case 'vendors':
      return <VendorsSection />;
    case 'contracts':
      return <ContractsSection filters={filters} />;
    case 'analytics':
      return <AnalyticsSection filters={filters} />;
    case 'systemHealth':
      return <SystemHealthSection filters={filters} />;
    default:
