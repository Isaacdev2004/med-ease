...
import type { ClinicalAlert, HealthScore, PatientDemographics } from '@/services/patient-records/types';
...
export function PatientBanner({ demographics, healthScore, alerts = [], className }: PatientBannerProps) {
  const initials = demographics.fullName.split(' ').map((p) => p[0]).join('').slice(0, 2).toUpperCase();
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{demographics.mrn} · {demographics.nationalId}</p>
        ...
        {healthScore ? (
          <div className="flex items-center gap-4 rounded-lg border px-4 py-3" aria-label={`Health score ${healthScore.overall}`}>
            <Heart className="h-8 w-8 text-primary" aria-hidden="true" />
            <div>
              <p className="text-2xl font-bold">{healthScore.overall}</p>
              <p className="text-xs text-muted-foreground capitalize">Health score · {healthScore.trend}</p>
            </div>
          </div>
        ) : null}
      </div>
      {activeAlerts.length > 0 ? (
        <div className="mt-4 flex flex-wrap gap-2" role="list" aria-label="Clinical alerts">
          {activeAlerts.map((alert) => (
            <Badge
              ...
              role="listitem"
            >
              {alert.severity === 'critical' ? <AlertTriangle className="h-3 w-3" /> : <Shield className="h-3 w-3" />}
              {alert.title}
            </Badge>
          ))}
        </div>
...
export function HealthStatusBanner({ message, severity = 'info' }: { message: string; severity?: 'info' | 'warning' | 'critical' }) {
  return (
    <div
      className={cn(
