        ...
        <Card>
          ...
          <CardContent>
            <ul className="list-disc pl-5 text-sm text-muted-foreground">
              {record.summary.activeTreatments.map((t) => <li key={t}>{t}</li>)}
            </ul>
          </CardContent>
        </Card>
      </div>
...
export function ProfileSection({ record }: { record: PatientHealthRecord }) {
  ...
  const rows = [
    ['Address', `${d.address.street}, ${d.address.postalCode} ${d.address.city}`],
    ['Language', d.language],
...
export function VitalsSection({ record }: { record: PatientHealthRecord }) {
  const bpData = record.vitals.slice(0, 8).reverse().map((v, i) => ({
    label: `#${i + 1}`,
    value: v.bloodPressureSystolic ?? 0,
  }));
  const glucoseData = record.vitals.filter((v) => v.bloodGlucose).slice(0, 8).reverse().map((v, i) => ({
    label: `#${i + 1}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        getRowId={(r) => r.id}
        columns={[
          { id: 'date', header: 'Date', cell: (r) => format(new Date(r.recordedAt), 'PP') },
          { id: 'bp', header: 'BP', cell: (r) => r.bloodPressureSystolic ? `${r.bloodPressureSystolic}/${r.bloodPressureDiastolic}` : '—' },
          { id: 'hr', header: 'HR', cell: (r) => r.heartRate ?? '—' },
          { id: 'temp', header: 'Temp', cell: (r) => r.temperatureC ? `${r.temperatureC.toFixed(1)}°C` : '—' },
          { id: 'spo2', header: 'SpO₂', cell: (r) => r.oxygenSaturation ? `${r.oxygenSaturation}%` : '—' },
        ]}
      />
    </div>
  );
    ...
    <DataTable
      ...
      columns={[
        ...
        { id: 'range', header: 'Reference', cell: (r) => r.referenceRange },
        {
          ...
          header: 'Flag',
          cell: (r) => (
            <Badge variant={r.flag === 'normal' ? 'secondary' : r.flag === 'critical' ? 'destructive' : 'warning'}>
              {r.flag}
            </Badge>
          ),
        },
        { id: 'date', header: 'Resulted', cell: (r) => format(new Date(r.resultedAt), 'PP') },
      ]}
    />
  );
}

export function MedicationsSection({ record }: { record: PatientHealthRecord }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {record.medications.map((med) => (
        <MedicationCard
          ...
          status={med.status === 'active' ? 'active' : med.status === 'paused' ? 'paused' : 'discontinued'}
...
export function AllergiesSection({ record }: { record: PatientHealthRecord }) {
  if (!record.allergies.length) {
    return <EmptyState icon={AlertTriangle} title="No known allergies" description="No allergies documented." />;
  }
  return (
    <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Card key={a.id}>
          <CardHeader className="pb-2">
            <div className="flex items-center justify-between">
              ...
              <Badge variant={a.severity === 'life_threatening' || a.severity === 'severe' ? 'destructive' : 'warning'}>
                {a.severity}
              </Badge>
            </div>
          </CardHeader>
...
export function ImmunizationsSection({ record }: { record: PatientHealthRecord }) {
  return (
    <DataTable
      ...
      columns={[
        ...
        { id: 'date', header: 'Date', cell: (r) => format(new Date(r.date), 'PP') },
...
export function ProceduresSection({ record }: { record: PatientHealthRecord }) {
  if (!record.procedures.length) {
    return <EmptyState title="No procedures" description="No procedures documented." />;
  }
  return (
    <div className="space-y-4">
      {record.procedures.map((p) => (
        <Card key={p.id}>
          ...
          <CardContent className="text-sm text-muted-foreground">
            <p>{p.category} · {format(new Date(p.date), 'PP')}</p>
            <p>{p.physician} — {p.facility}</p>
        ...
        <Card key={study.id}>
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base">{study.modality} — {study.bodyPart}</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="text-sm">
            <p className="text-muted-foreground mb-2">{format(new Date(study.date), 'PP')} · {study.radiologist}</p>
            <p>{study.report}</p>
  ...
  return (
    <TimelineEvent
      items={record.timeline.map((e, index) => ({
        ...
        status: (index === 0 ? 'current' : 'completed') as 'current' | 'completed',
      }))}
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      {record.documents.map((doc) => (
        <Card key={doc.id}>
          ...
          <CardContent className="text-sm text-muted-foreground">
            <p>{doc.type} · {format(new Date(doc.date), 'PP')}</p>
            <p>{doc.author}</p>
            <div className="mt-3 rounded border border-dashed p-4 text-center text-xs">PDF viewer placeholder</div>
          </CardContent>
        </Card>
      ))}
    </div>
...
export function EmergencySection({ record }: { record: PatientHealthRecord }) {
  ...
  return (
    <Card className="border-destructive/50">
      <CardHeader><CardTitle className="text-destructive">Emergency Summary</CardTitle></CardHeader>
      <CardContent className="space-y-4 text-sm">
        <p><strong>Blood group:</strong> {e.bloodGroup}</p>
        <div>
          <strong>Critical allergies:</strong>
          <ul className="list-disc pl-5">{e.criticalAllergies.map((a) => <li key={a}>{a}</li>)}</ul>
        </div>
        <div>
          <strong>Active medications:</strong>
          <ul className="list-disc pl-5">{e.activeMedications.map((m) => <li key={m}>{m}</li>)}</ul>
        </div>
        <div>
          <strong>Chronic conditions:</strong>
          <ul className="list-disc pl-5">{e.chronicConditions.map((c) => <li key={c}>{c}</li>)}</ul>
        </div>
        <p><strong>Primary physician:</strong> {e.primaryPhysician}</p>
      </CardContent>
    </Card>
  );
}
...
export function NotesSection({ record }: { record: PatientHealthRecord }) {
  if (!record.notes.length) {
    return <EmptyState title="No clinical notes" description="Clinical notes will appear here." />;
  }
  return (
    <div className="space-y-4">
        ...
        <Card key={note.id}>
          <CardHeader className="pb-2">
            ...
            <p className="text-xs text-muted-foreground">
              {note.author} · {format(new Date(note.date), 'PPp')} · {note.specialty}
            </p>
          </CardHeader>
...
export function CarePlansSection({ record }: { record: PatientHealthRecord }) {
  if (!record.carePlans.length) {
    return <EmptyState title="No care plans" description="Active care plans will appear here." />;
  }
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {record.carePlans.map((plan) => (
        <Card key={plan.id}>
          ...
          <CardContent className="space-y-2 text-sm text-muted-foreground">
            ...
            <p><strong>Interventions:</strong> {plan.interventions.join(', ')}</p>
            <p><strong>Assigned:</strong> {plan.assignedClinicians.join(', ')}</p>
            <p><strong>Review:</strong> {plan.reviewSchedule}</p>
...
export function FamilyHistorySection({ record }: { record: PatientHealthRecord }) {
  return (
    <DataTable
      ...
      columns={[
        ...
        { id: 'onset', header: 'Age of onset', cell: (r) => r.ageOfOnset ?? '—' },
      ]}
    />
  );
}
...
export function LifestyleSection({ record }: { record: PatientHealthRecord }) {
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Lifestyle Profile</CardTitle></CardHeader>
      <CardContent className="grid gap-3 sm:grid-cols-2 text-sm">
        {rows.map(([label, value]) => (
          <div key={label}>
            <p className="text-muted-foreground">{label}</p>
...
export function SocialHistorySection({ record }: { record: PatientHealthRecord }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <TimelineSection record={{ ...record, timeline: record.timeline.slice(0, 8) }} />
        </CardContent>
      </Card>
    </div>
  );
...
export function RecordSectionContent({
  ...
  switch (section) {
    case 'profile': return <ProfileSection record={record} />;
    case 'summary': return <SummarySection record={record} />;
    case 'vitals': return <VitalsSection record={record} />;
    case 'laboratory': return <LaboratorySection record={record} />;
    case 'medications': return <MedicationsSection record={record} />;
    case 'allergies': return <AllergiesSection record={record} />;
    case 'immunizations': return <ImmunizationsSection record={record} />;
    case 'procedures': return <ProceduresSection record={record} />;
    case 'radiology': return <RadiologySection record={record} />;
    case 'notes': return <NotesSection record={record} />;
    case 'care-plans': return <CarePlansSection record={record} />;
    case 'timeline': return <TimelineSection record={record} />;
    case 'documents': return <DocumentsSection record={record} />;
    case 'family-history': return <FamilyHistorySection record={record} />;
    case 'lifestyle': return <LifestyleSection record={record} />;
    case 'social-history': return <SocialHistorySection record={record} />;
    case 'emergency': return <EmergencySection record={record} />;
    default: return <DashboardSection record={record} />;
  }
}
