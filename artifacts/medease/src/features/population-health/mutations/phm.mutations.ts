...
function runOrQueue(label: string, execute: () => Promise<unknown>) {
  if (typeof navigator !== 'undefined' && !navigator.onLine) {
    phmOfflineQueue.enqueue({
      label,
      execute: () => execute().then(() => undefined),
    });
    appToast.offline(
      'Population health update queued until you are back online.',
    );
...
export function usePhmMutations() {
  ...
  const createCohort = useMutation({
    mutationFn: (input: CreateCohortInput) =>
      runOrQueue('Create cohort', () =>
        populationHealthService.createCohort(input),
      ),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Cohort created.' });
    },
  });
  ...
  const launchCampaign = useMutation({
    mutationFn: (input: LaunchCampaignInput) =>
      runOrQueue('Launch campaign', () =>
        populationHealthService.launchCampaign(input),
      ),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Campaign scheduled.' });
    },
  });

  const closeCareGap = useMutation({
    mutationFn: (input: CloseCareGapInput) =>
      runOrQueue('Close care gap', () =>
        populationHealthService.closeCareGap(input),
      ),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Care gap closed.' });
    },
  });

  const exportData = useMutation({
    mutationFn: (format: 'csv' | 'pdf' | 'xlsx') =>
      runOrQueue('Export PHM', () =>
        populationHealthService.exportData(format),
      ),
    onSuccess: () => {
      invalidateAll(client);
  ...
  const favorite = useMutation({
    mutationFn: ({
      ...
      runOrQueue('Favorite', () =>
        populationHealthService.favorite(userId, entityType, entityId),
      ),
