...
export function DashboardSection({ filters }: { filters?: InventoryFilters }) {
  ...
  if (!dashboard.data)
    return <EmptyState icon={Package} title="No inventory data" />;
  return (
    <div className="space-y-6">
      <InventoryMetrics dashboard={dashboard.data} />
      {dashboard.data.expiryAlerts.slice(0, 2).map((a) => (
...
export function StockSection({ filters }: { filters?: InventoryFilters }) {
  const query = useInventory({
    ...filters,
    department: filters?.department ?? 'pharmacy',
  });
  if (query.isLoading) return <LoadingView />;
  const items = query.data?.items ?? [];
  if (!items.length)
    return <EmptyState icon={Package} title="No stock items" />;
  return (
...
export function ExpirySection({ filters }: { filters?: InventoryFilters }) {
  ...
  if (!alerts.length)
    return <EmptyState icon={Package} title="No expiry alerts" />;
  return (
        ...
        <div key={o.purchaseOrderId} className="space-y-2">
          ...
          {o.status === 'pending_approval' ? (
            <button
              ...
              onClick={() =>
                void approvePurchaseOrder.mutateAsync(o.purchaseOrderId)
              }
            >
              Approve
            </button>
...
export function ReagentsSection() {
  return (
    <InventoryListSection
      filters={{ category: 'reagent', department: 'laboratory' }}
    />
  );
}
...
export function ConsumablesSection() {
  return (
    <InventoryListSection
      filters={{ category: 'consumable', department: 'laboratory' }}
    />
  );
}
...
export function WarehouseSection() {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {query.data?.slice(0, 9).map((w) => (
          <WarehouseCard key={w.warehouseId} warehouse={w} />
        ))}
      </div>
      <div className="grid gap-4 sm:grid-cols-2">
        {transfers.data?.items.slice(0, 6).map((t) => (
...
export function AnalyticsSection() {
  ...
  if (!analytics.data)
    return <EmptyState icon={Package} title="No analytics" />;
  return (
    <div className="space-y-6">
...
export function ScannerSection() {
  ...
  return (
    <BarcodeScanner onScan={(code) => void scanBarcode.mutateAsync(code)} />
  );
}
...
export function InventorySectionContent({
  section,
  filters,
}: {
  ...
  switch (section) {
    case 'stock':
      return <StockSection filters={filters} />;
    case 'expiry':
      return <ExpirySection filters={filters} />;
    case 'purchase-orders':
      return <PurchaseOrdersSection filters={filters} />;
    case 'reagents':
      return <ReagentsSection />;
    case 'consumables':
