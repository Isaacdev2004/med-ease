...
export function DashboardPanel({
  ...
  const metrics = [
    ...
    {
      label: 'Unread Inbox',
      value: dashboard.unreadInbox.toLocaleString(),
      icon: Mail,
    },
    {
    ...
    },
    {
      ...
      value: dashboard.pendingDeliveries.toLocaleString(),
      icon: Zap,
    },
    {
      label: 'Channel Health',
      value: `${dashboard.channelHealthScore}%`,
      icon: Radio,
    },
    {
      label: 'Delivery Rate',
  ...
  return (
    <div className="space-y-6">
      ...
      <BarChartPanel
        ...
        data={dashboard.channelBreakdown}
      />
    </div>
  );
}
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`font-medium truncate ${item.isRead ? 'text-muted-foreground' : ''}`}
                  >
                    {item.subject}
                  </span>
                </div>
              ...
              <div className="flex flex-col items-end gap-1 shrink-0">
                ...
                {!item.isRead && onMarkRead && (
                  <button
                    ...
                    onClick={() => onMarkRead(item.inboxId)}
                  >
                    Mark read
                  </button>
                )}
...
export function ChatPanel({
  ...
  const threadMessages = active
    ? messages.filter((m) => m.threadId === active.threadId)
    : [];
  return (
      ...
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          <span className="font-medium flex items-center gap-1">
            <Shield className="h-4 w-4" /> {message.subject}
          </span>
      ...
      <CardContent className="space-y-2">
        {broadcasts.slice(0, 8).map((b) => (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground">
              {b.audience} · {b.deliveredCount}/{b.recipientCount} delivered
            </p>
          </div>
        ))}
      </CardContent>
  ...
  return (
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          <span className="font-medium flex items-center gap-1">
            <Icon className="h-4 w-4" /> {channel.name}
          </span>
          <Badge variant={healthVariant[channel.health]} className="capitalize">
            {channel.health}
          </Badge>
        </div>
        <p className="text-xs text-muted-foreground">{channel.provider}</p>
        <p className="text-xs">
          {channel.sentToday.toLocaleString()} /{' '}
          {channel.dailyLimit.toLocaleString()} today · {channel.failureRate}%
          fail
        </p>
      </CardContent>
    </Card>
    ...
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        ...
        <p className="text-xs">
          ...
          {campaign.sentCount.toLocaleString()} delivered · {campaign.openRate}%
          open
        </p>
      </CardContent>
    </Card>
        ...
        {deliveries.slice(0, 10).map((d) => (
          ...
          >
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {d.recipientId} · {d.channel.replace('_', ' ')} · {d.attempts}{' '}
                attempt(s)
              </p>
            </div>
            <div className="text-right">
              ...
              {d.deliveredAt && (
                <p className="text-xs text-muted-foreground mt-1">
                  {format(new Date(d.deliveredAt), 'MMM d, HH:mm')}
                </p>
              )}
            </div>
          </div>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          {
            label: 'Avg Delivery (ms)',
            value: analytics.avgDeliveryTimeMs.toLocaleString(),
          },
          {
            label: 'Messages/Day',
            value: analytics.messagesSentDaily.toLocaleString(),
          },
        ].map((m) => (
          <Card key={m.label}>
    ...
    <Card>
      <CardContent className="pt-4 text-sm flex justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {integration.provider} · {integration.channel.replace('_', ' ')}
          </p>
        </div>
        <Badge
