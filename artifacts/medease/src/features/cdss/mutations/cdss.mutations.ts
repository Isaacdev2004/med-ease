...
export function useCdssMutations() {
  ...
  const acknowledgeAlert = useMutation({
    mutationFn: (input: AcknowledgeAlertInput) =>
      runOrQueue('Acknowledge alert', () =>
        cdssService.acknowledgeAlert(input),
      ),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Alert acknowledged.' });
    },
  });

  const overrideAlert = useMutation({
    mutationFn: (input: OverrideAlertInput) =>
      runOrQueue('Override alert', () => cdssService.overrideAlert(input)),
    onSuccess: () => {
      invalidateAll(client);
      appToast.info({ title: 'Alert overridden.' });
    },
  });

  const applyRecommendation = useMutation({
    mutationFn: (input: ApplyRecommendationInput) =>
      runOrQueue('Apply recommendation', () =>
        cdssService.applyRecommendation(input),
      ),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Recommendation applied.' });
    },
  });

  const createGuideline = useMutation({
    mutationFn: (input: CreateGuidelineInput) =>
      runOrQueue('Create guideline', () => cdssService.createGuideline(input)),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Guideline created.' });
    },
  });

  const updateRule = useMutation({
    mutationFn: (input: UpdateRuleInput) =>
      runOrQueue('Update rule', () => cdssService.updateRule(input)),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Rule updated.' });
    },
  });

  const publishProtocol = useMutation({
    mutationFn: (input: PublishProtocolInput) =>
      runOrQueue('Publish protocol', () => cdssService.publishProtocol(input)),
    onSuccess: () => {
      invalidateAll(client);
      appToast.success({ title: 'Protocol published.' });
    },
  });

  const calculateRisk = useMutation({
    mutationFn: (input: CalculateRiskInput) =>
      runOrQueue('Calculate risk', () =>
        cdssService.calculateRisk(input),
      ) as Promise<CalculatorResult | null>,
    onSuccess: () => invalidateAll(client),
  });

  const exportData = useMutation({
    mutationFn: (format: 'csv' | 'pdf' | 'xlsx') =>
      runOrQueue('Export CDSS', () => cdssService.exportData(format)),
    onSuccess: () => {
      invalidateAll(client);
  ...
  const favorite = useMutation({
    mutationFn: ({
      ...
      runOrQueue('Favorite', () =>
        cdssService.favorite(userId, entityType, entityId),
      ),
    onSuccess: () => invalidateAll(client),
  });
