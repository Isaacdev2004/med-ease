...
export function CdssTabs({
  ...
  return (
    <nav
      ...
      aria-label="Clinical decision support sections"
    >
      {tabs.map((tab) => {
...
export function getCdssSectionFromPath(
  pathname: string,
  variant: 'professional' | 'facility' | 'admin' = 'professional',
): CdssSection {
  if (
    variant === 'admin' &&
    (pathname.endsWith('/cdss') ||
      (pathname.includes('/cdss') && !pathname.includes('/cdss-')))
  )
    return 'rules';
  if (pathname.includes('/clinical-alerts')) return 'alerts';
  if (pathname.includes('/recommendations')) return 'recommendations';
  if (
    pathname.includes('/cdss-guidelines') ||
    (variant !== 'admin' && pathname.includes('/guidelines'))
  )
    return 'guidelines';
  ...
  if (pathname.includes('/cdss-order-sets') || pathname.includes('/order-sets'))
    return 'order-sets';
