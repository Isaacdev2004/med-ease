...
import type { DirectoryFilters, DirectorySort, ProviderType } from '@/services/directory/directory.types';
...
export function useDirectoryFilters(defaultType?: ProviderType | 'all') {
  ...
  const view = (searchParams.get('view') as 'cards' | 'table' | 'compact' | 'map') || 'cards';
  ...
  return {
    ...
    setSort: (value: DirectorySort) => url.setParam('sort', value === 'relevance' ? null : value),
    setType: (value: string) => url.setParam('type', value === 'all' ? null : value),
    ...
    setTeleconsultation: (value: boolean) => url.setParam('tele', value ? '1' : null),
    setEmergency: (value: boolean) => url.setParam('emergency', value ? '1' : null),
