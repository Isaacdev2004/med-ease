...
function resolvePatientBasePath(location: string) {
  if (location.includes('/invoices'))
    return resolveModuleBasePath(location, 'invoices');
  if (location.includes('/payments'))
    return resolveModuleBasePath(location, 'payments');
  if (location.includes('/insurance'))
    return resolveModuleBasePath(location, 'insurance');
  if (location.includes('/receipts'))
    return resolveModuleBasePath(location, 'receipts');
...
export default function PatientBillingPage() {
  ...
  return (
    <BillingShell
      basePath={resolvePatientBasePath(location)}
      variant="patient"
