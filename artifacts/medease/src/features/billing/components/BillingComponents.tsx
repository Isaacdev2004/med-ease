    ...
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        <p className="text-muted-foreground">
          {format(new Date(invoice.issueDate), 'MMM d, yyyy')} · Due{' '}
        ...
        {invoice.balance > 0 ? (
          <p className="text-xs text-muted-foreground">
            Balance: {invoice.currency} {invoice.balance.toLocaleString()}
          </p>
        ) : null}
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1 text-sm">
        ...
        <p>
          Claim: {claim.currency} {claim.totalClaim.toLocaleString()}
        </p>
...
export function InsuranceCard({ policy }: { policy: InsurancePolicy }) {
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm">{policy.payer}</CardTitle>
      </CardHeader>
      <CardContent className="text-sm space-y-1">
        <p className="text-muted-foreground">
          {policy.planType} · {policy.policyNumber}
        </p>
        <Badge className="capitalize">{policy.status}</Badge>
        <p className="text-xs">
          Deductible: {policy.deductible} · Copay: {policy.copay}
        </p>
        {policy.eligibilityVerified ? (
          <p className="text-xs text-emerald-600">Eligibility verified</p>
        ) : (
...
export function FinancialMetrics({
  ...
  const kpis = [
    {
      label: 'Daily revenue',
      value: `€${dashboard.dailyRevenue.toLocaleString()}`,
    },
    {
    ...
    {
      label: 'Outstanding',
      value: `€${dashboard.outstandingBalances.toLocaleString()}`,
    },
    ...
    {
      label: 'Net revenue',
      value: `€${dashboard.netRevenue.toLocaleString()}`,
    },
  ];
  return (
    ...
    <Card>
      <CardContent className="pt-4">
        ...
        <p className="text-2xl font-bold text-destructive">
          {balance.currency} {balance.totalOutstanding.toLocaleString()}
        </p>
        <p className="text-xs text-muted-foreground">
          {balance.invoiceCount} invoices · oldest due{' '}
          {format(new Date(balance.oldestDueDate), 'MMM d')}
        </p>
      </CardContent>
    </Card>
  );
    ...
    <div className="space-y-3">
      {entries.map((e) => (
        <div key={e.id} className="border-l-2 border-primary/30 pl-4">
          ...
          <p className="text-xs text-muted-foreground">
            {format(new Date(e.date), 'MMM d, HH:mm')} · {e.status}
          </p>
        </div>
      ))}
    </div>
    ...
    <Card>
      <CardContent className="pt-4 text-sm space-y-1">
        <div className="flex justify-between">
          ...
          <Badge className="capitalize">{refund.status}</Badge>
        </div>
            ...
            <tbody>
              {invoice.lineItems.map((li) => (
                <tr key={li.id} className="border-t">
                  ...
                  <td className="p-2 text-right">{li.total}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <CardTitle>{claim.claimId}</CardTitle>
        <ClaimStatusBadge status={claim.status} />
      </CardHeader>
      <CardContent className="text-sm space-y-2">
        <p>
          <span className="text-muted-foreground">Payer:</span> {claim.payer}
        </p>
        <p>
        ...
        <p>
          ...
          {claim.procedureCodes.join(', ')}
        </p>
        <div className="grid grid-cols-3 gap-2 pt-2">
          <div>
            <p className="text-muted-foreground">Claimed</p>
            <p className="font-bold">{claim.totalClaim}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Approved</p>
            <p className="font-bold text-emerald-600">{claim.approvedAmount}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Denied</p>
            <p className="font-bold text-destructive">{claim.deniedAmount}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
export function CollectionPanel({
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm space-y-2">
        <p>
          Collection rate: <span className="font-bold">{rate}%</span>
        </p>
        <p>
          Outstanding A/R:{' '}
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <BarChartPanel
        ...
        data={analytics.departmentRevenue.map((d) => ({
          label: d.department,
          value: d.amount,
        }))}
      />
      <BarChartPanel
        title="Claims trends"
        data={analytics.claimsTrends.map((c) => ({
          label: c.month,
          value: c.approved,
        }))}
      />
      <BarChartPanel
        title="Collections trends"
        data={analytics.collectionsTrends.map((c) => ({
          label: c.month,
          value: c.collected,
        }))}
      />
