    ...
    <Card>
      ...
      <CardContent className="text-sm space-y-1">
        ...
        <p className="text-muted-foreground">
          {prescription.prescribingPhysician}
        </p>
        <p className="text-muted-foreground">
          Expires {format(new Date(prescription.expiresAt), 'PP')}
        </p>
...
export function EnterpriseMedicationCard({
  medication,
  actions,
}: {
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm text-muted-foreground">
        <p>
          {refill.patientName} · {refill.pharmacyName}
        </p>
        <p>
          {refill.remainingTablets ?? '—'} tablets · {refill.daysLeft ?? '—'}{' '}
          days left
        </p>
...
export function InteractionAlert({
  ...
  return (
    <Card
      ...
      style={{
        borderLeftColor: getInteractionSeverityColor(interaction.severity),
      }}
    >
      <CardContent className="pt-4 flex gap-3">
        ...
        <div className="text-sm">
          ...
          <p className="text-muted-foreground capitalize">
            {interaction.type} · {interaction.severity}
          </p>
...
export function MedicationKpiCards({
  dashboard,
}: {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <MetricCard
        ...
        value={dashboard.medicationScore}
        status="success"
      />
    </div>
  );
...
export function PharmacyQueueCard({
  item,
}: {
  item: {
    patientName: string;
...
export function MedicationWarningBanner({
  title,
  message,
}: {
  title: string;
...
export function DispenseCard({
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">{dispense.medicationName}</CardTitle>
      </CardHeader>
      <CardContent className="text-sm space-y-1">
        <p>
          {dispense.quantity} {dispense.unit} · {dispense.pharmacyName}
        </p>
        <p className="text-muted-foreground">
          Dispensed {format(new Date(dispense.dispensedAt), 'PP')}
        </p>
      </CardContent>
    </Card>
  );
...
export function MedicationAnalyticsPanel({
  analytics,
}: {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <Card>
        <CardContent className="pt-4">
          <p className="text-2xl font-bold">{analytics.adherenceAverage}%</p>
          <p className="text-xs text-muted-foreground">Adherence</p>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="pt-4">
          <p className="text-2xl font-bold">{analytics.pendingRefills}</p>
          <p className="text-xs text-muted-foreground">Pending refills</p>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="pt-4">
          <p className="text-2xl font-bold">{analytics.interactionCount}</p>
