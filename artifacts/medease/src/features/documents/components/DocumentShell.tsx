...
import { DocumentTabs, getDocumentSectionFromPath } from '@/features/documents/components/DocumentTabs';
...
export function DocumentShell({
  ...
  const scopedFilters = useMemo((): DocumentFilters => ({ tenantId, facilityId, userId }), [tenantId, facilityId, userId]);

  if (!perms.canView) {
    return (
      <PageShell title={title}>
        <EmptyState title="Access denied" description="You do not have permission to view documents." />
      </PageShell>
    );
  }
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        <DocumentSectionContent section={section} filters={scopedFilters} variant={variant} />
      </div>
    </PageShell>
  );
}
