...
import {
  PublicHealthTabs,
  getPublicHealthSectionFromPath,
} from '@/features/public-health/components/PublicHealthTabs';
...
export function PublicHealthShell({
  ...
  const scopedFilters = useMemo(
    (): PublicHealthFilters => ({ facilityId }),
    [facilityId],
  );
  ...
  if (!perms.canView) {
    return (
      <PageShell title={title}>
        <EmptyState
          title="Access denied"
          description="You do not have permission to view public health."
        />
      </PageShell>
    );
  }
  ...
  return (
    <PageShell
      title={title}
      subtitle="Enterprise public health — disease surveillance, immunization registries, outbreak response, contact tracing, SDOH, and community health analytics."
    >
