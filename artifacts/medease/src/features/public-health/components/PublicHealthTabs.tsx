...
const PROFESSIONAL_TABS: Tab[] = [
  ...
  { segment: 'surveillance', label: 'Disease Surveillance', path: 'disease-surveillance' },
  { segment: 'immunizations', label: 'Immunizations', path: 'immunizations' },
  { segment: 'community-programs', label: 'Community Programs', path: 'community-programs' },
...
const FACILITY_TABS: Tab[] = [
  ...
  { segment: 'contact-tracing', label: 'Contact Tracing', path: 'contact-tracing' },
  { segment: 'environmental', label: 'Environmental Health', path: 'environmental-health' },
  { segment: 'outreach', label: 'Community Outreach', path: 'community-outreach' },
];

const ADMIN_TABS: Tab[] = [
  { segment: 'hub', label: 'Public Health Hub', path: 'public-health' },
  { segment: 'epidemiology', label: 'Epidemiology', path: 'epidemiology' },
  { segment: 'immunization-registry', label: 'Immunization Registry', path: 'immunization-registry' },
  { segment: 'maternal-child', label: 'Maternal & Child', path: 'maternal-child-health' },
  { segment: 'school-health', label: 'School Health', path: 'school-health' },
  { segment: 'occupational', label: 'Occupational Health', path: 'occupational-health' },
  { segment: 'analytics', label: 'Analytics', path: 'public-health-analytics' },
  { segment: 'community-dashboard', label: 'Community Dashboard', path: 'community-health-dashboard' },
];
...
export function PublicHealthTabs({ basePath, variant = 'professional' }: PublicHealthTabsProps) {
  ...
  return (
    <nav className="flex flex-wrap gap-1 border-b pb-2" aria-label="Public health sections">
      {tabs.map((tab) => {
...
export function getPublicHealthSectionFromPath(pathname: string, variant: 'professional' | 'facility' | 'admin' = 'professional'): PublicHealthSection {
  if (pathname.includes('/disease-surveillance')) return 'surveillance';
  if (pathname.includes('/immunizations') && !pathname.includes('/immunization-registry')) return 'immunizations';
  ...
  if (pathname.includes('/immunization-registry')) return 'immunization-registry';
  ...
  if (pathname.includes('/community-health-dashboard')) return 'community-dashboard';
  if (variant === 'admin' && pathname.includes('/public-health') && !pathname.includes('/public-health-analytics')) return 'hub';
  return 'dashboard';
}
