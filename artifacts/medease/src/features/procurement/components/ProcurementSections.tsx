...
export function DashboardSection({
  ...
  if (!dashboard.data)
    return <EmptyState icon={ShoppingCart} title="No procurement data" />;
  return (
...
export function RequestsSection({ filters }: { filters?: ProcurementFilters }) {
  ...
  if (!items.length)
    return <EmptyState icon={ShoppingCart} title="No purchase requests" />;
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
...
export function AnalyticsSection() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(invoices.data?.items ?? []).slice(0, 6).map((i) => (
...
export function ProcurementSectionContent({
  section,
  filters,
}: {
  ...
  switch (section) {
    case 'requests':
      return <RequestsSection filters={filters} />;
    case 'purchase-orders':
      return <PurchaseOrdersSection filters={filters} />;
    case 'suppliers':
      return <SuppliersSection filters={filters} />;
    case 'rfqs':
      return <RFQsSection filters={filters} />;
    case 'contracts':
      return <ContractsSection filters={filters} />;
    case 'budgets':
      return <BudgetsSection filters={filters} />;
    case 'receiving':
      return <ReceivingSection filters={filters} />;
    case 'deliveries':
      return <DeliveriesSection filters={filters} />;
    case 'approvals':
