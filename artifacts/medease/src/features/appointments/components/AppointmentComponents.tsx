    ...
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      <div>
        ...
        <p className="text-muted-foreground">
          {appointment.id} · {appointment.patient.fullName}
        </p>
      </div>
...
export function EnterpriseAppointmentCard({
  ...
  if (compact) {
    return (
      <UpcomingAppointmentCard appointment={appointment} actions={actions} />
    );
  }
  return (
    <Card>
      ...
      <CardContent className="space-y-2 text-sm">
        ...
        <p className="flex items-center gap-2">
          ...
          {appointment.facility.name} · {appointment.room}
        </p>
        <p className="flex items-center gap-2">
...
export function AppointmentSummary({
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <StatCard
        label="Duration"
        value={`${appointment.durationMinutes}m`}
        icon={Clock}
      />
      <StatCard
        label="Priority"
      ...
      <StatCard
        label="Visit"
        value={appointment.visitType.replace('_', ' ')}
        icon={appointment.visitType === 'telemedicine' ? Video : Stethoscope}
      />
    </div>
  );
}
...
export function AppointmentMetrics({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <MetricCard
        title="Avg Wait"
        value={`${analytics.averageWaitMinutes}m`}
        status="info"
      />
      <MetricCard
        title="Utilization"
        value={`${analytics.utilizationPercent}%`}
        status="success"
      />
      <MetricCard
        title="Booking Rate"
        value={`${analytics.bookingRatePercent}%`}
        status="info"
      />
      <MetricCard
        title="Queue"
        value={analytics.queueLength}
        status="warning"
      />
      <MetricCard
        title="Telemedicine"
        value={analytics.telemedicineCount}
        status="info"
      />
    </div>
  );
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">
            #{entry.position} {entry.patientName}
          </CardTitle>
...
export function CheckInCard({
  ...
  return (
    <Card
      className={cn(
        appointment.checkInStatus !== 'not_checked_in' && 'border-success/50',
      )}
    >
      <CardHeader className="pb-2">
