...
export function DashboardSection({
  ...
  if (upcoming.isLoading || analytics.isLoading)
    return <LoadingView label="Loading appointments…" />;

  return (
    <div className="space-y-6">
      {analytics.data ? (
        <AppointmentMetrics analytics={analytics.data} />
      ) : null}
...
export function CalendarSection({ filters }: { filters?: AppointmentFilters }) {
  ...
  return (
    <CalendarView
      ...
      grid={
        'grid' in (data ?? {})
          ? (data as { grid: Parameters<typeof CalendarView>[0]['grid'] }).grid
          : undefined
      }
      dayEvents={
        'dayEvents' in (data ?? {})
          ? (
              data as {
                dayEvents: Parameters<typeof CalendarView>[0]['dayEvents'];
              }
            ).dayEvents
          : undefined
      }
      agendaEvents={
        'agenda' in (data ?? {})
          ? (
              data as {
                agenda: Parameters<typeof CalendarView>[0]['agendaEvents'];
              }
            ).agenda
          : undefined
      }
...
export function TelemedicineSection({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      {(query.data ?? []).map((a) => (
        <EnterpriseAppointmentCard key={a.id} appointment={a} />
      ))}
    </div>
  );
}
...
export function DetailSection({
  ...
  appointment: import('@/services/appointments/types').Appointment;
}) {
  return (
    <div className="space-y-6">
...
export function AppointmentSectionContent({
  ...
  switch (section) {
    case 'book':
      return <BookSection patientId={patientId} />;
    case 'calendar':
    ...
    case 'queue':
      return <QueueSection filters={filters} />;
    case 'telemedicine':
      return <TelemedicineSection filters={filters} />;
    case 'resources':
      return <ResourcesSection filters={filters} />;
    case 'analytics':
      return <AnalyticsSection filters={filters} />;
    case 'follow-ups':
      return <UpcomingSection filters={{ ...filters, followUp: true }} />;
    default:
