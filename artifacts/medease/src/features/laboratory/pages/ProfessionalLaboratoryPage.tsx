...
export default function ProfessionalLaboratoryPage() {
  ...
  const segment = location.includes('/critical')
    ? 'laboratory/critical'
  ...
  const fullBase = segment.includes('/')
    ? resolveModuleBasePath(location, segment)
    : basePath;
  const title = location.includes('/critical')
    ? 'Critical Results'
    : location.includes('/orders')
      ? 'Lab Orders'
      : location.includes('/results')
        ...
        : 'Laboratory';
  return (
    <LaboratoryShell basePath={fullBase} variant="clinician" title={title} />
  );
}
