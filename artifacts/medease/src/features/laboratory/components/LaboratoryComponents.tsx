...
export function ReferenceRangeBadge({ flag }: { flag: ResultFlag }) {
  const variant =
    flagSeverity(flag) === 'success'
      ? 'success'
      : flagSeverity(flag) === 'critical'
        ? 'destructive'
        : 'warning';
  return (
    <Badge variant={variant} className="capitalize">
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm space-y-1">
        <p>
          {order.orderingPhysician} · {order.facilityName}
        </p>
        <p className="text-muted-foreground capitalize">
          {order.priority} · {order.collectionMethod.replace('_', ' ')}
        </p>
        {order.scheduledAt ? (
          <p>Scheduled {format(new Date(order.scheduledAt), 'PP')}</p>
        ) : null}
      </CardContent>
    </Card>
  );
  ...
  return (
    <Card className={cn(critical && 'border-destructive/50')}>
      <CardHeader className="pb-2 flex-row justify-between">
        ...
        <Badge variant={report.status === 'released' ? 'success' : 'info'}>
          {report.status}
        </Badge>
      </CardHeader>
      <CardContent className="text-sm space-y-2">
        <p>
          {report.reportNumber} · {report.category.replace('_', ' ')}
        </p>
        ...
        {report.releasedAt ? (
          <p className="text-muted-foreground">
            Released {format(new Date(report.releasedAt), 'PP')}
          </p>
        ) : null}
      </CardContent>
    </Card>
  ...
  return (
    <Card>
      <CardHeader className="pb-2 flex-row justify-between">
        ...
        <Badge
          variant={specimen.status === 'rejected' ? 'destructive' : 'outline'}
      ...
      <CardContent className="text-sm space-y-1">
        ...
        {specimen.collectedAt ? (
          <p>Collected {format(new Date(specimen.collectedAt), 'PPp')}</p>
        ) : null}
        {specimen.temperature ? (
          <p className="text-muted-foreground">
            Storage: {specimen.temperature}
          </p>
        ) : null}
      </CardContent>
    </Card>
...
export function CriticalAlertCard({ alert }: { alert: LabAlert }) {
  return (
    <Card
      ...
      style={{ borderLeftColor: alertSeverityColor(alert.severity) }}
    >
...
export function LaboratoryKpiCards({
  dashboard,
}: {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <MetricCard
        title="In Processing"
        value={dashboard.inProcessing}
        status="info"
      />
      <MetricCard
        title="Results Ready"
        value={dashboard.resultsReady}
        status="success"
      />
      <MetricCard
        title="Critical"
        value={dashboard.criticalResults}
        status={dashboard.criticalResults > 0 ? 'critical' : 'success'}
      />
    </div>
  );
}
  ...
  return (
    <ChartPanel title={`${series.testName} trend`}>
      ...
      <p className="text-xs text-muted-foreground mt-2">
        Reference: {series.referenceRange}
      </p>
    </ChartPanel>
  );
}
...
export function LaboratoryAnalyticsPanel({
  analytics,
}: {
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardContent className="pt-4">
            <p className="text-2xl font-bold">
              {analytics.averageTurnaroundHours}h
            </p>
        ...
        <Card>
          <CardContent className="pt-4">
            <p className="text-2xl font-bold">{analytics.verificationRate}%</p>
      ...
      <BarChartPanel
        ...
        data={analytics.turnaroundByDepartment}
      />
    </div>
  );
}
...
export function ResultStatusBadge({
  ...
  status: LabDiagnosticReport['status'];
}) {
  const variant =
    status === 'released'
      ? 'success'
...
export function CriticalValueBanner({
  title,
  message,
}: {
  title: string;
...
export function ReferenceRangePanel({
  observations,
}: {
  observations: LabObservation[];
}) {
      ...
      <CardContent className="text-sm space-y-2">
        ...
        {result.cultures.map((c, i) => (
          <div key={i} className="rounded border p-2">
            ...
            {c.colonyCount ? (
              <p className="text-muted-foreground">{c.colonyCount}</p>
            ) : null}
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm space-y-2">
        {result.macroscopic ? (
          <p>
            <strong>Macroscopic:</strong> {result.macroscopic}
          </p>
        ) : null}
        {result.microscopic ? (
          <p>
            <strong>Microscopic:</strong> {result.microscopic}
          </p>
        ) : null}
        ...
        {result.pathologistName ? (
          <p className="text-muted-foreground">
            Pathologist: {result.pathologistName}
          </p>
        ) : null}
      </CardContent>
    </Card>
...
export function BloodBankPanel({ record }: { record: BloodBankRecord }) {
  return (
    <Card>
      ...
      <CardContent className="text-sm space-y-1">
        <p>
          Group {record.bloodGroup}
          {record.rhFactor === 'Positive' ? '+' : '−'}
        </p>
        <p className="capitalize">Cross-match: {record.crossMatchResult}</p>
        <p className="text-muted-foreground">
          Collected {format(new Date(record.collectedAt), 'PP')}
        </p>
      </CardContent>
    </Card>
  );
      ...
      <div className="grid gap-2">
        {dashboard.recentQc.slice(0, 6).map((qc) => (
          ...
          >
            ...
            <Badge variant={qc.withinRange ? 'success' : 'destructive'}>
              {qc.withinRange ? 'Pass' : 'Fail'}
            </Badge>
          </div>
        ))}
      </div>
  ...
  return (
    <Card>
      <CardHeader className="pb-2 flex-row justify-between">
        ...
        <Badge
          variant={instrument.status === 'online' ? 'success' : 'warning'}
          className="capitalize"
        >
          {instrument.status}
        </Badge>
      ...
      <CardContent className="text-sm space-y-1">
        ...
        <p className="text-muted-foreground">
          {instrument.department} · {instrument.utilizationPercent}% util.
        </p>
      </CardContent>
    </Card>
  );
...
export function TechnologistCard({
  technologist,
}: {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-base">{technologist.name}</CardTitle>
      </CardHeader>
      <CardContent className="text-sm space-y-1">
        <p>{technologist.credentials}</p>
        <p className="text-muted-foreground">
          {technologist.department} · {technologist.activeOrders} active
        </p>
      </CardContent>
    </Card>
  );
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm text-muted-foreground capitalize">
        {category.replace('_', ' ')} · {testCount} tests
      </CardContent>
    </Card>
  );
}
