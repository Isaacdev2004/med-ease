...
const PROFESSIONAL_TABS: { segment: LaboratorySection; label: string; path: string }[] = [
...
const FACILITY_TABS: { segment: LaboratorySection; label: string; path: string }[] = [
...
const ADMIN_TABS: { segment: LaboratorySection; label: string; path: string }[] = [
  { segment: 'catalog', label: 'Laboratory', path: 'laboratory' },
  { segment: 'analytics', label: 'Analytics', path: 'laboratory/analytics' },
  { segment: 'instruments', label: 'Instruments', path: 'laboratory/instruments' },
...
function portalRoot(basePath: string, variant: LaboratoryTabsProps['variant']) {
  if (variant === 'facility') return basePath.replace(/\/(laboratory|dashboard|specimens|quality)$/, '');
  if (variant === 'admin') return basePath.replace(/\/laboratory(\/.*)?$/, '');
  if (variant === 'clinician') return basePath.replace(/\/(laboratory|results|microbiology|pathology)(\/.*)?$/, '');
  return basePath;
}

export function LaboratoryTabs({ basePath, variant = 'patient' }: LaboratoryTabsProps) {
  ...
  if (variant === 'facility' || variant === 'admin') {
    ...
    return (
      <nav className="flex flex-wrap gap-1 border-b pb-2" aria-label="Laboratory sections">
        {tabs.map((tab) => {
  ...
  function isActive(segment: LaboratorySection | '') {
    if (segment === '') return location === basePath || location.endsWith('/laboratory');
...
export function getLaboratorySectionFromPath(pathname: string): LaboratorySection {
  const segment = pathname.split('/').filter(Boolean).pop() ?? '';
  const valid: LaboratorySection[] = [
    'orders', 'results', 'history', 'trends', 'list', 'critical',
    'queue', 'specimens', 'verification', 'pending', 'catalog', 'analytics', 'reports', 'quality',
    'microbiology', 'pathology', 'bloodBank', 'instruments', 'dashboard',
  ];
  if (segment === 'laboratory') return pathname.includes('/admin') ? 'catalog' : pathname.includes('/facility') ? 'queue' : 'list';
  ...
  return valid.includes(segment as LaboratorySection) ? (segment as LaboratorySection) : 'dashboard';
}
