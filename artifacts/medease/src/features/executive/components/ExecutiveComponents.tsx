...
export function ExecutiveDashboard({
  ...
  dashboard: ExecutiveCommandCenter;
}) {
  const metrics = [
    {
      label: 'Enterprise KPIs',
      value: dashboard.totalKpis.toLocaleString(),
      icon: Target,
    },
    {
      label: 'Active Alerts',
    ...
    {
      label: 'Bed Occupancy',
      value: `${dashboard.bedOccupancy}%`,
      icon: BedDouble,
    },
    {
      label: 'Revenue MTD',
      value: `$${(dashboard.revenueMtd / 1000000).toFixed(1)}M`,
      icon: DollarSign,
    },
    {
      label: 'Quality Score',
      value: dashboard.qualityScore.toFixed(0),
      icon: Shield,
    },
    {
      label: 'Initiatives On Track',
      value: dashboard.initiativesOnTrack.toLocaleString(),
      icon: Activity,
    },
  ];
  return (
    ...
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <TrendIcon
            className={`h-4 w-4 ${kpi.trend === 'up' ? 'text-green-600' : kpi.trend === 'down' ? 'text-red-600' : 'text-muted-foreground'}`}
          />
    ...
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          ...
          <Badge>
            {scorecardGrade(scorecard.overallScore)} · {scorecard.overallScore}
          </Badge>
        </div>
        <p className="text-xs text-muted-foreground">
          {scorecard.period} · {scorecard.facilityId}
        </p>
...
export function HospitalOperationsBoard({
  operations,
}: {
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Building2 className="h-5 w-5" /> Hospital Operations
        </CardTitle>
      </CardHeader>
        ...
        {snapshots.slice(0, 6).map((s) => (
          <Card key={s.snapshotId}>
            <CardContent className="pt-4 text-sm space-y-1">
              ...
              <p className="text-xs">
                {s.occupiedBeds}/{s.totalBeds} beds · {s.occupancyRate}%
              </p>
            </CardContent>
          </Card>
        ))}
...
export function BedManagementBoard({
  snapshots,
}: {
  snapshots: CapacitySnapshot[];
}) {
  return (
    <CapacityDashboard
      snapshots={snapshots}
      aggregateOccupancy={Math.round(
        snapshots.reduce((s, c) => s + c.occupancyRate, 0) /
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 text-sm">
          <div>
            ...
            <p className="text-lg font-semibold">{flow.arrivalsToday}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Discharges</p>
            <p className="text-lg font-semibold">{flow.dischargesToday}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Transfers</p>
            <p className="text-lg font-semibold">{flow.transfersToday}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Avg LOS</p>
            <p className="text-lg font-semibold">
              {flow.avgLengthOfStay.toFixed(1)}d
            </p>
          </div>
        </div>
      ...
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
          <div>
            ...
            <p className="text-lg font-semibold">
              ${(revenue.revenueMtd / 1000000).toFixed(1)}M
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">YTD</p>
            <p className="text-lg font-semibold">
              ${(revenue.revenueYtd / 1000000).toFixed(1)}M
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Collection Rate</p>
            <p className="text-lg font-semibold">{revenue.collectionRate}%</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Denial Rate</p>
            <p className="text-lg font-semibold">{revenue.denialRate}%</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Net Margin</p>
            <p className="text-lg font-semibold">{revenue.netMargin}%</p>
          </div>
        </div>
...
export function ClinicalQualityDashboard({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Shield className="h-5 w-5" /> Clinical Quality
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
          <div>
            ...
            <p className="text-lg font-semibold">{quality.overallScore}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">
            ...
            <p className="text-lg font-semibold">
              {quality.patientSatisfaction}%
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Infection Rate</p>
            <p className="text-lg font-semibold">{quality.infectionRate}%</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Mortality Index</p>
            <p className="text-lg font-semibold">{quality.mortalityIndex}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Compliance</p>
            <p className="text-lg font-semibold">{quality.complianceRate}%</p>
          </div>
        </div>
...
export function WorkforceDashboardPanel({
  workforce,
}: {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Users className="h-5 w-5" /> Workforce
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
          <div>
            ...
            <p className="text-lg font-semibold">
              {workforce.totalStaff.toLocaleString()}
            </p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Vacancy Rate</p>
            <p className="text-lg font-semibold">{workforce.vacancyRate}%</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Overtime Hours</p>
            <p className="text-lg font-semibold">{workforce.overtimeHours}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Turnover</p>
            <p className="text-lg font-semibold">{workforce.turnoverRate}%</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground">Satisfaction</p>
            <p className="text-lg font-semibold">
              {workforce.satisfactionScore}%
            </p>
          </div>
        </div>
...
export function PopulationHealthDashboard({
  population,
}: {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <HeartPulse className="h-5 w-5" /> Population Health
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 text-sm">
...
export function EnterpriseAlertCenter({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <AlertTriangle className="h-5 w-5" /> Enterprise Alerts
        </CardTitle>
      </CardHeader>
          ...
          alerts.slice(0, 8).map((a) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {a.sourceModule} · {format(new Date(a.createdAt), 'MMM d')}
                </p>
              </div>
      ...
      <CardContent className="pt-4 text-sm space-y-2">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <div>
            ...
            <p className="font-medium">{report.internalValue}</p>
          </div>
          <div>
            <p className="text-muted-foreground">Peer</p>
            <p className="font-medium">{report.peerAverage}</p>
          </div>
          <div>
            <p className="text-muted-foreground">National</p>
            <p className="font-medium">{report.nationalBenchmark}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
export function StrategicInitiativeCard({
  initiative,
}: {
    ...
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          ...
          <Badge
            variant={initiativeVariant[initiative.status]}
        ...
        <p className="text-xs text-muted-foreground">
          {initiative.progress}% · ${(initiative.spent / 1000).toFixed(0)}K / $
...
export function ForecastPanel({ forecast }: { forecast: ExecutiveForecast }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <LineChart className="h-5 w-5" /> {forecast.metric}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          {forecast.horizon} horizon · {forecast.facilityId}
        </p>
  ...
  return (
    <div className="space-y-6">
        ...
        {[
          {
            ...
            value: `${analytics.kpiAchievementRate}%`,
          },
          {
          ...
          },
          {
            ...
            value: `${analytics.financialPerformance}%`,
          },
          ...
          },
          {
            ...
            value: `${analytics.benchmarkPercentile}th`,
          },
        ].map((m) => (
          <Card key={m.label}>
            <CardContent className="pt-4">
      ...
      <BarChartPanel
        ...
        data={analytics.moduleContributions}
      />
      <BarChartPanel
        title="Initiative Progress"
        data={analytics.initiativeProgress}
      />
    </div>
  );
}
