...
export default function AdminRadiologyPage() {
  ...
  const segment = location.includes('radiology/analytics')
    ? 'radiology/analytics'
    : location.includes('radiology/devices')
      ? 'radiology/devices'
      : location.includes('radiology/workload')
        ? 'radiology/workload'
        : 'radiology';
  const basePath = resolveModuleBasePath(
    location,
    segment.startsWith('radiology/') ? segment : 'radiology',
  );
  ...
  return (
    <RadiologyShell
      basePath={basePath}
      variant="admin"
      title={titles[segment] ?? 'Radiology'}
    />
  );
}
