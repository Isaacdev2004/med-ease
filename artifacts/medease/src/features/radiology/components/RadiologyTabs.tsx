...
function portalRoot(basePath: string, variant: RadiologyTabsProps['variant']) {
  if (variant === 'facility')
    return basePath.replace(/\/(radiology|dashboard|imaging|devices)$/, '');
  if (variant === 'admin') return basePath.replace(/\/radiology(\/.*)?$/, '');
  if (variant === 'clinician')
    return basePath.replace(
      /\/(radiology|worklist|reports|compare)(\/.*)?$/,
      '',
    );
  return basePath;
}
...
export function RadiologyTabs({
  ...
  function isActive(segment: RadiologySection | '') {
    if (segment === '')
      return location === basePath || location.endsWith('/radiology');
...
export function getRadiologySectionFromPath(
  pathname: string,
): RadiologySection {
  ...
  if (pathname.endsWith('/radiology'))
