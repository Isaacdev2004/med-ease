...
export function DashboardSection({
  ...
  if (!dashboard.data)
    return <EmptyState icon={Sparkles} title="No AI intelligence data" />;
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(recommendations.data?.items ?? []).slice(0, 4).map((r) => (
          <RecommendationCard key={r.recommendationId} recommendation={r} />
        ))}
      </div>
...
export function PredictionsSection({
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(predictions.data?.items ?? []).slice(0, 12).map((p) => (
          <PredictionCard key={p.predictionId} prediction={p} />
        ))}
      </div>
      {(explainability.data?.items ?? []).slice(0, 2).map((r) => (
...
export function RiskScoresSection({
  filters,
}: {
  filters?: AiIntelligenceFilters;
}) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(riskScores.data?.items ?? []).slice(0, 12).map((a) => (
        <RiskScoreCard key={a.assessmentId} assessment={a} />
      ))}
    </div>
  );
}
...
export function SummariesSection({
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(summaries.data?.items ?? []).slice(0, 12).map((s) => (
...
export function ResourcePredictionsSection({
  filters,
}: {
  filters?: AiIntelligenceFilters;
}) {
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {(models.data?.items ?? []).map((m) => (
        <ModelPerformanceCard
          key={m.modelId}
          model={m}
          evaluations={performance.data?.items}
        />
      ))}
    </div>
  );
...
export function ModelMonitoringSection({
  filters,
}: {
  filters?: AiIntelligenceFilters;
}) {
        ...
        {(performance.data?.items ?? []).slice(0, 9).map((e) => (
          <ModelPerformanceCard
            ...
            model={{
              ...
              version: '',
              type: 'deterioration',
              status: e.passed ? 'production' : 'staging',
              accuracy: e.value,
              createdAt: e.evaluatedAt,
            }}
            evaluations={[e]}
          />
        ))}
...
export function AnalyticsSection({
  ...
  if (!analytics.data)
    return <EmptyState icon={Sparkles} title="No analytics data" />;
  return (
    <div className="space-y-6">
...
export function AiIntelligenceSectionContent({
  section,
  filters,
}: {
  ...
  switch (section) {
    case 'copilot':
      return <CopilotSection filters={filters} />;
    case 'predictions':
      return <PredictionsSection filters={filters} />;
    case 'risk-scores':
      return <RiskScoresSection filters={filters} />;
    case 'summaries':
      return <SummariesSection filters={filters} />;
    case 'forecasting':
      return <ForecastingSection filters={filters} />;
    case 'resource-predictions':
      return <ResourcePredictionsSection filters={filters} />;
    case 'capacity':
      return <CapacitySection filters={filters} />;
    case 'hub':
      return <DashboardSection filters={filters} />;
    case 'model-registry':
      return <ModelRegistrySection filters={filters} />;
    case 'model-monitoring':
      return <ModelMonitoringSection filters={filters} />;
    case 'governance':
      return <GovernanceSection filters={filters} />;
    case 'bias':
      return <BiasSection filters={filters} />;
    case 'analytics':
