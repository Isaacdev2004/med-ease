...
export function BatteryIndicator({
  ...
  percent?: number;
  status: MonitoringDevice['battery'];
}) {
  const color =
    status === 'critical'
      ? 'text-destructive'
...
export function NEWSScoreCard({ score }: { score: EarlyWarningScore }) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm">NEWS2 Score</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-4xl font-bold">{score.score}</p>
        <Badge
          className="mt-2 capitalize"
          variant={score.riskLevel === 'critical' ? 'destructive' : 'secondary'}
        >
...
export function MEWSCard({ score }: { score: EarlyWarningScore }) {
  ...
  return (
    <Card>
      <CardHeader className="pb-2">
        <CardTitle className="text-sm">MEWS Score</CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-4xl font-bold">{score.score}</p>
        <Badge
...
export function RPMEnrollmentCard({
  program,
}: {
...
export function MonitoringAnalyticsPanel({
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <BarChartPanel
        title="Daily vital trends"
        data={analytics.dailyVitalTrends}
      />
      <BarChartPanel title="Alert trends" data={analytics.alertTrends} />
      <BarChartPanel
        title="RPM enrollment growth"
        data={analytics.rpmEnrollmentGrowth}
      />
      <BarChartPanel
        title="Device utilization"
        data={analytics.deviceUtilizationChart}
      />
    </div>
  );
}
        ...
        <tbody>
          {observations.map((o) => (
            <tr key={o.id} className="border-t">
              ...
              <td className="p-2">
                {format(new Date(o.recordedAt), 'MMM d, HH:mm')}
              </td>
...
export function MonitoringSessionCard({
  ...
  return (
    <Card>
      ...
      <CardContent className="text-sm text-muted-foreground">
        <p>
          {session.observationCount} observations · {session.alertCount} alerts
        </p>
...
export function ObservationFiltersBar({
  onSearch,
}: {
