...
export function DashboardSection({ filters }: { filters?: MonitoringFilters }) {
  ...
  if (!dashboard.data)
    return <EmptyState icon={Activity} title="No monitoring data" />;
  return (
    <div className="space-y-6">
      ...
      {(alerts.data?.items ?? []).slice(0, 3).map((a) => (
...
export function VitalsSection({ filters }: { filters?: MonitoringFilters }) {
  ...
  if (!items.length)
    return <EmptyState icon={Activity} title="No vital signs" />;
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {items.slice(0, 18).map((v) => (
...
export function ObservationsSection({
  ...
  const items = query.data?.items ?? [];
  if (!items.length)
    return <EmptyState icon={Activity} title="No observations" />;
...
export function TimelineSection({ filters }: { filters?: MonitoringFilters }) {
  ...
  if (!entries.length)
    return <EmptyState icon={Activity} title="No timeline entries" />;
          ...
          {a.status === 'active' && (
            <div className="flex gap-2">
              <button
                ...
                onClick={() =>
                  void resolveAlert.mutateAsync({ id: a.id, by: 'Clinician' })
                }
              >
                Resolve
              </button>
              <button
                type="button"
                className="text-xs underline"
                onClick={() => void dismissAlert.mutateAsync(a.id)}
              >
                Dismiss
              </button>
            </div>
      ...
      {devices.slice(0, 12).map((d) => (
        <div key={d.id} className="space-y-2">
          ...
          <button
            type="button"
            className="text-xs underline"
            onClick={() => void syncDevice.mutateAsync(d.id)}
          >
            Sync device
          </button>
        </div>
...
export function RPMSection({ filters }: { filters?: MonitoringFilters }) {
  ...
  if (!programs.length)
    return <EmptyState icon={Activity} title="No RPM programs" />;
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
...
export function AnalyticsSection() {
  ...
  if (!analytics.data)
    return <EmptyState icon={Activity} title="No analytics" />;
...
export function ScoresSection({ filters }: { filters?: MonitoringFilters }) {
  ...
  if (!scores.length)
    return <EmptyState icon={Activity} title="No early warning scores" />;
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {scores
        .slice(0, 8)
...
export function MonitoringSectionContent({
  section,
  filters,
}: {
  ...
  switch (section) {
    case 'vitals':
      return <VitalsSection filters={filters} />;
    case 'observations':
      return <ObservationsSection filters={filters} />;
    case 'timeline':
      return <TimelineSection filters={filters} />;
    case 'alerts':
      return <AlertsSection filters={filters} />;
    case 'devices':
      return <DevicesSection filters={filters} />;
    case 'rpm':
      return <RPMSection filters={filters} />;
    case 'analytics':
      return <AnalyticsSection />;
    case 'scores':
      return <ScoresSection filters={filters} />;
    case 'history':
      return <TimelineSection filters={filters} />;
    default:
