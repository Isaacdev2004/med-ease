...
export default function MonitoringDetailPage() {
  ...
  if (query.isLoading)
    return (
      <PageShell title="Observation Details">
        <LoadingView />
      </PageShell>
    );
  ...
  if (!obs)
    return (
      <PageShell title="Observation Details">
        <EmptyState icon={Activity} title="Observation not found" />
      </PageShell>
    );
  ...
  return (
    <PageShell
      title={obs.display}
      subtitle={`Recorded ${new Date(obs.recordedAt).toLocaleString()}`}
    >
      <div className="space-y-4 max-w-xl">
        <VitalCard
          vital={{
            ...
            context: obs.context,
            status:
              obs.interpretation === 'critical'
                ? 'critical'
                : obs.interpretation === 'abnormal'
                  ? 'warning'
                  : 'normal',
          }}
        />
