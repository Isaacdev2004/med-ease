...
export function WorkflowDashboardPanel({
  ...
  const metrics = [
    ...
    {
      label: 'Pending Tasks',
      value: dashboard.pendingTasks.toLocaleString(),
      icon: ListTodo,
    },
    {
    ...
    {
      label: 'SLA Breaches',
      value: dashboard.slaBreaches.toLocaleString(),
      icon: AlertTriangle,
    },
    {
      label: 'Background Jobs',
      value: dashboard.backgroundJobsRunning.toLocaleString(),
      icon: Zap,
    },
  ];
  return (
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Workflow className="h-5 w-5" /> Workflow Designer
        </CardTitle>
      </CardHeader>
      <CardContent>
  ...
  return (
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        ...
        <Badge variant={slaVariant[instance.slaStatus]} className="capitalize">
    ...
    <Card>
      <CardContent className="pt-4 text-sm flex justify-between gap-2">
        <div>
          ...
          {trigger.eventName && (
            <p className="text-xs mt-1">{trigger.eventName}</p>
          )}
        </div>
      ...
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          <span className="font-medium flex items-center gap-1">
            <Clock className="h-4 w-4" /> {sla.name}
          </span>
          <Badge variant={slaVariant[sla.status]} className="capitalize">
            {sla.status.replace('_', ' ')}
          </Badge>
        </div>
        <p className="text-xs">
          Target: {sla.targetMinutes} min · Warning: {sla.warningMinutes} min
        </p>
      </CardContent>
    </Card>
  );
...
export function EscalationPanel({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <AlertTriangle className="h-5 w-5" /> Escalations
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
  ...
  return (
    <Card>
      <CardContent className="pt-4 text-sm space-y-2">
        <div className="flex justify-between gap-2">
          <span className="font-medium flex items-center gap-1">
            <Zap className="h-4 w-4" /> {job.name}
          </span>
...
export function WorkflowMonitor({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-base flex items-center gap-2">
          <Settings className="h-5 w-5" /> Process Monitor
        </CardTitle>
      </CardHeader>
    ...
    <div className="space-y-6">
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {[
          ...
          {
            label: 'Approval Turnaround (hrs)',
            value: analytics.approvalTurnaroundHours,
          },
      ...
      <BarChartPanel
        title="Module Throughput"
        data={analytics.moduleThroughput}
      />
    </div>
  );
}
