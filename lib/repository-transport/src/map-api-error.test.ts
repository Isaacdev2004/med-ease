...
describe('mapApiError', () => {
  ...
  it('maps 404 to NotFoundError', () => {
    const error = mapApiError(
      new ApiError(mockResponse(404, 'Not Found'), null, {
        method: 'GET',
        url: '/api/iam/users/1',
      }),
    );
  ...
  it('maps 409 to ConflictError', () => {
    const error = mapApiError(
      new ApiError(mockResponse(409, 'Conflict'), null, {
        method: 'POST',
        url: '/api/iam/users',
      }),
    );
  ...
  it('maps 429 to RateLimitError', () => {
    const error = mapApiError(
      new ApiError(mockResponse(429, 'Too Many Requests'), null, {
        method: 'GET',
        url: '/api/iam/users',
      }),
    );
